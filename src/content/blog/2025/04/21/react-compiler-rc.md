...
O [React Compiler](https://react.dev/learn/react-compiler) é uma ferramenta em tempo de build que otimiza seu aplicativo React através de memoização automática. No ano passado, publicamos o [primeiro beta](https://react.dev/blog/2024/10/21/react-compiler-beta-release) do React Compiler e recebemos muito feedback e contribuições excelentes. Estamos animados com os sucessos que vimos de pessoas adotando o compilador (veja estudos de caso do [Sanity Studio](https://github.com/reactwg/react-compiler/discussions/33) e [Wakelet](https://github.com/reactwg/react-compiler/discussions/52)) e estamos trabalhando para um lançamento estável.
...
Também ouvimos da comunidade que a validação de `ref` em renderizações às vezes tem falsos positivos. Como uma filosofia geral, queremos que você possa confiar totalmente nas mensagens de erro e dicas do compilador, estamos desativando-a por padrão por enquanto. Continuaremos trabalhando para melhorar essa validação e a reativaremos em uma versão futura.
...
Se você já instalou `eslint-plugin-react-compiler`, agora pode removê-lo e usar `eslint-plugin-react-hooks@rc`. Muitos agradecimentos a [@michaelfaith](https://bsky.app/profile/michael.faith) por contribuir para esta melhoria!
...
O linter não requer que o compilador seja instalado, portanto, não há risco em atualizar `eslint-plugin-react-hooks`. Recomendamos que todos atualizem hoje.
...
O React Compiler funciona melhor quando a auto-memoização aplicada é estritamente para performance. Versões futuras do compilador podem alterar a forma como a memoização é aplicada, por exemplo, ela pode se tornar mais granular e precisa.

No entanto, como o código do produto pode às vezes violar as [regras do React](https://react.dev/reference/rules) de maneiras que nem sempre são detectáveis estaticamente em JavaScript, a alteração da memoização pode ocasionalmente ter resultados inesperados. Por exemplo, um valor previamente memoizado pode ser usado como dependência para um `useEffect` em algum lugar na árvore de componentes. Alterar como ou se esse valor é memoizado pode causar disparos excessivos ou insuficientes desse `useEffect`. Embora incentivemos o [useEffect apenas para sincronização](https://react.dev/learn/synchronizing-with-effects), seu codebase pode ter `useEffect`s que cobrem outros casos de uso, como efeitos que precisam ser executados apenas em resposta a valores específicos que mudam.
...
* ✅ Beta Público: Disponível hoje, para feedback de autores de bibliotecas.
...
Após o lançamento estável, planejamos adicionar mais otimizações e melhorias ao compilador. Isso inclui melhorias contínuas na memoização automática e novas otimizações, com pouca ou nenhuma alteração no código do produto. Cada atualização continuará a melhorar o desempenho e adicionar melhor tratamento a diversos padrões JavaScript e React.

---

Graças a [Joe Savona](https://x.com/en_JS), [Jason Bonta](https://x.com/someextent), [Jimmy Lai](https://x.com/feedthejim) e [Kang Dongyoon](https://x.com/kdy1dev) (@kdy1dev) por revisar e editar este post.