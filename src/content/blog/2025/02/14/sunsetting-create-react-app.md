...
Se seu aplicativo tiver restrições incomuns, ou se você preferir resolver esses problemas criando seu próprio framework, ou se apenas quiser aprender como o React funciona do zero, você pode criar sua própria configuração personalizada com React usando Vite, Parcel ou Rsbuild.
...
Assim como o Svelte tem o Sveltekit, o Vue tem o Nuxt e o Solid tem o SolidStart, o [React recomenda o uso de um framework](#why-we-recommend-frameworks) que integra totalmente o roteamento em recursos como data-fetching e code-splitting pronto para uso. Isso evita o incômodo de precisar escrever suas próprias configurações complexas e, essencialmente, construir um framework você mesmo.
...
Mas fazer isso significa que os dados são buscados após a renderização do componente, o que pode causar "network waterfalls" (cascatas de rede). Network waterfalls são causadas pela busca de dados quando seu aplicativo renderiza em vez de em paralelo enquanto o código está sendo baixado:
...
Server Components não exigem um servidor. Eles podem ser executados no momento da compilação no seu servidor de CI para criar um aplicativo com site estático gerado (SSG), ou em tempo de execução em um servidor web para um aplicativo renderizado no lado do servidor (SSR).
