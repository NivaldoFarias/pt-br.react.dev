...
Isso permite que você comece imediatamente a codificar em React com recursos como JSX, regras de linting padrão e um bundler para executar em desenvolvimento e produção. No entanto, essa configuração não possui as ferramentas que você precisa para construir um aplicativo de produção real.
...
Server Components não exigem um servidor. Eles podem ser executados no momento da compilação em seu servidor de CI para criar um aplicativo gerado estaticamente (SSG), ou em tempo de execução em um servidor web para um aplicativo renderizado no lado do servidor (SSR).
