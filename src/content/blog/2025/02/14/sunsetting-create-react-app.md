...
Mas fazer isso significa que os dados são buscados após a renderização do componente, o que pode causar "network waterfalls" (quedas em cascata de rede). Network waterfalls são causadas pela busca de dados quando seu aplicativo renderiza em vez de em paralelo enquanto o código está sendo baixado:
...
É por isso que a equipe do Chrome tem incentivado os desenvolvedores a considerar a renderização estática ou no lado do servidor em vez de uma abordagem totalmente do lado do cliente para alcançar o melhor desempenho possível.
