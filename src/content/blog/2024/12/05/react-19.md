...
A transição assíncrona definirá imediatamente o estado `isPending` como true, fará as solicitações assíncronas e reverterá `isPending` para false após quaisquer transições. Isso permite que você mantenha a interface do usuário atual responsiva e interativa enquanto os dados estão sendo alterados.
...
Quando o componente for desmontado, o React chamará a função de limpeza retornada do callback `ref`. Isso funciona para refs de DOM, refs de componentes de classe e `useImperativeHandle`.
...
Folhas de estilo, tanto vinculadas externamente (`<link rel="stylesheet" href="...">`) quanto inline (`<style>...</style>`), exigem posicionamento cuidadoso no DOM devido às regras de precedência de estilo. Construir uma capacidade de folha de estilo que permita a composição dentro de componentes é difícil, então os usuários frequentemente acabam carregando todos os seus estilos longe dos componentes que podem depender deles, ou usam uma biblioteca de estilo que encapsula essa complexidade.
...
Bibliotecas de estilo e integrações de estilo com bundlers também podem adotar essa nova capacidade, então mesmo que você não renderize suas próprias folhas de estilo diretamente, você ainda pode se beneficiar à medida que suas ferramentas são atualizadas para usar esse recurso.
...
No Server Side Rendering, scripts assíncronos serão incluídos no `<head>` e priorizados atrás de recursos mais críticos que bloqueiam a renderização, como folhas de estilo, fontes e pré-carregamentos de imagem.
...
No React 19, tags inesperadas no `<head>` e `<body>` serão ignoradas, evitando erros de incompatibilidade. Se o React precisar re-renderizar o documento inteiro devido a uma incompatibilidade de hidratação não relacionada, ele deixará no lugar as folhas de estilo inseridas por scripts e extensões de navegador de terceiros.
