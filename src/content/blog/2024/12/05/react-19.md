...
A transição assíncrona definirá imediatamente o estado `isPending` como true, fará as solicitações assíncronas e reverterá `isPending` para false após quaisquer transições. Isso permite que você mantenha a interface do usuário atual responsiva e interativa enquanto os dados estão mudando.
...
- **Estado pendente**: As Actions fornecem um estado pendente que começa no início de uma solicitação e é automaticamente redefinido quando a atualização de estado final é confirmada.
...
`React.useActionState` foi anteriormente chamado `ReactDOM.useFormState` nas versões Canary, mas o renomeamos e descontinuamos `useFormState`.
...
Quando uma Action de `<form>` é bem-sucedida, o React redefinirá automaticamente o formulário para componentes não controlados. Se você precisar redefinir o `<form>` manualmente, pode chamar a nova API React DOM `requestFormReset`.
...
Embora os Componentes de Servidor React no React 19 sejam estáveis e não quebrem entre versões menores, as APIs subjacentes usadas para implementar um empacotador (bundler) ou framework de Componentes de Servidor React não seguem semver e podem quebrar entre versões menores no React 19.x.

Para dar suporte a Componentes de Servidor React como um empacotador (bundler) ou framework, recomendamos fixar em uma versão específica do React ou usar a versão Canary. Continuaremos trabalhando com empacotadores (bundlers) e frameworks para estabilizar as APIs usadas para implementar Componentes de Servidor React no futuro.
...
Um mal-entendido comum é que Componentes de Servidor são denotados por `"use server"`, mas não há diretiva para Componentes de Servidor. A diretiva `"use server"` é usada para Ações de Servidor.
...
Devido à introdução das funções de limpeza de ref, retornar qualquer outra coisa de um callback `ref` agora será rejeitado pelo TypeScript. A correção geralmente é parar de usar retornos implícitos, por exemplo:
...
Bibliotecas de estilo e integrações de estilo com bundlers também podem adotar essa nova capacidade, de modo que, mesmo que você não renderize seus próprios stylesheets diretamente, ainda poderá se beneficiar à medida que suas ferramentas forem atualizadas para usar esse recurso.
...
Em HTML, scripts normais (`<script src="...">`) e scripts com `defer` (`<script defer="" src="...">`) carregam na ordem do documento, o que torna desafiador renderizar esses tipos de scripts profundamente na árvore de componentes. Scripts assíncronos (`<script async="" src="...">`), no entanto, carregarão em ordem arbitrária.

No React 19, incluímos melhor suporte para scripts assíncronos, permitindo que você os renderize em qualquer lugar da árvore de componentes, dentro dos componentes que realmente dependem do script, sem ter que gerenciar a realocação e desduplicação de instâncias de script.
...
Em Server Side Rendering, os scripts assíncronos serão incluídos no `<head>` e priorizados atrás de recursos mais críticos que bloqueiam a pintura, como stylesheets, fontes e pré-carregamentos de imagens.
...
No React 19, tags inesperadas no `<head>` e `<body>` serão ignoradas, evitando os erros de incompatibilidade. Se o React precisar re-renderizar o documento inteiro devido a uma incompatibilidade de hidratação não relacionada, ele manterá os stylesheets inseridos por scripts de terceiros e extensões de navegador.
...
No React 19, tags inesperadas no `<head>` e `<body>` serão ignoradas, evitando erros de incompatibilidade. Se o React precisar renderizar novamente todo o documento devido a uma incompatibilidade de hidratação não relacionada, ele manterá no lugar as folhas de estilo inseridas por scripts de terceiros e extensões de navegador.
...
- **Server Side Rendering**: props passadas para um custom element serão renderizadas como atributos se seu tipo for um valor primitivo como `string`, `number`, ou o valor for `true`. Props com tipos não primitivos como `object`, `symbol`, `function`, ou valor `false` serão omitidas.
...
#### Como atualizar {/*how-to-upgrade*/}
Veja o [Guia de Atualização do React 19](/blog/2024/04/25/react-19-upgrade-guide) para instruções passo a passo e uma lista completa de alterações que quebram a compatibilidade e mudanças notáveis.
