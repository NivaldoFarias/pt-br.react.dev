...
Para dar suporte a Componentes de Servidor React como um bundler ou framework, recomendamos fixar em uma versão específica do React, ou usar a release Canary. Continuaremos trabalhando com bundlers e frameworks para estabilizar as APIs usadas para implementar Componentes de Servidor React no futuro.
...
Novos componentes de função não precisarão mais de `forwardRef`, e publicaremos um codemod para atualizar automaticamente seus componentes para usar a nova prop `ref`. Em versões futuras, desativaremos e removeremos `forwardRef`.
...
Novos providers de Contexto podem usar `<Context>` e publicaremos um codemod para converter os providers existentes. Em versões futuras, desativaremos `<Context.Provider>`.
...
Em versões futuras, desativaremos a chamada de refs com `null` ao desmontar componentes.

</Note>

Devido à introdução de funções de limpeza de refs, retornar qualquer outra coisa de um callback `ref` agora será rejeitado pelo TypeScript. A correção geralmente é parar de usar retornos implícitos, por exemplo:
...
Stylesheets, tanto vinculados externamente (`<link rel="stylesheet" href="...">`) quanto inline (`<style>...</style>`), exigem posicionamento cuidadoso no DOM devido às regras de precedência de estilo. Construir uma capacidade de stylesheet que permita a composição dentro de componentes é difícil, então os usuários geralmente acabam carregando todos os seus estilos longe dos componentes que podem depender deles, ou usam uma biblioteca de estilo que encapsula essa complexidade.
...
O React 19 inclui uma série de novas APIs para carregar e pré-carregar recursos do navegador para tornar o mais fácil possível construir ótimas experiências que não são prejudicadas por carregamento ineficiente de recursos.
...
Melhoramos a hidratação para acomodar scripts de terceiros e extensões de navegador.
