...
A transição assíncrona definirá imediatamente `isPending` como true, fará a(s) solicitação(ões) assíncrona(s) e definirá `isPending` como false após quaisquer transições. Isso permite que você mantenha a interface do usuário atual responsiva e interativa enquanto os dados estão mudando.
...
- **Estado pendente**: As Actions fornecem um estado pendente que começa no início de uma solicitação e é redefinido automaticamente quando a atualização final do estado é confirmada.
...
As Actions também são integradas aos novos recursos de `<form>` do React 19 para `react-dom`. Adicionamos suporte para passar funções como props `action` e `formAction` dos elementos `<form>`, `<input>` e `<button>` para enviar formulários automaticamente com Actions:
...
Devido à introdução das funções de limpeza de refs, retornar qualquer outra coisa de um callback `ref` agora será rejeitado pelo TypeScript. A correção geralmente é parar de usar retornos implícitos, por exemplo:
...
Durante o Client Side Rendering, o React esperará que os stylesheets recém-renderizados sejam carregados antes de confirmar a renderização. Se você renderizar este componente de vários lugares dentro de sua aplicação, o React incluirá o stylesheet apenas uma vez no documento:
...
No React 19, incluímos melhor suporte para scripts assíncronos, permitindo que você os renderize em qualquer lugar da árvore de componentes, dentro dos componentes que realmente dependem do script, sem ter que gerenciar a realocação e deduplicação de instâncias de script.
...
Em versões anteriores, usar Custom Elements no React era difícil porque o React tratava props não reconhecidas como atributos em vez de propriedades. No React 19, adicionamos suporte para propriedades que funciona no cliente e durante SSR com a seguinte estratégia:
