...
- **Formulários**: Elementos `<form>` agora suportam o envio de funções para as props `action` e `formAction`. O envio de funções para as props `action` usa Actions por padrão e redefine o formulário automaticamente após o envio.
...
As Actions também são integradas aos novos recursos `<form>` do React 19 para `react-dom`. Adicionamos suporte para passar funções como props `action` e `formAction` dos elementos `<form>`, `<input>` e `<button>` para enviar formulários automaticamente com Actions:
...
No Server Side Rendering, os scripts assíncronos serão incluídos no `<head>` e priorizados atrás de recursos mais críticos que bloqueiam a renderização, como stylesheets, fontes e pré-carregamentos de imagens.
