...
date: 2024/12/05
description: React 19 está agora disponível no npm! Neste post, daremos uma visão geral dos novos recursos do React 19 e como você pode adotá-los.
---
...
React v19 está agora disponível no npm!
...
Para uma lista de alterações que quebram a compatibilidade, veja o [Guia de Atualização](/blog/2024/04/25/react-19-upgrade-guide).
...
Um componente foi suspenso por uma promise não cacheada. Criar promises dentro de um Client Component ou hook ainda não é suportado, exceto via uma biblioteca ou framework compatível com Suspense.
...
Anteriormente, o React chamava as funções de `ref` com `null` ao desmontar o componente. Se sua `ref` retornar uma função de limpeza, o React agora pulará esta etapa.
...
No React 19, tags inesperadas no `<head>` e `<body>` serão ignoradas, evitando erros de incompatibilidade. Se o React precisar re-renderizar o documento inteiro devido a uma incompatibilidade de hidratação não relacionada, ele deixará no lugar os stylesheets inseridos por scripts e extensões de navegador de terceiros.
...
- `onUncaughtError`: chamada quando um erro é lançado e não capturado por um Error Boundary.
...
#### Como atualizar {/*how-to-upgrade*/}
Veja o [Guia de Atualização do React 19](/blog/2024/04/25/react-19-upgrade-guide) para instruções passo a passo e uma lista completa de alterações que quebram e notáveis.
