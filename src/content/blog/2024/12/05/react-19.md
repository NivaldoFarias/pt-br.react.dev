...
December 05, 2024 por [The React Team](/community/team)
...
No nosso [Guia de Atualização do React 19](/blog/2024/04/25/react-19-upgrade-guide), compartilhamos instruções passo a passo para atualizar seu aplicativo para o React 19. Neste post, daremos uma visão geral dos novos recursos no React 19 e como você pode adotá-los.
...
No React 19, estamos adicionando suporte para o uso de funções assíncronas em transições para tratar automaticamente estados pendentes, erros, formulários e atualizações otimistas.

Por exemplo, você pode usar `useTransition` para manipular o estado pendente para você:
...
- **Estado pendente**: Actions fornecem um estado pendente que começa no início de uma solicitação e redefine automaticamente quando a atualização final do estado é confirmada.
- **Atualizações otimistas**: Actions oferecem suporte ao novo hook [`useOptimistic`](#new-hook-optimistic-updates) para que você possa mostrar aos usuários feedback instantâneo enquanto as solicitações estão sendo enviadas.
- **Tratamento de erros**: Actions fornecem tratamento de erros para que você possa exibir Error Boundaries quando uma solicitação falhar e reverter atualizações otimistas para seu valor original automaticamente.
- **Formulários**: elementos `<form>` agora suportam a passagem de funções para as propriedades `action` e `formAction`. A passagem de funções para os props `action` usam Actions por padrão e reiniciam o formulário automaticamente após o envio.
...
`useActionState` aceita uma função (a "Action") e retorna uma Action encapsulada para chamar. Isso funciona porque Actions compõem. Quando a Action encapsulada for chamada, `useActionState` retornará o último resultado da Action como `data` e o estado pendente da Action como `pending`.
...
Actions também estão integrados aos novos recursos de `<form>` do React 19 para `react-dom`. Adicionamos suporte para a passagem de funções como as propriedades `action` e `formAction` dos elementos `<form>`, `<input>` e `<button>` para enviar formulários automaticamente com Actions:

```js [[1,1,"actionFunction"]]
<form action={actionFunction}>
```

Quando uma `<form>` Action é bem-sucedida, o React redefinirá automaticamente o formulário para componentes não controlados. Se você precisar redefinir o `<form>` manualmente, poderá chamar a nova API do React DOM `requestFormReset`.

Para obter mais informações, consulte a documentação `react-dom` para [`<form>`](/reference/react-dom/components/form), [`<input>`](/reference/react-dom/components/input) e `<button>`.
...
Para obter mais informações, consulte a documentação `react-dom` para [`useFormStatus`](/reference/react-dom/hooks/useFormStatus).

### Novo hook: `useOptimistic` {/*new-hook-optimistic-updates*/}

Outro padrão comum da UI ao realizar uma mutação de dados é mostrar o estado final de forma otimista enquanto a solicitação assíncrona está em andamento. No React 19, estamos adicionando um novo hook chamado `useOptimistic` para facilitar isso:
...
O hook `useOptimistic` renderizará imediatamente o `optimisticName` enquanto a solicitação `updateName` estiver em andamento. Quando a atualização terminar ou ocorrer um erro, o React alternará automaticamente de volta para o valor `currentName`.
...
#### `use` não suporta promises criadas em render. {/*use-does-not-support-promises-created-in-render*/}

Se você tentar passar uma promise criada no render para `use`, React avisará:
...
Para corrigir, você precisa passar uma promise de uma biblioteca ou framework com suporte para Suspense que suporte caching para promises. No futuro, planejamos enviar recursos para facilitar o caching de promises no render.
...
A API `use` só pode ser chamada no render, semelhante aos hooks. Ao contrário dos hooks, `use` pode ser chamado condicionalmente. No futuro, planejamos oferecer suporte a mais maneiras de consumir recursos no render com `use`.
...
As APIs prerender aguardarão o carregamento de todos os dados antes de retornar o fluxo HTML estático. Os fluxos podem ser convertidos em strings ou enviados com uma resposta de streaming. Eles não suportam conteúdo de streaming à medida que ele carrega, o que é suportado pelas [APIs de renderização do servidor React DOM](/reference/react-dom/server) existentes.
...
O React 19 inclui todos os recursos do React Server Components incluídos do canal Canary. Isso significa que as bibliotecas que são fornecidas com os Server Components agora podem ter como alvo o React 19 como uma dependência peer com uma [condição de exportação](https://github.com/reactjs/rfcs/blob/main/text/0227-server-module-conventions.md#react-server-conditional-exports) `react-server` para uso em frameworks que suportam a [Arquitetura Full-stack React](/learn/start-a-new-react-project#which-features-make-up-the-react-teams-full-stack-architecture-vision).

<Note>

#### Como faço para criar suporte para Server Components? {/*how-do-i-build-support-for-server-components*/}

Embora o React Server Components no React 19 seja estável e não quebre entre as versões secundárias, as APIs subjacentes usadas para implementar um bundler ou framework do React Server Components não seguem o semver e podem quebrar entre as versões secundárias no React 19.x.

Para suportar o React Server Components como um bundler ou framework, recomendamos fixar em uma versão específica do React ou usar a versão Canary. Continuaremos trabalhando com bundlers e frameworks para estabilizar as APIs usadas para implementar o React Server Components no futuro.
...
Quando um Server Action é definido com a diretiva `"use server"`, seu framework criará automaticamente uma referência à função do servidor e passará essa referência ao Client Component. Quando essa função é chamada no cliente, o React enviará uma solicitação ao servidor para executar a função e retornar o resultado.

<Note>

#### Não há diretiva para Server Components. {/*there-is-no-directive-for-server-components*/}

Um equívoco comum é que os Server Components são denotados por `"use server"`, mas não há diretiva para Server Components. A diretiva `"use server"` é usada para Server Actions.

Para mais informações, consulte a documentação para [Diretivas](/reference/rsc/directives).

</Note>

Server Actions podem ser criados em Server Components e passados ​​como props para Client Components, ou podem ser importados e usados ​​em Client Components.
...
`refs` passados ​​para classes não são passados ​​como props, pois eles referenciam a instância do componente.

</Note>

### Diffs para erros de hidratação {/*diffs-for-hydration-errors*/}

Também melhoramos a geração de relatórios de erros para erros de hidratação em `react-dom`. Por exemplo, em vez de registrar vários erros no DEV sem nenhuma informação sobre a incompatibilidade:
...
Agora, registramos uma única mensagem com a diferença da incompatibilidade:

<ConsoleBlockMulti>

<ConsoleLogLine level="error">

Erro não capturado: A hidratação falhou porque o HTML renderizado pelo servidor não corresponde ao cliente. Como resultado, esta árvore será regenerada no cliente. Isso pode acontecer se um Componente do Cliente SSR-ed usou:{'\n'}
\- Um `if (typeof window !== 'undefined')` de ramificação de servidor/cliente.
\- Entrada de variável como `Date.now()` ou `Math.random()` que muda a cada vez que é chamada.
\- Formatação de data na localidade de um usuário que não corresponde ao servidor.
\- Dados externos em mudança sem enviar um snapshot junto com o HTML.
\- Aninhamento de tag HTML inválido.{'\n'}
Também pode acontecer se o cliente tiver uma extensão de navegador instalada que mexe com o HTML antes que o React seja carregado.{'\n'}
...
Novos provedores Context podem usar `<Context>` e estaremos publicando um codemod para converter provedores existentes. Em versões futuras, descontinuaremos `<Context.Provider>`.

### Funções de limpeza para refs {/*cleanup-functions-for-refs*/}

Agora oferecemos suporte para retornar uma função de limpeza de retornos de chamada `ref`:
...
Quando o componente desmontar, React chamará a função de limpeza retornada do retorno de chamada `ref`. Isso funciona para refs do DOM, refs para componentes de classe e `useImperativeHandle`.

<Note>

Anteriormente, React chamaria funções `ref` com `null` ao desmontar o componente. Se seu `ref` retornar uma função de limpeza, o React agora ignorará esta etapa.
...
O código original retornava a instância do `HTMLDivElement` e o TypeScript não saberia se isso _deveria_ ser uma função de limpeza ou se você não queria retornar uma função de limpeza.

Você pode transformar esse padrão com [`no-implicit-ref-callback-return`](https://github.com/eps1lon/types-react-codemod/#no-implicit-ref-callback-return).
...
Quando o React renderiza este componente, ele verá as tags `<title>`, `<link>` e `<meta>` e as moverá automaticamente para a seção `<head>` do documento. Ao oferecer suporte a essas tags de metadados nativamente, podemos garantir que elas funcionem com aplicativos somente no cliente, SSR de streaming e Server Components.

<Note>

#### Você ainda pode querer uma biblioteca de metadados {/*you-may-still-want-a-metadata-library*/}

Para casos de uso simples, renderizar Metadados do Documento como tags pode ser adequado, mas as bibliotecas podem oferecer recursos mais poderosos, como substituir metadados genéricos por metadados específicos com base na rota atual. Esses recursos facilitam para frameworks e bibliotecas como, por exemplo, [`react-helmet`](https://github.com/nfl/react-helmet), oferecer suporte a tags de metadados, em vez de substituí-las.
...
As folhas de estilo, tanto as vinculadas externamente (`<link rel="stylesheet" href="...">`) quanto as embutidas (`<style>...</style>`), exigem um posicionamento cuidadoso no DOM devido às regras de precedência de estilo. Criar um recurso de folha de estilo que permita a composição dentro dos componentes é difícil, então os usuários costumam acabar carregando todos os seus estilos longe dos componentes que podem depender deles, ou usam uma biblioteca de estilo que encapsula essa complexidade.
...
Durante o Server Side Rendering, o React incluirá a folha de estilo no `<head>`, o que garante que o navegador não renderize até que ela tenha sido carregada. Se a folha de estilo for descoberta tardiamente depois que já começamos a transmitir, o React garantirá que a folha de estilo seja inserida no `<head>` no cliente antes de revelar o conteúdo de um limite Suspense que depende dessa folha de estilo.

Durante o Client Side Rendering, o React aguardará o carregamento de folhas de estilo recém-renderizadas antes de confirmar a renderização. Se você renderizar este componente de vários lugares dentro do seu aplicativo, o React incluirá a folha de estilo apenas uma vez no documento:
...
Para usuários acostumados a carregar folhas de estilo manualmente, esta é uma oportunidade de localizar essas folhas de estilo ao lado dos componentes que dependem delas, permitindo um raciocínio local melhor e um tempo mais fácil para garantir que você carregue apenas as folhas de estilo das quais realmente depende.

Bibliotecas de estilo e integrações de estilo com empacotadores também podem adotar essa nova capacidade, então, mesmo que você não renderize diretamente suas próprias folhas de estilo, você ainda pode se beneficiar à medida que suas ferramentas são atualizadas para usar esse recurso.
...
No HTML, scripts normais (`<script src="...">`) e scripts adiados (`<script defer="" src="...">`) são carregados na ordem do documento, o que torna a renderização desse tipo de script profundamente em sua árvore de componentes desafiadora. No entanto, os scripts assíncronos (`<script async="" src="...">`) serão carregados em ordem arbitrária.
...
function App() {
  <html>
    <body>
      <MyComponent>
      ...
      <MyComponent> // não resultará em script duplicado no DOM
    </body>
  </html>
}
```
...
No Server Side Rendering, os scripts assíncronos serão incluídos no `<head>` e priorizados em relação a recursos mais críticos que bloqueiam a pintura, como folhas de estilo, fontes e pré-carregamento de imagem.
...
O React 19 inclui vários novos APIs para carregar e pré-carregar os recursos do navegador para tornar o mais fácil possível criar ótimas experiências que não são prejudicadas pelo carregamento ineficiente de recursos.
...
function MyComponent() {
  ...
  prefetchDNS('https://...') // quando você não pode realmente solicitar nada desse host
...
<html>
  <head>
    <!-- links/scripts são priorizados por sua utilidade para carregamento antecipado, não ordem de chamada -->
...
Essas APIs podem ser usadas para otimizar o carregamento inicial da página, movendo a descoberta de recursos adicionais, como fontes, do carregamento da folha de estilo. Eles também podem tornar as atualizações do cliente mais rápidas, pré-buscando uma lista de recursos usados por uma navegação antecipada e, em seguida, pré-carregando ansiosamente esses recursos no clique ou até mesmo ao passar o mouse.
...
Melhoramos a hidratação para ter em conta scripts de terceiros e extensões de navegador.
...
No React 19, as tags inesperadas no `<head>` e no `<body>` serão ignoradas, evitando os erros de incompatibilidade. Se o React precisar renderizar novamente o documento inteiro devido a um erro de incompatibilidade de hidratação não relacionado, ele deixará no lugar as folhas de estilo inseridas por scripts de terceiros e extensões de navegador.

### Melhor relatório de erros {/*error-handling*/}

Melhoramos o tratamento de erros no React 19 para remover a duplicação e fornecer opções para lidar com erros capturados e não capturados. Por exemplo, quando há um erro na renderização capturado por um Error Boundary, o React costumava lançar o erro duas vezes (uma vez para o erro original e, em seguida, novamente após falhar em se recuperar automaticamente) e, em seguida, chamava `console.error` com informações sobre onde o erro ocorreu.
...
Em versões anteriores, usar Custom Elements no React era difícil porque o React tratava as props não reconhecidas como atributos em vez de propriedades. No React 19, adicionamos suporte para propriedades que funcionam no cliente e durante o SSR com a seguinte estratégia:

- **Server Side Rendering**: as props passadas para um elemento personalizado serão renderizadas como atributos se seu tipo for um valor primitivo como `string`, `number` ou o valor for `true`. Props com tipos não primitivos como `object`, `symbol`, `function` ou valor `false` serão omitidos.
- **Client Side Rendering**: as props que correspondem a uma propriedade na instância do Custom Element serão atribuídas como propriedades, caso contrário, elas serão atribuídas como atributos.

Agradecimentos a [Joey Arhar](https://github.com/josepharhar) por conduzir o projeto e a implementação do suporte a Custom Element no React.
