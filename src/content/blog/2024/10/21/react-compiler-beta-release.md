...
author: Lauren Tan
date: 2024/10/21
description: Na React Conf 2024, anunciamos o lançamento experimental do React Compiler, uma ferramenta de build-time que otimiza seu app React através de memoização automática. Neste post, queremos compartilhar o que vem a seguir para o open source e nosso progresso no compilador.
...
2. Estamos oficialmente suportando o React Compiler para apps no React 17+, através de um pacote opcional `react-compiler-runtime`.
...
Desde o primeiro lançamento, corrigimos inúmeros bugs reportados pela comunidade React, recebemos várias correções de bugs e contribuições de alta qualidade[^1] para o compilador, tornamos o compilador mais resiliente à ampla diversidade de padrões JavaScript e continuamos a implantar o compilador mais amplamente na Meta.
...
Nosso lançamento inicial focou em identificar problemas importantes com o uso do compilador em aplicações. Recebemos ótimos feedbacks e melhoramos substancialmente o compilador desde então. Agora estamos prontos para um feedback amplo da comunidade, e para que os autores de bibliotecas experimentem o compilador para melhorar o desempenho e a experiência do desenvolvedor na manutenção de suas bibliotecas.
...
A partir de hoje, juntamente com o lançamento Beta do React Compiler, estamos abrindo a adesão ao Working Group para todos. O objetivo do React Compiler Working Group é preparar o ecossistema para uma adoção suave e gradual do React Compiler por aplicações e bibliotecas existentes. Por favor, continue enviando relatórios de bugs no [repositório do React](https://github.com/facebook/react), mas deixe seu feedback, faça perguntas ou compartilhe ideias no [fórum de discussão do Working Group](https://github.com/reactwg/react-compiler/discussions).
...
Esses lançamentos também incluem o plugin ESLint do compilador, que expõe diagnósticos analisados estaticamente pelo compilador. Planejamos combinar o plugin `eslint-plugin-react-hooks` existente com o plugin ESLint do compilador, para que apenas um plugin precise ser instalado.
...
[^3]: Após controlar o tempo de experiência do autor, o comprimento/complexidade das diffs e outros fatores potenciais de confusão.