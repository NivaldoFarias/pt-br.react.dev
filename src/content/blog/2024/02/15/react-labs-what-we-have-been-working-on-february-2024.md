...
O Compilador React não é mais um projeto de pesquisa: o compilador agora impulsiona o instagram.com em produção, e estamos trabalhando para implantar o compilador em superfícies adicionais na Meta e para preparar o primeiro lançamento de código aberto.
...
Claro, entendemos que os desenvolvedores às vezes dobram as regras um pouco, e nosso objetivo é fazer com que o Compilador React funcione "out of the box" na maior parte do código possível. O compilador tenta detectar quando o código não segue estritamente as regras do React e irá compilar o código onde for seguro ou pular a compilação se não for seguro. Estamos testando contra a base de código grande e variada da Meta para ajudar a validar essa abordagem.
...
Para ver o compilador em ação, você pode conferir nossa [palestra do outono passado](https://www.youtube.com/watch?v=qOQClO3g8-Y). Na época da palestra, tínhamos dados experimentais iniciais de testar o Compilador React em uma página do instagram.com. Desde então, implantamos o compilador em produção em todo o instagram.com. Também expandimos nossa equipe para acelerar a implantação em superfícies adicionais na Meta e para código aberto. Estamos animados com o caminho à frente e teremos mais a compartilhar nos próximos meses.
...
Nós nos referimos a essa coleção mais ampla de recursos simplesmente como "Ações". As Ações permitem que você passe uma função para elementos DOM como [`<form/>`](/reference/react-dom/components/form):
...
React Server Components, Asset Loading, Document Metadata e Actions já chegaram ao React Canary, e adicionamos documentação para esses recursos em react.dev:
...
Activity ainda está sob pesquisa e nosso trabalho restante é finalizar os primitivos que são expostos aos desenvolvedores de bibliotecas. Priorizamos essa área enquanto nos concentramos em lançar recursos mais completos.
