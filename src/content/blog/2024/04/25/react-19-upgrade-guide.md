...
author: Ricky Hanlon
date: 2024/04/25
description: As melhorias adicionadas ao React 19 exigem algumas breaking changes, mas trabalhamos para tornar a atualização o mais tranquila possível e não esperamos que as mudanças impactem a maioria dos apps. Neste post, vamos guiá-lo pelas etapas de atualização de apps e bibliotecas para o React 19.
---
...
Para obter uma lista das mudanças no 18.3, consulte as [Notas de versão](https://github.com/facebook/react/blob/main/CHANGELOG.md#1830-april-25-2024).

</Note>

Neste post, vamos guiá-lo pelas etapas de atualização para o React 19:
...
Seu app (ou uma de suas dependências) está usando um transform JSX desatualizado. Atualize para o transform JSX moderno para um desempenho mais rápido: https://react.dev/link/new-jsx-transform
...
Esperamos que a maioria dos apps não seja afetada, pois o transform já está habilitado na maioria dos ambientes. Para obter instruções manuais sobre como atualizar, consulte a [publicação de anúncio](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html).
...
Isso não inclui as mudanças do TypeScript. Veja [Mudanças no TypeScript](#typescript-changes) abaixo.

</Note>

As mudanças que incluem um codemod incluem o comando abaixo.

Para obter uma lista de todos os codemods disponíveis, consulte o [`react-codemod` repo](https://github.com/reactjs/react-codemod).

## Breaking changes {/*breaking-changes*/}

### Erros em render não são relançados {/*errors-in-render-are-not-re-thrown*/}

Em versões anteriores do React, os erros lançados durante a renderização eram capturados e relançados. Em DEV, também faríamos o log para `console.error`, resultando em logs de erro duplicados.
...
Essa mudança não deve impactar a maioria dos apps, mas se seu relatório de erros de produção depender de erros sendo relançados, talvez seja necessário atualizar seu tratamento de erros. Para dar suporte a isso, adicionamos novos métodos a `createRoot` e `hydrateRoot` para tratamento de erros personalizado:
...
Para obter mais informações, consulte os documentos para [`createRoot`](https://react.dev/reference/react-dom/client/createRoot) e [`hydrateRoot`](https://react.dev/reference/react-dom/client/hydrateRoot).
...
No React 19, estamos removendo as verificações de `propType` do pacote React, e usá-las será ignorado silenciosamente. Se você estiver usando `propTypes`, recomendamos migrar para o TypeScript ou outra solução de verificação de tipo.

Também estamos removendo `defaultProps` de componentes de função em vez de parâmetros padrão ES6. Os componentes de classe continuarão a dar suporte a `defaultProps`, pois não há alternativa ES6.
...
#### Removido: string refs {/*removed-string-refs*/}
String refs foram obsoletas em [março de 2018 (v16.3.0)](https://legacy.reactjs.org/blog/2018/03/27/update-on-async-rendering.html).

Os componentes de classe davam suporte a string refs antes de serem substituídos por callbacks de ref devido a [múltiplas desvantagens](https://github.com/facebook/react/issues/1373). No React 19, estamos removendo string refs para tornar o React mais simples e fácil de entender.

Se você ainda estiver usando string refs em componentes de classe, precisará migrar para callbacks de ref:
...
Codemod string refs com callbacks `ref`:
...
#### Removido: Module pattern factories {/*removed-module-pattern-factories*/}
Module pattern factories foram obsoletos em [agosto de 2019 (v16.9.0)](https://legacy.reactjs.org/blog/2019/08/08/react-v16.9.0.html#deprecating-module-pattern-factories).

Este padrão era raramente usado e dar suporte a ele faz com que o React seja um pouco maior e mais lento do que o necessário. No React 19, estamos removendo o suporte para module pattern factories, e você precisará migrar para funções regulares:
...
Usar `createFactory` era comum antes do amplo suporte para JSX, mas raramente é usado hoje e pode ser substituído por JSX. No React 19, estamos removendo `createFactory` e você precisará migrar para JSX:
...
`ReactDOMTestUtils.act` está obsoleto em favor de `React.act`. Importe `act` de `react` em vez de `react-dom/test-utils`. Consulte https://react.dev/warnings/react-dom-test-utils para obter mais informações.
...
Todas as outras funções `test-utils` foram removidas. Esses utilitários eram incomuns e tornavam muito fácil depender de detalhes de implementação de baixo nível de seus componentes e do React. No React 19, essas funções apresentarão erro quando chamadas e suas exportações serão removidas em uma versão futura.
