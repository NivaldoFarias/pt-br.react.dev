...
Todos os codemods estão disponíveis no repositório [`react-codemod`](https://github.com/reactjs/react-codemod) e a equipe Codemod se juntou para ajudar a manter os codemods. Para executar esses codemods, recomendamos usar o comando `codemod` em vez do `react-codemod`, pois ele é executado mais rapidamente, lida com migrações de código mais complexas e fornece melhor suporte para TypeScript.
...
Estamos removendo `findDOMNode` porque era uma "escape hatch" legada que era lenta para executar, frágil a refatorações, retornava apenas o primeiro filho e quebrava níveis de abstração (veja mais [aqui](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Você pode substituir `ReactDOM.findDOMNode` por [refs DOM](/learn/manipulating-the-dom-with-refs):
...
Assim como todos os comportos do Strict Mode, esses recursos são projetados para expor proativamente bugs em seus componentes durante o desenvolvimento, para que você possa corrigi-los antes que sejam enviados para produção. Por exemplo, durante o desenvolvimento, o Strict Mode invocará duas vezes as funções de callback de ref na montagem inicial, para simular o que acontece quando um componente montado é substituído por um fallback do Suspense.
...
Os `props` de elementos React agora têm o valor padrão `unknown` em vez de `any` se o elemento for tipado como `ReactElement`. Isso não afeta você se você passar um argumento de tipo para `ReactElement`:
...
No entanto, isso exigiu uma alteração que quebra a compatibilidade, onde `useReducer` não aceita o tipo completo do reducer como parâmetro de tipo, mas em vez disso precisa de nenhum (e confia na tipagem contextual) ou precisa de ambos os tipos de estado e ação.
