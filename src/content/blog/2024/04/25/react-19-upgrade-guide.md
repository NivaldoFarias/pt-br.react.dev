...
author: Ricky Hanlon
date: 2024/04/25
description: As melhorias adicionadas ao React 19 exigem algumas mudanças significativas, mas trabalhamos para tornar a atualização o mais tranquila possível e não esperamos que as mudanças impactem a maioria dos aplicativos. Neste post, guiaremos você pelas etapas para atualizar aplicativos e bibliotecas para o React 19.
---
...
As melhorias adicionadas ao React 19 exigem algumas mudanças significativas, mas trabalhamos para tornar a atualização o mais tranquila possível, e não esperamos que as mudanças impactem a maioria dos aplicativos.
...
Para obter uma lista de mudanças na 18.3, consulte as [Notas de Lançamento](https://github.com/facebook/react/blob/main/CHANGELOG.md#1830-april-25-2024).
...
Esperamos que a maioria dos aplicativos não seja afetada, pois a transformação já está habilitada na maioria dos ambientes. Para obter instruções manuais sobre como atualizar, consulte o [post de anúncio](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html).
...
### Erros em renderização não são relançados {/*errors-in-render-are-not-re-thrown*/}
...
Esta mudança não deve impactar a maioria dos aplicativos, mas se o seu relatório de erros de produção depender de erros sendo relançados, pode ser necessário atualizar o tratamento de erros. Para suportar isso, adicionamos novos métodos a `createRoot` e `hydrateRoot` para tratamento de erros personalizado:
...
Também estamos removendo `defaultProps` de componentes de função em vez de parâmetros padrão ES6. Componentes de classe continuarão a suportar `defaultProps`, pois não há alternativa ES6.
...
Componentes de classe suportavam refs de string antes de serem substituídos por ref callbacks devido a [várias desvantagens](https://github.com/facebook/react/issues/1373). No React 19, estamos removendo refs de string para tornar o React mais simples e fácil de entender.
...
Todas as outras funções `test-utils` foram removidas. Essas utilidades eram incomuns e tornavam muito fácil depender de detalhes de implementação de baixo nível de seus componentes e do React. No React 19, essas funções gerarão um erro quando chamadas e suas exportações serão removidas em uma versão futura.
...
O React 19 suporta [`ref` como uma prop](/blog/2024/04/25/react-19#ref-as-a-prop), então estamos descontinuando o `element.ref` em vez de `element.props.ref`.
...
Estamos descontinuando `react-test-renderer` porque ele implementa seu próprio ambiente de renderizador que não corresponde ao ambiente que os usuários usam, promove detalhes de implementação de teste e depende da introspecção dos detalhes internos do React.

O renderizador de teste foi criado antes que houvesse estratégias de teste mais viáveis disponíveis, como [React Testing Library](https://testing-library.com), e agora recomendamos o uso de uma biblioteca de teste moderna.
...
O React 19 inclui diversas correções e melhorias no Strict Mode.
...
Como acontece com todos os comportamentos do Strict Mode, esses recursos são projetados para expor proativamente erros em seus componentes durante o desenvolvimento, para que você possa corrigi-los antes de serem enviados para a produção. Por exemplo, durante o desenvolvimento, o Strict Mode invocará duas vezes as funções de retorno de chamada de ref na montagem inicial, para simular o que acontece quando um componente montado é substituído por um fallback do Suspense.

### Melhorias no Suspense {/*improvements-to-suspense*/}

No React 19, quando um componente suspende, o React irá imediatamente confirmar o fallback do limite do Suspense mais próximo sem esperar que toda a árvore de irmãos seja renderizada. Após a confirmação do fallback, o React agenda outra renderização para os irmãos suspensos para "pré-aquecer" as requisições lazy no restante da árvore:
...
Esta versão inclui mudanças nos detalhes internos do React que podem impactar bibliotecas que ignoram nossos apelos para não usar detalhes internos como `SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED`. Essas mudanças são necessárias para implementar melhorias no React 19 e não quebrarão bibliotecas que seguem nossas diretrizes.

Com base em nossa [Política de Versionamento](https://react.dev/community/versioning-policy#what-counts-as-a-breaking-change), essas atualizações não são listadas como mudanças de quebra, e não estamos incluindo documentos sobre como atualizá-las. A recomendação é remover qualquer código que dependa de detalhes internos.

Para refletir o impacto do uso de detalhes internos, renomeamos o sufixo `SECRET_INTERNALS` para:

`_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE`

No futuro, bloquearemos de forma mais agressiva o acesso a detalhes internos do React para desencorajar o uso e garantir que os usuários não sejam impedidos de atualizar.

## Mudanças no TypeScript {/*typescript-changes*/}

### Tipos TypeScript descontinuados removidos {/*removed-deprecated-typescript-types*/}

Fizemos uma limpeza nos tipos TypeScript com base nas APIs removidas no React 19. Alguns dos tipos removidos foram movidos para pacotes mais relevantes, e outros não são mais necessários para descrever o comportamento do React.
...
Confira [`types-react-codemod`](https://github.com/eps1lon/types-react-codemod/) para obter uma lista de substituições suportadas. Se você sentir que um codemod está faltando, ele pode ser rastreado na [lista de codemods do React 19 ausentes](https://github.com/eps1lon/types-react-codemod/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+label%3A%22React+19%22+label%3Aenhancement).

### Limpezas de `ref` necessárias {/*ref-cleanup-required*/}

_Esta mudança está incluída no preset `react-19` codemod como [`no-implicit-ref-callback-return
`](https://github.com/eps1lon/types-react-codemod/#no-implicit-ref-callback-return)._

Devido à introdução de funções de limpeza de ref, retornar qualquer outra coisa de um retorno de chamada de ref agora será rejeitado pelo TypeScript. A correção geralmente é parar de usar retornos implícitos:
...
Uma reclamação de longa data de como TypeScript e React funcionam tem sido `useRef`. Mudamos os tipos para que `useRef` agora exija um argumento. Isso simplifica significativamente sua assinatura de tipo. Agora ele se comportará mais como `createContext`.
...
Isso agora também significa que todas as refs são mutáveis. Você não terá mais o problema de não poder mutar uma ref porque você a inicializou com `null`:
...
As `props` dos elementos React agora são definidas como `unknown` em vez de `any` se o elemento for tipado como `ReactElement`. Isso não afeta você se você passar um argumento de tipo para `ReactElement`:
...
Mas se você confiava no padrão, agora você tem que lidar com `unknown`:
...
Você só deve precisar disso se tiver muito código legado que dependa de acesso não seguro às props do elemento. A introspecção do elemento só existe como uma saída de emergência, e você deve deixar explícito que seu acesso às props é inseguro por meio de um `any` explícito.
...
Um pedido de longa data é remover o namespace global `JSX` de nossos tipos em favor de `React.JSX`. Isso ajuda a evitar a poluição de tipos globais, o que impede conflitos entre diferentes bibliotecas de UI que usam JSX.
...
### Melhores tipagens de `useReducer` {/*better-usereducer-typings*/}

`useReducer` agora tem inferência de tipo aprimorada graças a [@mfp22](https://github.com/mfp22).

No entanto, isso exigiu uma mudança de quebra em que `useReducer` não aceita o tipo de redutor completo como um parâmetro de tipo, mas em vez disso precisa de nenhum (e confiar na tipagem contextual) ou precisa dos tipos de estado e ação.
...
```
Isso também é o que você teria que fazer se mover o redutor para fora da chamada `useReducer`:
...
### Outras mudanças de quebra {/*other-breaking-changes*/}
...
- **react**: Lotes de sincronização, lanes padrão e contínuos [#25700](https://github.com/facebook/react/pull/25700)
