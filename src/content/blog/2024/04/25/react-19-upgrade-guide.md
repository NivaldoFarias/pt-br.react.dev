...
## Mudanças que causam quebra {/*breaking-changes*/}
...
Esta mudança não deve impactar a maioria dos aplicativos, mas se o seu sistema de reporte de erros em produção depende do relançamento de erros, você pode precisar atualizar o tratamento de erros. Para dar suporte a isso, adicionamos novos métodos ao `createRoot` e `hydrateRoot` para tratamento de erros personalizado:
...
No React 19, removemos as verificações de `propTypes` do pacote React, e o uso delas será silenciosamente ignorado. Se você está usando `propTypes`, recomendamos migrar para TypeScript ou outra solução de verificação de tipos.
...
Removemos `findDOMNode` porque era uma "escape hatch" legado que era lenta para executar, frágil a refatorações, retornava apenas o primeiro filho e quebrava níveis de abstração (veja mais [aqui](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Você pode substituir `ReactDOM.findDOMNode` por [refs DOM](/learn/manipulating-the-dom-with-refs):
...
O React 19 suporta [`ref` como prop](/blog/2024/04/25/react-19#ref-as-a-prop), então estamos depreciando `element.ref` em favor de `element.props.ref`.
...
Estamos depreciando `react-test-renderer` porque ele implementa seu próprio ambiente de renderização que não corresponde ao ambiente que os usuários utilizam, promove o teste de detalhes de implementação e depende da introspecção dos detalhes internos do React.

O renderizador de teste foi criado antes de existirem estratégias de teste mais viáveis como [React Testing Library](https://testing-library.com), e agora recomendamos o uso de uma biblioteca de testes moderna em vez disso.
...
Anteriormente, quando um componente suspendia, os irmãos suspensos eram renderizados e, em seguida, o fallback era confirmado.
...
No React 19, quando um componente suspende, o fallback é confirmado e, em seguida, os irmãos suspensos são renderizados.
...
### Tipos de TypeScript obsoletos removidos {/*removed-deprecated-typescript-types*/}
...
Se você tiver muitos acessos não seguros a `element.props`, poderá executar este codemod adicional:
...
_Esta mudança está incluída no preset do codemod `react-19` como [`no-implicit-ref-callback-return`](https://github.com/eps1lon/types-react-codemod/#no-implicit-ref-callback-return)._
...
_Esta mudança está incluída no preset do codemod `react-19` como [`refobject-defaults`](https://github.com/eps1lon/types-react-codemod/#refobject-defaults)._

Uma reclamação antiga sobre como o TypeScript e o React funcionam tem sido o `useRef`. Alteramos os tipos para que `useRef` agora exija um argumento. Isso simplifica significativamente sua assinatura de tipo. Agora se comportará mais como `createContext`.
...
As `props` dos elementos React agora têm o padrão `unknown` em vez de `any` se o elemento for tipado como `ReactElement`. Isso não afeta você se você passar um argumento de tipo para `ReactElement`:
...
Mas se você dependeu do padrão, agora terá que lidar com `unknown`:
...
Esta mudança está incluída no preset do codemod `react-19` como [`scoped-jsx`](https://github.com/eps1lon/types-codemod#scoped-jsx)

Uma solicitação antiga é remover o namespace global `JSX` de nossos tipos em favor de `React.JSX`. Isso ajuda a evitar a poluição de tipos globais, o que previne conflitos entre diferentes bibliotecas de UI que utilizam JSX.
...
No entanto, isso exigiu uma alteração que quebra a compatibilidade, onde `useReducer` não aceita o tipo completo do redutor como um parâmetro de tipo, mas em vez disso precisa de nenhum (e confiar na tipagem contextual) ou precisa de ambos os tipos de estado e ação.
...
- **react-is**: Remove métodos obsoletos de `react-is` [28224](https://github.com/facebook/react/pull/28224)
