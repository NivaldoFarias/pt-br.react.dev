...
author: Ricky Hanlon
date: 2024/04/25
description: Os aprimoramentos adicionados ao React 19 exigem algumas mudanças significativas, mas trabalhamos para tornar a atualização a mais tranquila possível e não esperamos que as mudanças impactem a maioria dos apps. Neste post, vamos guiar você pelas etapas de atualização de apps e bibliotecas para o React 19.
---
...
Os aprimoramentos adicionados ao React 19 exigem algumas mudanças significativas, mas trabalhamos para tornar a atualização a mais tranquila possível e não esperamos que as mudanças impactem a maioria dos apps.
...
Para ajudar a facilitar a atualização para o React 19, publicamos uma versão `react@18.3` que é idêntica à 18.2, mas adiciona avisos para APIs descontinuadas e outras mudanças que são necessárias para o React 19.
...
Para uma lista de mudanças na 18.3, consulte as [Notas da versão](https://github.com/facebook/react/blob/main/CHANGELOG.md).

</Note>

Neste post, vamos guiar você pelas etapas de atualização para o React 19:
...
Introduzimos uma [nova transformação JSX](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) em 2020 para melhorar o tamanho do bundle e usar JSX sem importar React. No React 19, estamos adicionando melhorias adicionais, como usar ref como uma prop e melhorias de velocidade JSX que exigem a nova transformação.
...
Se você estiver usando TypeScript, também precisa atualizar os tipos.
```bash
...
Para ajudar com a atualização, trabalhamos com a equipe em [codemod.com](https://codemod.com) para publicar codemods que atualizarão automaticamente seu código para muitas das novas APIs e padrões no React 19.
...
Isso não inclui as mudanças no TypeScript. Consulte [Mudanças no TypeScript](#typescript-changes) abaixo.

</Note>

As mudanças que incluem um codemod incluem o comando abaixo.

Para uma lista de todos os codemods disponíveis, consulte o repositório [`react-codemod`](https://github.com/reactjs/react-codemod).

## Breaking changes {/*breaking-changes*/}

### Erros em render não são relançados {/*errors-in-render-are-not-re-thrown*/}

Em versões anteriores do React, os erros lançados durante a renderização eram capturados e relançados. No DEV, também registraríamos no `console.error`, resultando em logs de erro duplicados.
...
- **Erros não capturados**: erros que não são capturados por uma Error Boundary são relatados para `window.reportError`.
- **Erros capturados**: erros que são capturados por uma Error Boundary são relatados para `console.error`.

Essa mudança não deve impactar a maioria dos apps, mas se seus relatórios de erro de produção dependem de erros sendo relançados, pode ser necessário atualizar seu tratamento de erros. Para suportar isso, adicionamos novos métodos a `createRoot` e `hydrateRoot` para tratamento de erros personalizados:
...
Também estamos removendo `defaultProps` de componentes de função no lugar de parâmetros padrão ES6. Componentes de classe continuarão a suportar `defaultProps` , pois não há alternativa ES6.
...
Contexto Legado estava disponível apenas em componentes de classe usando as APIs `contextTypes` e `getChildContext` e foi substituído por `contextType` devido a erros sutis que eram fáceis de perder. No React 19, estamos removendo o Contexto Legado para tornar o React um pouco menor e mais rápido.
...
##### Por favor, reconsidere renderização superficial {/*please-reconsider-shallow-rendering*/}
...
`ReactDOMTestUtils.act` está descontinuado a favor de `React.act`. Importe `act` de `react` em vez de `react-dom/test-utils`. Consulte https://react.dev/warnings/react-dom-test-utils para obter mais informações.
...
Todas as outras funções `test-utils` foram removidas. Essas utilidades eram incomuns e tornavam muito fácil depender de detalhes de implementação de baixo nível de seus componentes e React. No React 19, essas funções emitirão um erro quando chamadas e suas exportações serão removidas em uma versão futura.
...
Estamos removendo `findDOMNode` porque era uma forma de escape legada que era lenta para executar, frágil para refatoração, só retornava o primeiro filho e quebrava os níveis de abstração (veja mais [aqui](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Você pode substituir `ReactDOM.findDOMNode` por [refs do DOM](/learn/manipulating-the-dom-with-refs):
...
Acessar `element.ref` irá emitir um aviso:
...
Estamos descontinuando `react-test-renderer` porque ele implementa seu próprio ambiente de renderizador que não corresponde ao ambiente que os usuários usam, promove detalhes de implementação de teste e se baseia na introspecção dos elementos internos do React.
...
Ao renderizar duas vezes no Modo Strict em desenvolvimento, `useMemo` e `useCallback` reutilizarão os resultados memorizados da primeira renderização durante a segunda renderização. Componentes que já são compatíveis com o Modo Strict não devem notar uma diferença no comportamento.

Assim como acontece com todos os comportamentos do Modo Strict, esses recursos são projetados para detectar proativamente erros em seus componentes durante o desenvolvimento, para que você possa corrigi-los antes que sejam enviados para produção. Por exemplo, durante o desenvolvimento, o Modo Strict invocará duas vezes as funções de callback de ref na montagem inicial, para simular o que acontece quando um componente montado é substituído por um fallback do Suspense.

### Melhorias no Suspense {/*improvements-to-suspense*/}

No React 19, quando um componente suspende, o React irá confirmar imediatamente o fallback da fronteira Suspense mais próxima sem esperar que toda a árvore de irmãos seja renderizada. Após a confirmação do fallback, o React agenda outra renderização para os irmãos suspensos para "pré-aquecer" as solicitações lazy no restante da árvore:

<Diagram name="prerender" height={162} width={1270} alt="Diagrama mostrando uma árvore de três componentes, um pai rotulado como Accordion e dois filhos rotulados como Panel. Ambos os componentes Panel contêm isActive com o valor false.">
...
Essa mudança significa que os fallbacks do Suspense são exibidos mais rapidamente, enquanto ainda aquecem as solicitações lentas na árvore suspensa.
...
Esta versão inclui alterações nos elementos internos do React que podem impactar bibliotecas que ignoram nossos apelos para não usar elementos internos como `SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED`. Essas alterações são necessárias para efetuar melhorias no React 19 e não quebrarão bibliotecas que seguem nossas diretrizes.

Com base em nossa [Política de Versionamento](https://react.dev/community/versioning-policy#what-counts-as-a-breaking-change), essas atualizações não são listadas como alterações de quebra e não estamos incluindo documentação sobre como atualizá-las. A recomendação é remover qualquer código que dependa de elementos internos.

Para refletir o impacto do uso de elementos internos, renomeamos o sufixo `SECRET_INTERNALS` para:

`_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE`

No futuro, bloquearemos de forma mais agressiva o acesso a elementos internos do React para desencorajar o uso e garantir que os usuários não sejam impedidos de atualizar.

## Mudanças no TypeScript {/*typescript-changes*/}

### Tipos de TypeScript descontinuados removidos {/*removed-deprecated-typescript-types*/}

Removemos os tipos de TypeScript com base nas APIs removidas no React 19. Alguns dos tipos removidos foram movidos para pacotes mais relevantes e outros não são mais necessários para descrever o comportamento do React.

<Note>
Publicamos o [`types-react-codemod`](https://github.com/eps1lon/types-react-codemod/) para migrar a maioria das alterações de quebra relacionadas a tipos:

```bash
npx types-react-codemod@latest preset-19 ./path-to-app
```

Se você tiver muitos acessos inválidos a `element.props`, poderá executar este codemod adicional:
...
Uma longa reclamação de como TypeScript e React funcionam tem sido `useRef`. Alteramos os tipos para que `useRef` agora exija um argumento. Isso simplifica significativamente sua assinatura de tipo. Agora, ele se comportará mais como `createContext`.
...
Isso agora também significa que todos os refs são mutáveis. Você não encontrará mais o problema em que não pode mutar um ref porque o inicializou com `null`:
...
`useRef` ainda possui uma sobrecarga de conveniência para `useRef<T>(null)` que retorna automaticamente `RefObject<T | null>`. Para facilitar a migração devido ao argumento obrigatório para `useRef`, uma sobrecarga de conveniência para `useRef(undefined)` foi adicionada que retorna automaticamente `RefObject<T | undefined>`.

Confira [[RFC] Tornar todos os refs mutáveis](https://github.com/DefinitelyTyped/DefinitelyTyped/pull/64772) para discussões anteriores sobre essa alteração.

### Alterações no tipo TypeScript `ReactElement` {/*changes-to-the-reactelement-typescript-type*/}
...
As `props` dos elementos React agora são padrão para `unknown` em vez de `any` se o elemento for tipado como `ReactElement`. Isso não o afetará se você passar um argumento de tipo para `ReactElement`:
...
Mas se você dependesse do padrão, agora precisa manipular `unknown`:
...
Você só precisa dela se tiver muito código legado que dependa de acesso inválido às props do elemento. A introspecção do elemento existe apenas como uma forma de escape e você deve deixar claro que seu acesso às props é inválido por meio de um `any` explícito.

### O namespace JSX em TypeScript {/*the-jsx-namespace-in-typescript*/}
...
Um pedido de longa data é remover o namespace `JSX` global de nossos tipos em favor de `React.JSX`. Isso ajuda a evitar a poluição de tipos globais, o que impede conflitos entre diferentes bibliotecas de UI que usam JSX.
...
O especificador de módulo exato depende do tempo de execução JSX que você especificou nas `compilerOptions` de seu `tsconfig.json`:
...
No entanto, isso exigiu uma alteração de quebra em que `useReducer` não aceita o tipo de redutor completo como um parâmetro de tipo, mas, em vez disso, precisa de nenhum (e depender da tipagem contextual) ou precisa de ambos, o estado e o tipo de ação.
...
```
Se você definir o redutor embutido, incentivamos a anotar os parâmetros da função em vez disso:
```diff
- useReducer<React.Reducer<State, Action>>((state, action) => state)
+ useReducer((state: State, action: Action) => state)
```
Isso é o que você também teria que fazer se mover o redutor para fora da chamada `useReducer`:
...
### Outras alterações de quebra {/*other-breaking-changes*/}
...
- **react**: Lançar vias de sincronização, padrão e contínuas [#25700](https://github.com/facebook/react/pull/25700)
- **react**: Não renderizar os irmãos do componente suspenso [#26380](https://github.com/facebook/react/pull/26380)
- **react**: Detectar loops de atualização infinita causados ​​por atualizações da fase de renderização [#26625](https://github.com/facebook/react/pull/26625)
- **react-dom**: As transições em popstate agora são síncronas [#26025](https://github.com/facebook/react/pull/26025)
- **react-dom**: Remover o aviso de efeito de layout durante SSR [#26395](https://github.com/facebook/react/pull/26395)
