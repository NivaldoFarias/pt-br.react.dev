...
Para ajudar a facilitar a atualização para o React 19, publicamos uma versão `react@18.3` que é idêntica à 18.2, mas adiciona avisos para APIs depreciadas e outras mudanças necessárias para o React 19.
...
#### Novo Transform JSX agora é obrigatório {/*new-jsx-transform-is-now-required*/}

Introduzimos um [novo transform JSX](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) em 2020 para melhorar o tamanho do bundle e usar JSX sem importar o React. No React 19, estamos adicionando melhorias adicionais, como usar `ref` como uma prop e melhorias de velocidade do JSX que exigem o novo transform.
...
Esperamos que a maioria dos aplicativos não seja afetada, já que o transform já está habilitado na maioria dos ambientes. Para instruções manuais sobre como atualizar, consulte o [post de anúncio](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html).
...
### APIs depreciadas do React removidas {/*removed-deprecated-react-apis*/}

#### Removido: `propTypes` e `defaultProps` para funções {/*removed-proptypes-and-defaultprops-for-functions*/}
...
No React 19, removemos as verificações de `propTypes` do pacote React, e usá-los será silenciosamente ignorado. Se você usa `propTypes`, recomendamos migrar para TypeScript ou outra solução de verificação de tipos.

Também removemos `defaultProps` de componentes de função em favor de parâmetros padrão do ES6. Componentes de classe continuarão a suportar `defaultProps`, pois não há alternativa no ES6.
...
#### Removido: Contexto legado usando `contextTypes` e `getChildContext` {/*removed-legacy-context-using-contexttypes-and-getchildcontext*/}
...
Se você ainda usa Contexto Legado em componentes de classe, precisará migrar para a nova API `contextType`:
...
Se você ainda usa refs de string em componentes de classe, precisará migrar para callbacks de ref:
...
Use o codemod de refs de string para callbacks de `ref`:
...
Fábricas de padrão de módulo foram depreciadas em [agosto de 2019 (v16.9.0)](https://legacy.reactjs.org/blog/2019/08/08/react-v16.9.0.html#deprecating-module-pattern-factories).

Este padrão era raramente usado e suportá-lo torna o React um pouco maior e mais lento do que o necessário. No React 19, removemos o suporte para fábricas de padrão de módulo, e você precisará migrar para funções regulares:
...
### APIs depreciadas do React DOM removidas {/*removed-deprecated-react-dom-apis*/}
...
Todas as outras funções de `test-utils` foram removidas. Essas utilidades eram incomuns e tornavam muito fácil depender de detalhes de implementação de baixo nível dos seus componentes e do React. No React 19, essas funções gerarão um erro quando chamadas e suas exportações serão removidas em uma versão futura.
...
`ReactDOM.render` foi depreciado em [março de 2022 (v18.0.0)](https://react.dev/blog/2022/03/08/react-18-upgrade-guide). No React 19, removemos `ReactDOM.render` e você precisará migrar para usar [`ReactDOM.createRoot`](https://react.dev/reference/react-dom/client/createRoot):
...
`ReactDOM.hydrate` foi depreciado em [março de 2022 (v18.0.0)](https://react.dev/blog/2022/03/08/react-18-upgrade-guide). No React 19, removemos `ReactDOM.hydrate` e você precisará migrar para usar [`ReactDOM.hydrateRoot`](https://react.dev/reference/react-dom/client/hydrateRoot),
...
Estamos removendo `findDOMNode` porque era uma "escape hatch" legado que era lenta para executar, frágil a refatorações, retornava apenas o primeiro filho e quebrava níveis de abstração (veja mais [aqui](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Você pode substituir `ReactDOM.findDOMNode` por [refs DOM](/learn/manipulating-the-dom-with-refs):
...
O React 19 suporta [`ref` como uma prop](/blog/2024/04/25/react-19#ref-as-a-prop), então estamos depreciando `element.ref` em favor de `element.props.ref`.
...
Estamos depreciando `react-test-renderer` porque ele implementa seu próprio ambiente de renderização que não corresponde ao ambiente que os usuários utilizam, promove o teste de detalhes de implementação e depende da introspecção dos internos do React.

O renderizador de teste foi criado antes de existirem estratégias de teste mais viáveis como [React Testing Library](https://testing-library.com), e agora recomendamos o uso de uma biblioteca de teste moderna em vez disso.
...
Essa mudança significa que os fallbacks do Suspense são exibidos mais rapidamente, enquanto ainda aquecem as requisições preguiçosas na árvore suspensa.
...
### Tipos de TypeScript depreciados removidos {/*removed-deprecated-typescript-types*/}
...
Uma reclamação antiga sobre como o TypeScript e o React funcionam tem sido o `useRef`. Alteramos os tipos para que `useRef` agora exija um argumento. Isso simplifica significativamente sua assinatura de tipo. Agora ele se comportará mais como `createContext`.
...
As `props` dos elementos React agora têm `unknown` como padrão em vez de `any` se o elemento for tipado como `ReactElement`. Isso não afeta você se passar um argumento de tipo para `ReactElement`:
...
Você só precisará disso se tiver muito código legado dependendo de acesso não seguro às props do elemento. A introspecção de elementos existe apenas como uma saída de emergência, e você deve tornar explícito que seu acesso às props não é seguro por meio de um `any` explícito.
...
No entanto, isso exigiu uma alteração que quebra a compatibilidade, onde `useReducer` não aceita o tipo completo do redutor como parâmetro de tipo, mas em vez disso precisa de nenhum (e confiar na tipagem contextual) ou precisa de ambos os tipos de estado e ação.

A nova prática recomendada é _não_ passar argumentos de tipo para `useReducer`.
```diff
...
- **react-is**: Remove métodos depreciados de `react-is` [28224](https://github.com/facebook/react/pull/28224)

### Outras mudanças notáveis {/*other-notable-changes*/}

- **react**: Batch sync, default e continuous lanes [#25700](https://github.com/facebook/react/pull/25700)
...
Obrigado a [Andrew Clark](https://twitter.com/acdlite), [Eli White](https://twitter.com/Eli_White), [Jack Pope](https://github.com/jackpope), [Jan Kassens](https://github.com/kassens), [Josh Story](https://twitter.com/joshcstory), [Matt Carroll](https://twitter.com/mattcarrollcode), [Noah Lemen](https://twitter.com/noahlemen), [Sophie Alpert](https://twitter.com/sophiebits) e [Sebastian Silbermann](https://twitter.com/sebsilbermann) por revisar e editar este post.