...
author: Ricky Hanlon
date: 2024/04/25
description: As melhorias adicionadas ao React 19 exigem algumas mudanças interruptivas, mas trabalhamos para tornar a atualização o mais fácil possível e não esperamos que as mudanças impactem a maioria dos aplicativos. Neste post, guiaremos você pelas etapas para atualizar aplicativos e bibliotecas para React 19.
---
...
As melhorias adicionadas ao React 19 exigem algumas mudanças interruptivas, mas trabalhamos para tornar a atualização o mais fácil possível, e não esperamos que as mudanças impactem a maioria dos aplicativos.
...
Para tornar a atualização para o React 19 mais fácil, publicamos uma versão `react@18.3` que é idêntica à 18.2, mas adiciona avisos para APIs obsoletas e outras alterações que são necessárias para o React 19.
...
Neste post, iremos guiá-lo pelas etapas para atualizar para o React 19:

- [Instalando](#installing)
- [Codemods](#codemods)
- [Mudanças interruptivas](#breaking-changes)
...
Introduzimos uma [nova transformação JSX](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html) em 2020 para melhorar o tamanho do pacote e usar JSX sem importar React. No React 19, estamos adicionando melhorias adicionais, como usar ref como uma prop e melhorias de velocidade do JSX que exigem a nova transformação.
...
Seu aplicativo (ou uma de suas dependências) está usando uma transformação JSX desatualizada. Atualize para a transformação JSX moderna para obter um desempenho mais rápido: https://react.dev/link/new-jsx-transform
...
Esperamos que a maioria dos aplicativos não seja afetada, pois a transformação já está habilitada na maioria dos ambientes. Para obter instruções manuais sobre como atualizar, consulte a [postagem de anúncio](https://legacy.reactjs.org/blog/2020/09/22/introducing-the-new-jsx-transform.html).
...
### Erros no renderizar não são lançados novamente {/*errors-in-render-are-not-re-thrown*/}

Em versões anteriores do React, os erros lançados durante o renderizar eram capturados e lançados novamente. Em DEV, também faríamos o log em `console.error`, resultando em logs de erros duplicados.
...
- **Erros não capturados**: Erros que não são capturados por uma Error Boundary são reportados para `window.reportError`.
- **Erros capturados**: Erros que são capturados por uma Error Boundary são reportados para `console.error`.

Essa mudança não deve impactar a maioria dos aplicativos, mas se o relatório de erros de produção depender de erros sendo lançados novamente, você pode precisar atualizar o tratamento de erros. Para suportar isso, adicionamos novos métodos a `createRoot` e `hydrateRoot` para tratamento personalizado de erros:
...
Também estamos removendo `defaultProps` de componentes de função em vez de parâmetros padrão ES6. Componentes de classe continuarão a oferecer suporte a `defaultProps`, pois não há alternativa ES6.
...
#### Removido: Contexto herdado usando `contextTypes` e `getChildContext` {/*removed-removing-legacy-context*/}

O Contexto Herdado foi descontinuado em [outubro de 2018 (v16.6.0)](https://legacy.reactjs.org/blog/2018/10/23/react-v-16-6.html).

O Contexto Herdado só estava disponível em componentes de classe usando as APIs `contextTypes` e `getChildContext`, e foi substituído por `contextType` devido a erros sutis que eram fáceis de perder. No React 19, estamos removendo o Contexto Herdado para tornar o React um pouco menor e mais rápido.

Se você ainda estiver usando o Contexto Herdado em componentes de classe, precisará migrar para a nova API `contextType`:
...
Componentes de classe suportavam refs de string antes de serem substituídos por callbacks ref devido a [múltiplas desvantagens](https://github.com/facebook/react/issues/1373). No React 19, estamos removendo refs de string para tornar o React mais simples e fácil de entender.

Se você ainda estiver usando refs de string em componentes de classe, precisará migrar para callbacks ref:
...
Este padrão raramente era usado e o suporte a ele faz com que o React seja ligeiramente maior e mais lento do que o necessário. No React 19, estamos removendo o suporte para fábricas de padrão de módulo, e você precisará migrar para funções regulares:
...
Usar `createFactory` era comum antes do suporte generalizado para JSX, mas é raramente usado hoje em dia e pode ser substituído por JSX. No React 19, estamos removendo `createFactory` e você precisará migrar para JSX:
...
A renderização superficial depende de detalhes internos do React e pode impedi-lo de atualizações futuras. Recomendamos a migração de seus testes para [@testing-library/react](https://testing-library.com/docs/react-testing-library/intro/) ou [@testing-library/react-native](https://testing-library.com/docs/react-native-testing-library/intro/).
...
`ReactDOMTestUtils.act` está descontinuado em favor de `React.act`. Importe `act` de `react` em vez de `react-dom/test-utils`. Consulte https://react.dev/warnings/react-dom-test-utils para obter mais informações.
...
Estamos removendo `findDOMNode` porque era um escape hatch legado que era lento para executar, frágil para refatoração, só retornava o primeiro filho e quebrava os níveis de abstração (veja mais [aqui](https://legacy.reactjs.org/docs/strict-mode.html#warning-about-deprecated-finddomnode-usage)). Você pode substituir `ReactDOM.findDOMNode` por [refs DOM](/learn/manipulating-the-dom-with-refs):
...
Estamos depreciando `react-test-renderer` porque ele implementa seu próprio ambiente de renderizador que não corresponde ao ambiente que os usuários usam, promove detalhes de implementação de teste e depende da introspecção dos detalhes internos do React.
...
Como acontece com todos os comportamentos do Strict Mode, esses recursos são projetados para expor proativamente erros em seus componentes durante o desenvolvimento, para que você possa corrigi-los antes de serem enviados para produção. Por exemplo, durante o desenvolvimento, o Strict Mode irá invocar duas vezes funções de callback ref na montagem inicial, para simular o que acontece quando um componente montado é substituído por um fallback do Suspense.

### Melhorias no Suspense {/*improvements-to-suspense*/}

No React 19, quando um componente suspende, o React irá imediatamente comitar o fallback do limite Suspense mais próximo sem aguardar a renderização de toda a árvore de irmãos. Após o commit do fallback, o React agenda outra renderização para os irmãos suspensos para "pré-aquecer" as solicitações lazy no restante da árvore:
...
Anteriormente, quando um componente suspendia, os irmãos suspensos eram renderizados e então o fallback era commitado.
...
No React 19, quando um componente suspende, o fallback é comitad e então os irmãos suspensos são renderizados.

</Diagram>

Essa mudança significa que os fallbacks do Suspense são exibidos mais rápido, enquanto ainda aquecem as solicitações lazy na árvore suspensa.

### Builds UMD removidos {/*umd-builds-removed*/}

UMD era amplamente usado no passado como uma maneira conveniente de carregar o React sem uma etapa de build. Agora, existem alternativas modernas para carregar módulos como scripts em documentos HTML. Começando com o React 19, o React não irá mais produzir builds UMD para reduzir a complexidade de seu processo de teste e lançamento.
...
### Bibliotecas que dependem de detalhes internos do React podem bloquear atualizações {/*libraries-depending-on-react-internals-may-block-upgrades*/}

Esta versão inclui alterações nos detalhes internos do React que podem impactar bibliotecas que ignoram nossos apelos para não usar detalhes internos como `SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED`. Essas alterações são necessárias para implementar melhorias no React 19 e não quebrarão as bibliotecas que seguem nossas diretrizes.

Com base em nossa [Política de Versionamento](https://react.dev/community/versioning-policy#what-counts-as-a-breaking-change), essas atualizações não são listadas como alterações disruptivas e não estamos incluindo documentos sobre como atualizá-las. A recomendação é remover qualquer código que dependa de detalhes internos.

Para refletir o impacto do uso de detalhes internos, renomeamos o sufixo `SECRET_INTERNALS` para:

`_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE`

No futuro, bloquearemos de forma mais agressiva o acesso aos detalhes internos do React para desencorajar o uso e garantir que os usuários não sejam impedidos de atualizar.

## Mudanças no TypeScript {/*typescript-changes*/}

### Tipos TypeScript depreciados removidos {/*removed-deprecated-typescript-types*/}

Removemos os tipos TypeScript com base nas APIs removidas no React 19. Alguns dos removidos tiveram seus tipos movidos para pacotes mais relevantes e outros não são mais necessários para descrever o comportamento do React.

<Note>
Publicamos [`types-react-codemod`](https://github.com/eps1lon/types-react-codemod/) para migrar a maioria das alterações disruptivas relacionadas a tipos:

```bash
npx types-react-codemod@latest preset-19 ./caminho-para-o-app
```

Se você tiver muitos acessos incorretos a `element.props`, pode executar este codemod adicional:
...
Uma reclamação de longa data de como o TypeScript e o React funcionam tem sido `useRef`. Alteramos os tipos para que `useRef` agora requeira um argumento. Isso simplifica significativamente sua assinatura de tipo. Agora se comportará mais como `createContext`.

```ts
// @ts-expect-error: Esperado 1 argumento, mas viu nenhum
useRef();
// Passa
useRef(undefined);
// @ts-expect-error: Esperado 1 argumento, mas viu nenhum
...
Isso agora também significa que todas as refs são mutáveis. Você não enfrentará mais o problema de não poder mutar uma ref porque a inicializou com `null`:
...
`MutableRef` agora está deprecated em favor de um único tipo `RefObject` que `useRef` sempre retornará:
...
Confira [[RFC] Make all refs mutable](https://github.com/DefinitelyTyped/DefinitelyTyped/pull/64772) para discussões anteriores sobre essa alteração.

### Mudanças no tipo TypeScript `ReactElement` {/*changes-to-the-reactelement-typescript-type*/}

_Essa alteração está incluída no codemod [`react-element-default-any-props`](https://github.com/eps1lon/types-react-codemod#react-element-default-any-props)._
...
Você só deve precisar disso se tiver muito código legado confiando em acesso sem som de props de elementos. A introspecção de elementos só existe como um escape hatch, e você deve deixar explícito que seu acesso de props é não-sã via um `any` explícito.
...
Um pedido de longa data é remover o namespace `JSX` global de nossos tipos em favor de `React.JSX`. Isso ajuda a evitar a poluição de tipos globais, o que impede conflitos entre diferentes bibliotecas de UI que alavancam JSX.
...
O especificador de módulo exato depende do runtime JSX especificado nas `compilerOptions` do seu `tsconfig.json`:
...
No entanto, isso exigiu uma alteração disruptiva onde `useReducer` não aceita o tipo completo do redutor como um parâmetro de tipo, mas precisa de nenhum (e confiar na digitação contextual) ou precisa dos tipos state e action.
...
```
Isso também é o que você também teria que fazer se você mover o redutor para fora da chamada `useReducer`:
...
- **react**: Lotes síncronos, vias padrão e contínuas [#25700](https://github.com/facebook/react/pull/25700)
...
Para obter uma lista completa de alterações, consulte o [Changelog](https://github.com/facebook/react/blob/main/CHANGELOG.md#1900-december-5-2024).
