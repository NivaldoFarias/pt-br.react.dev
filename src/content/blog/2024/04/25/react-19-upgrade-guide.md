...
- [Novas preterições](#new-deprecations)
...
As mudanças que incluem um codemod são acompanhadas pelo comando abaixo.
...
Essa mudança não deve afetar a maioria dos aplicativos, mas se o seu relatório de erros de produção depender do relançamento de erros, você pode precisar atualizar seu tratamento de erros. Para dar suporte a isso, adicionamos novos métodos a `createRoot` e `hydrateRoot` para tratamento de erros personalizado:
...
Codemod refs de string para callbacks de `ref`:
...
Todas as outras funções de `test-utils` foram removidas. Essas utilidades eram incomuns e facilitavam muito a dependência de detalhes de implementação de baixo nível de seus componentes e do React. No React 19, essas