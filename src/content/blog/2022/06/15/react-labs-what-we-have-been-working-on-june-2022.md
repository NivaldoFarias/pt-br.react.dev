...
Atualmente, assets como scripts, estilos externos, fontes e imagens são tipicamente pré-carregados e carregados usando sistemas externos. Isso pode tornar complicado a coordenação entre novos ambientes como streaming, Server Components e mais.
Estamos analisando a adição de APIs para pré-carregar e carregar assets externos deduplicados através de APIs React que funcionam em todos os ambientes React.
...
Offscreen introduz uma terceira opção: ocultar a UI visualmente, mas diminuir a prioridade de seu conteúdo. A ideia é semelhante em espírito à propriedade CSS `content-visibility`: quando o conteúdo está oculto, ele não precisa permanecer em sincronia com o resto da UI. O React pode adiar o trabalho de renderização até que o resto do aplicativo esteja ocioso, ou até que o conteúdo se torne visível novamente.
