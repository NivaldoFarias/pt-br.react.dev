---
title: "React Labs: O Que Temos Feito – Junho de 2022"
...
Normalmente, temos vários projetos em andamento a qualquer momento, variando do mais experimental ao claramente definido. Olhando para o futuro, gostaríamos de começar a compartilhar regularmente mais sobre o que temos feito com a comunidade em todos esses projetos.
...
Anunciamos uma [demonstração experimental dos React Server Components](https://legacy.reactjs.org/blog/2020/12/21/data-fetching-with-react-server-components.html) (RSC) em dezembro de 2020. Desde então, temos finalizado suas dependências no React 18 e trabalhado em mudanças inspiradas em feedback experimental.
...
Atualmente, ativos como scripts, estilos externos, fontes e imagens são tipicamente pré-carregados e carregados usando sistemas externos. Isso pode tornar complicado a coordenação em novos ambientes como streaming, Server Components e mais.
Estamos analisando a adição de APIs para pré-carregar e carregar ativos externos deduplicados através de APIs do React que funcionam em todos os ambientes React.

Também estamos analisando a possibilidade de esses ativos suportarem Suspense, para que você possa ter imagens, CSS e fontes que bloqueiam a exibição até serem carregadas, mas não bloqueiam o streaming e a renderização concorrente. Isso pode ajudar a evitar ["pipoca"](https://twitter.com/sebmarkbage/status/1516852731251724293) (popcorning) à medida que os visuais aparecem e o layout muda.

## Static Server Rendering Optimizations {/*static-server-rendering-optimizations*/}

Static Site Generation (SSG) e Incremental Static Regeneration (ISR) são ótimas maneiras de obter desempenho para páginas cacheadas, mas achamos que podemos adicionar recursos para melhorar o desempenho do Server Side Rendering (SSR) dinâmico – especialmente quando a maioria, mas não todo, o conteúdo é cacheável. Estamos explorando maneiras de otimizar a renderização do servidor utilizando compilação e passes estáticos.

## React Optimizing Compiler {/*react-compiler*/}

Apresentamos uma [prévia inicial](https://www.youtube.com/watch?v=lGEMwh32soc) do React Forget na React Conf 2021. É um compilador que gera automaticamente chamadas equivalentes a `useMemo` e `useCallback` para minimizar o custo de re-renderização, mantendo o modelo de programação do React.
...
Também estamos trabalhando em um playground para explorar muitos aspectos do compilador. Embora o objetivo do playground seja facilitar o desenvolvimento do compilador, acreditamos que ele facilitará a experimentação e a construção de intuição sobre o que o compilador faz. Ele revela vários insights sobre como ele funciona internamente e renderiza ao vivo as saídas do compilador enquanto você digita. Isso será lançado junto com o compilador quando ele for liberado.
...
A outra opção é manter o componente montado e alternar a aparência visualmente usando CSS. Isso preserva o estado da sua UI, mas tem um custo de desempenho, porque o React deve continuar renderizando o componente oculto e todos os seus filhos sempre que ele recebe novas atualizações.

Offscreen introduz uma terceira opção: ocultar a UI visualmente, mas reduzir a prioridade de seu conteúdo. A ideia é semelhante em espírito à propriedade CSS `content-visibility`: quando o conteúdo está oculto, ele não precisa permanecer em sincronia com o resto da UI. O React pode adiar o trabalho de renderização até que o resto do aplicativo esteja ocioso, ou até que o conteúdo se torne visível novamente.
...
*   **Renderização de lista virtualizada.** Ao exibir grandes listas de itens, frameworks de lista virtualizada pré-renderizarão mais linhas do que as atualmente visíveis. Você pode usar Offscreen para pré-renderizar as linhas ocultas com menor prioridade do que os itens visíveis na lista.
*   **Conteúdo em segundo plano.** Também estamos explorando um recurso relacionado para reduzir a prioridade do conteúdo em segundo plano sem ocultá-lo, como ao exibir uma sobreposição modal.
