...
* `value`: O `value` que você quer verificar. Ele pode ser qualquer valor de qualquer tipo.
...
*   **Somente [tags JSX](/learn/writing-markup-with-jsx) e objetos retornados por [`createElement`](/reference/react/createElement) são considerados Elementos React.** Por exemplo, mesmo que um número como `42` seja um *nó* React válido (e pode ser retornado de um componente), ele não é um Elemento React válido. Arrays e portais criados com [`createPortal`](/reference/react-dom/createPortal) também *não* são considerados Elementos React.
...
É muito incomum precisar de `isValidElement`. É principalmente útil se você estiver chamando outra API que *somente* aceita elementos (como [`cloneElement`](/reference/react/cloneElement) faz) e você quer evitar um erro quando seu argumento não é um Elemento React.
...
**Nota: `isValidElement` verifica se o argumento é um *Elemento React,* não se é um nó React.** Por exemplo, `42` não é um Elemento React válido. No entanto, é um nó React perfeitamente válido:
