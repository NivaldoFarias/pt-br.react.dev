...
*   Valores produzidos chamando [`createElement`](/reference/react/createElement)
...
É muito incomum precisar de `isValidElement`. É útil principalmente se você estiver chamando outra API que *apenas* aceita elementos (como [`cloneElement`](/reference/react/cloneElement) faz) e deseja evitar um erro quando seu argumento não é um Elemento React.

A menos que você tenha alguma razão muito específica para adicionar uma verificação `isValidElement`, provavelmente você não precisa dela.
...
**Observe que `isValidElement` verifica se o argumento é um *Elemento React,* não se é um nó React.** Por exemplo, `42` não é um Elemento React válido. No entanto, é um nó React perfeitamente válido:

```js
function MyComponent() {
  return 42; // É ok retornar um número de um componente
}
```
