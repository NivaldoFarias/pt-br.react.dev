...
`startTransition` permite que você renderize uma parte da UI em segundo plano.
...
A função `startTransition` permite que você marque uma atualização de state como uma Transition.
...
* Você pode encapsular uma atualização em uma Transition apenas se tiver acesso à função `set` desse state. Se você quiser iniciar uma Transition em resposta a alguma prop ou um valor de retorno de um Hook personalizado, tente [`useDeferredValue`](/reference/react/useDeferredValue) em vez disso.
...
* Uma atualização de state marcada como uma Transition será interrompida por outras atualizações de state. Por exemplo, se você atualizar um componente de gráfico dentro de uma Transition, mas então começar a digitar em uma entrada enquanto o gráfico está no meio de uma nova renderização, React reiniciará o trabalho de renderização no componente de gráfico após tratar a atualização de state da entrada.
...
Transitions permitem manter as atualizações da user interface responsivas mesmo em dispositivos lentos.

Com uma Transition, sua UI permanece responsiva no meio de uma nova renderização. Por exemplo, se o usuário clicar em uma aba, mas depois mudar de ideia e clicar em outra aba, ele pode fazer isso sem esperar a primeira nova renderização terminar.

<Note>

`startTransition` é muito semelhante a [`useTransition`](/reference/react/useTransition), exceto que não fornece a flag `isPending` para rastrear se uma Transition está em andamento. Você pode chamar `startTransition` quando `useTransition` não estiver disponível. Por exemplo, `startTransition` funciona fora de componentes, como de uma biblioteca de dados.
