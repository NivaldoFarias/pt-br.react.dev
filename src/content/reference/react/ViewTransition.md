...
Envolva elementos com `<ViewTransition>` para animá-los quando eles são atualizados dentro de uma [Transição](/reference/react/useTransition). O React usa as seguintes heurísticas para determinar se uma Transição de Visualização é ativada para uma animação:
...
- `update`: Se um `ViewTransition` tiver quaisquer mutações DOM dentro dele que o React esteja realizando (como uma prop mudando) ou se o próprio limite do `ViewTransition` mudar de tamanho ou posição devido a um irmão imediato. Se houver `ViewTransition` aninhados, a mutação se aplica a eles e não ao pai.
...
Se um `flushSync` ocorrer no meio dessa sequência, o React pulará a Transição, pois depende de ser capaz de concluir de forma síncrona.
...
Em muitos exemplos iniciais de View Transitions na web, você verá o uso de [`view-transition-name`](https://developer.mozilla.org/en-US/docs/Web/CSS/view-transition-name) e depois estilizando-o usando seletores `::view-transition-...(meu-nome)`. Não recomendamos isso para estilização. Em vez disso, normalmente recomendamos o uso de uma Classe de Transição de Visualização.
...
Quando `show` volta para `false`, uma animação de `exit` é acionada.
