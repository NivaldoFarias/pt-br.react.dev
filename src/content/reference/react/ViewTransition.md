...
- `update`: Se um `ViewTransition` tem quaisquer mutações DOM dentro dele que o React está realizando (como uma mudança de prop) ou se o próprio limite do `ViewTransition` muda de tamanho ou posição devido a um irmão imediato. Se houver `ViewTransition` aninhados, a mutação se aplica a eles e não ao pai.
...
- Esperar que as fontes sejam carregadas.
...
Após a resolução da Promise `ready` do `startViewTransition`, o React reverterá o `view-transition-name`. Em seguida, o React invocará os callbacks `onEnter`, `onExit`, `onUpdate` e `onShare` para permitir o controle programático manual sobre as Animações. Isso ocorrerá após os padrões integrados já terem sido computados.
...
Após a resolução da Promise `finished` do `startViewTransition`, o React invocará `useEffect`. Isso evita que eles interfiram no desempenho da Animação. No entanto, isso não é uma garantia, pois se outra `setState` ocorrer enquanto a Animação estiver em execução, ela ainda terá que invocar o `useEffect` mais cedo para preservar as garantias sequenciais.
...
Estes callbacks permitem ajustar a animação imperativamente usando as APIs [animate](https://developer.mozilla.org/en-US/docs/Web/API/Element/animate):
...
- `<ViewTransition>` cria uma imagem que pode ser movida, dimensionada e com fade cruzado. Diferente das Animações de Layout que você pode ter visto no React Native ou Motion, isso significa que nem todo elemento individual dentro dele anima sua posição. Isso pode levar a um melhor desempenho e uma sensação mais contínua, animação suave em comparação com a animação de cada peça individual. No entanto, também pode perder continuidade em coisas que deveriam estar se movendo por conta própria. Portanto, você pode ter que adicionar mais `<ViewTransition>` limites manualmente como resultado.
- Muitos usuários podem preferir não ter animações na página. O React não desabilita automaticamente as animações para este caso. Recomendamos o uso da consulta de mídia `@media (prefers-reduced-motion)` para desabilitar animações ou atenuá-las com base na preferência do usuário. No futuro, bibliotecas CSS podem ter isso integrado em seus presets.
...
Quando `setShow` é chamado, `show` muda para `true` e o componente `Child` é renderizado. Quando `setShow` é chamado dentro de `startTransition`, e `Child` renderiza um `ViewTransition` antes de qualquer outro nó DOM, uma animação de `entrada` é acionada.

Quando `show` volta para `false`, uma animação de `saída` é acionada.
...
Ao contrário de uma animação de saída/entrada, isso pode estar profundamente dentro da árvore excluída/montada. Se um `<ViewTransition>` também fosse elegível para saída/entrada, a animação "compartilhada" teria precedência.

Se a Transição primeiro desmontar um lado e depois levar a um fallback `<Suspense>` a ser exibido antes que eventualmente o novo nome seja montado, nenhuma transição de elemento compartilhado acontece.
...
Se o lado montado ou desmontado de um par estiver fora da viewport, nenhum par é formado. Isso garante que ele não voe para dentro ou para fora da viewport quando algo é rolado. Em vez disso, ele é tratado como uma entrada/saída regular por si só.
...
```
Em vez disso, qualquer `<ViewTransition>` pai faria um cross-fade. Se não houver um `<ViewTransition>` pai, então não haverá animação nesse caso.
...
Isso significa que você pode querer evitar elementos wrapper em listas onde deseja permitir que o Componente controle sua própria animação de reordenação:
...
É importante usar corretamente as chaves para preservar a identidade ao reordenar listas. Pode parecer que você poderia usar "nome", transições de elementos compartilhados, para animar reordenações, mas isso não seria acionado se um lado estivesse fora da viewport. Para animar uma reordenação, você geralmente quer mostrar que ela foi para uma posição fora da viewport.

</Pitfall>

---

### Animação a partir do conteúdo do Suspense {/*animating-from-suspense-content*/}

Assim como qualquer Transição, o React espera pelos dados e por novo CSS (`<link rel="stylesheet" precedence="...">`) antes de executar a animação. Além disso, as ViewTransitions também esperam até 500ms para que novas fontes sejam carregadas antes de iniciar a animação, para evitar que elas apareçam piscando posteriormente. Pelo mesmo motivo, uma imagem envolvida por uma ViewTransition esperará que a imagem seja carregada.

Se estiver dentro de uma nova instância de boundary do Suspense, o fallback será exibido primeiro. Após o boundary do Suspense carregar completamente, ele aciona a `<ViewTransition>` para animar a revelação do conteúdo.

Atualmente, isso ocorre apenas para Transições do lado do cliente. No futuro, isso também animará o boundary do Suspense para SSR em streaming quando o conteúdo do servidor suspender durante o carregamento inicial.

Existem duas maneiras de animar boundaries do Suspense, dependendo de onde você coloca a `<ViewTransition>`:
...
```
Neste cenário, quando o conteúdo muda de A para B, ele será tratado como uma "atualização" e aplicará essa classe, se apropriado. Tanto A quanto B receberão o mesmo `view-transition-name` e, portanto, atuarão como um cross-fade por padrão.
...
Você pode obter efeitos diferentes dependendo de onde escolher colocar o boundary `<ViewTransition>`.
...
Você pode usar a classe "none" para optar por não participar de uma animação. Ao envolver seus filhos em um "none", você pode desabilitar animações para eles enquanto o pai ainda aciona.
...
Isso animará apenas se o tema mudar e não se apenas os filhos forem atualizados. Os filhos ainda podem optar por participar novamente com sua própria `<ViewTransition>`, mas pelo menos isso será manual novamente.

---

### Optando por não participar de uma animação {/*opting-out-of-an-animation*/}

Às vezes, você está encapsulando um componente grande existente, como uma página inteira, e deseja animar algumas atualizações, como a mudança de tema. No entanto, você não quer que todas as atualizações dentro da página inteira participem de um cross-fade quando estiverem sendo atualizadas. Especialmente se você estiver adicionando mais animações incrementalmente.
...
No futuro, roteadores e outras bibliotecas poderão adicionar suporte para tipos e estilos padrão de transição de visualização.
...
O React aguarda a finalização de qualquer Navegação pendente para garantir que a restauração da rolagem ocorra dentro da animação. Se a Navegação estiver bloqueada no React, seu roteador deve ser desbloqueado em `useLayoutEffect`, pois `useEffect` levaria a um deadlock.
...
A duplicata existente `<ViewTransition name=%s>` tem este rastreamento de pilha.
{'    '}at Item
{'    '}at ItemList
