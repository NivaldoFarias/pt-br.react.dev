...
`createRef` cria um objeto de [ref](/learn/referencing-values-with-refs) que pode conter um valor arbitrário.
...
* `current`: Inicialmente, é definido como `null`. Você pode mais tarde definir isso como outra coisa. Se você passar o objeto ref para o React como um atributo `ref` para um nó JSX, o React definirá sua propriedade `current`.
...
Se você agora passar `ref={this.inputRef}` para um `<input>` no seu JSX, o React irá preencher `this.inputRef.current` com o nó DOM de entrada. Por exemplo, aqui está como você faz um botão que foca na entrada:
...
Recomendamos usar componentes de função em vez de [componentes de classe](/reference/react/Component) em código novo. Se você tem alguns componentes de classe existentes usando `createRef`, veja como você pode convertê-los. Este é o código original:
