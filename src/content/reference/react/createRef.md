...
`createRef` é usado principalmente para [componentes de classe.](/reference/react/Component) Componentes de função normalmente usam [`useRef`](/reference/react/useRef) em vez disso.
...
* `current`: Inicialmente, é definido como `null`. Você pode configurá-lo mais tarde para outra coisa. Se você passar o objeto ref para o React como um atributo `ref` para um nó JSX, o React irá definir sua propriedade `current`.
...
Para declarar um ref dentro de um [componente de classe,](/reference/react/Component) chame `createRef` e atribua seu resultado para um campo da classe:
...
Se você agora passar `ref={this.inputRef}` para um `<input>` em seu JSX, o React populará `this.inputRef.current` com o nó DOM de entrada. Por exemplo, aqui está como você faz um botão que foca na entrada:
...
Recomendamos o uso de componentes de função em vez de [componentes de classe](/reference/react/Component) em código novo. Se você tem alguns componentes de classe existentes que usam `createRef`, aqui está como você pode convertê-los. Este é o código original:
