...
Nós recomendamos definir componentes como funções em vez de classes. [Veja como migrar.](#alternatives)

</Pitfall>

<Intro>

`PureComponent` é similar a [`Component`](/reference/react/Component), mas pula as re-renderizações para as mesmas props e state. Componentes de classe ainda são suportados pelo React, mas não recomendamos usá-los em código novo.
...
Um componente React sempre deve ter [lógica de renderização pura.](/learn/keeping-components-pure) Isso significa que ele deve retornar a mesma saída se suas props, state e contexto não mudaram. Ao usar `PureComponent`, você está dizendo ao React que seu componente está em conformidade com este requisito, então o React não precisa re-renderizar, desde que suas props e state não tenham mudado. No entanto, seu componente ainda re-renderizará se um contexto que ele está usando mudar.
...
Nós recomendamos usar componentes de função em vez de [componentes de classe](/reference/react/Component) em código novo. Se você tem alguns componentes de classe existentes usando `PureComponent`, aqui está como você pode convertê-los. Este é o código original:
