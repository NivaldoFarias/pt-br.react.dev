...
`createElement` permite que você crie um Elemento React. Ele serve como uma alternativa para a escrita de [JSX.](/learn/writing-markup-with-jsx)
...
* `props`: As `props` que você passou, exceto `ref` e `key`.
* `ref`: A `ref` que você passou. Se ausente, `null`.
* `key`: A `key` que você passou, convertida em uma string. Se ausente, `null`.

Normalmente, você retornará o elemento do seu componente ou o tornará um filho de outro elemento. Embora você possa ler as propriedades do elemento, é melhor tratar cada elemento como opaco após sua criação e apenas renderizá-lo.

#### Ressalvas {/*caveats*/}

* Você deve **tratar Elementos React e suas props como [imutáveis](https://pt.wikipedia.org/wiki/Objeto_imutável)** e nunca mudar seu conteúdo após a criação. Em desenvolvimento, React irá [congelar](https://developer.mozilla.org/pt-BR/docs/Web/JavaScript/Reference/Global_Objects/Object/freeze) o elemento retornado e sua propriedade `props` superficialmente para impor isso.

* Quando você usa JSX, **você deve começar uma tag com uma letra maiúscula para renderizar seu próprio componente personalizado.** Em outras palavras, `<Something />` é equivalente a `createElement(Something)`, mas `<something />` (minúsculo) é equivalente a `createElement('something')` (observe que é uma string, então será tratado como uma tag HTML embutida).

* Você só deve **passar filhos como múltiplos argumentos para `createElement` se eles forem todos estaticamente conhecidos,** como `createElement('h1', {}, child1, child2, child3)`. Se seus filhos forem dinâmicos, passe o array inteiro como o terceiro argumento: `createElement('ul', {}, listItems)`. Isso garante que o React [avise sobre `key`s faltando](/learn/rendering-lists#keeping-list-items-in-order-with-key) para quaisquer listas dinâmicas. Para listas estáticas, isso não é necessário porque elas nunca reordenam.
...
Ambos os estilos de codificação são bons, então você pode usar o que preferir para o seu projeto. O principal benefício de usar JSX em comparação com `createElement` é que é fácil ver qual tag de fechamento corresponde a qual tag de abertura.
...
**Observe que criar este objeto não renderiza o componente `Greeting` nem cria nenhum elemento DOM.**

Um Elemento React é mais como uma descrição--uma instrução para o React renderizar mais tarde o componente `Greeting`. Ao retornar este objeto do seu componente `App`, você diz ao React o que fazer em seguida.
