...
Neste exemplo, a aba de Contato tem um `<textarea>` onde o usuário pode digitar uma mensagem. Se você digitar algum texto, mudar para a aba Home e depois voltar para a aba Contato, a mensagem rascunhada será perdida:
...
Se mudarmos para usar um limite de `Activity` para mostrar e ocultar a aba ativa, podemos preservar o estado do DOM de cada aba. Tente digitar texto e mudar de aba novamente, e você verá que a mensagem rascunhada não é mais redefinida:
...
A pré-renderização de componentes com limites ocultos do Activity é uma maneira poderosa de reduzir os tempos de carregamento para partes da UI com as quais o usuário provavelmente interagirá em seguida.
...
Os limites do Suspense participam da Hidratação Seletiva, pois eles naturalmente dividem a árvore de componentes em unidades que são independentes umas das outras:
...
Como exemplo, considere uma tag `<video>`. Normalmente, ela não requer nenhuma limpeza, porque mesmo que você esteja reproduzindo um vídeo, desmontar a tag impede que o vídeo e o áudio sejam reproduzidos no navegador. Tente reproduzir o vídeo e depois pressione Home nesta demonstração:
...
Normalmente, no entanto, a maioria dos seus componentes React já deve ser robusta para ser ocultada por um limite de `Activity`. E conceitualmente, você deve pensar em `Activity` "ocultas" como sendo desmontadas.
...
Se você depende de um `Effect` ser montado para limpar os efeitos colaterais de um componente, refatore o `Effect` para fazer o trabalho na função de limpeza retornada em vez disso.
