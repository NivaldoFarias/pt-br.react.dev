...
**A API `<Activity />` está disponível apenas nos canais Canary e Experimental do React no momento.**
...
Mas desmontar um componente destrói seu estado interno, o que nem sempre é o que você quer.
...
Se mudarmos para usar um limite de `Activity` para mostrar e ocultar a aba ativa, podemos preservar o estado do DOM de cada aba. Tente digitar texto e mudar de aba novamente, e você verá que o rascunho da mensagem não é mais redefinido:
...
Até agora, vimos como o Activity pode ocultar algum conteúdo com o qual o usuário interagiu, sem descartar o estado efêmero desse conteúdo.
...
Quando um limite do Activity está <CodeStep step={1}>oculto</CodeStep> durante sua renderização inicial, seus filhos não serão visíveis na página — mas eles _ainda serão renderizados_, embora com uma prioridade menor do que o conteúdo visível e sem montar seus Effects.

Esse _pré-renderização_ permite que os filhos carreguem qualquer código ou dados de que precisam com antecedência, para que, mais tarde, quando o limite do Activity se tornar visível, os filhos possam aparecer mais rapidamente com tempos de carregamento reduzidos.
...
Para descobrir proativamente outros Efeitos que não possuem limpeza adequada, o que é importante não apenas para os limites de `Activity`, mas para muitos outros comportamentos em React, recomendamos o uso de [`<StrictMode>`](/reference/react/StrictMode).
