...
* `reducer`: A função reducer que especifica como o estado é atualizado. Ela deve ser pura, receber o estado e a ação como argumentos, e retornar o próximo estado. Estado e ação podem ser de qualquer tipo.
...
* `useReducer` é um Hook, portanto, você só pode chamá-lo **no nível superior do seu componente** ou de seus próprios Hooks. Você não pode chamá-lo dentro de loops ou condições. Se precisar disso, extraia um novo componente e mova o estado para ele.
...
As ações podem ter qualquer formato. Por convenção, é comum passar objetos com uma propriedade `type` que identifica a ação. Ela deve incluir as informações mínimas necessárias para que o reducer compute o próximo estado.
...
Os nomes dos tipos de ação são locais ao seu componente. [Cada ação descreve uma única interação, mesmo que isso leve a várias alterações nos dados.](/learn/extracting-state-logic-into-a-reducer#writing-reducers-well) O formato do estado é arbitrário, mas geralmente será um objeto ou um array.

Leia [extraindo lógica de estado para um reducer](/learn/extracting-state-logic-into-a-reducer) para saber mais.
