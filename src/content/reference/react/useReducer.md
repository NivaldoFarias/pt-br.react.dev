...
Você mutou um objeto `state` existente e o retornou, então o React ignorou a atualização. Para corrigir isso, você precisa garantir que você sempre [atualiza objetos no estado](/learn/updating-objects-in-state) e [atualiza arrays no estado](/learn/updating-arrays-in-state) em vez de mutá-los:
...
### Recebo um erro: "Muitas re-renderizações" {/*im-getting-an-error-too-many-re-renders*/}
