...
* O React [agrupa atualizações de estado.](/learn/queueing-a-series-of-state-updates) Ele atualiza a tela **após todos os manipuladores de eventos terem sido executados** e chamado suas funções `set`. Isso evita múltiplas re-renderizações durante um único evento. No caso raro em que você precise forçar o React a atualizar a tela mais cedo, por exemplo, para acessar o DOM, você pode usar [`flushSync`.](/reference/react-dom/flushSync)
...
Os nomes dos tipos de ação são locais ao seu componente. [Cada ação descreve uma única interação, mesmo que isso leve a várias alterações nos dados.](/learn/extracting-state-logic-into-a-reducer#writing-reducers-well) A forma do estado é arbitrária, mas geralmente será um objeto ou um array.
