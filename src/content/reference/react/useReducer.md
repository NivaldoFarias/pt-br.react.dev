...
Para atualizar o que está na tela, chame <CodeStep step={2}>`dispatch`</CodeStep> com um objeto que representa o que o usuário fez, chamado de *ação*:
...
Em seguida, você precisa preencher o código que calculará e retornará o próximo estado. Por convenção, é comum escrevê-la como uma instrução [`switch`.](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/switch) Para cada `case` no `switch`, calcule e retorne algum próximo estado.
...
As actions podem ter qualquer formato. Por convenção, é comum passar objetos com uma propriedade `type` que identifica a action. Ela deve incluir as informações mínimas necessárias que o reducer precisa para computar o próximo estado.
...
Se atualizar arrays e objetos sem mutação parecer tedioso, você pode usar uma biblioteca como [Immer](https://github.com/immerjs/use-immer#useimmerreducer) para reduzir o código repetitivo:
...
Embora o resultado de `createInitialState(username)` seja usado apenas para a renderização inicial, você ainda está chamando essa função em cada renderização. Isso pode ser um desperdício se ela estiver criando arrays grandes ou realizando cálculos caros.
...
### Recebo um erro: "Too many re-renders" {/*im-getting-an-error-too-many-re-renders*/}

Você pode receber um erro dizendo: `Too many re-renders. React limits the number of renders to prevent an infinite loop.` Geralmente, isso significa que você está despachando incondicionalmente uma ação *durante a renderização*, fazendo com que seu componente entre em um loop: renderizar, despachar (o que causa uma renderização), renderizar, despachar (o que causa uma renderização) e assim por diante. Muito frequentemente, isso é causado por um erro ao especificar um manipulador de eventos:
...
### Minha função reducer ou inicializadora executa duas vezes {/*my-reducer-or-initializer-function-runs-twice*/}
...
Esse comportamento, **apenas em desenvolvimento**, ajuda você a [manter componentes puros.](/learn/keeping-components-pure) O React usa o resultado de uma das chamadas e ignora o resultado da outra. Contanto que suas funções de componente, inicializadora e reducer sejam puras, isso não afetará sua lógica. No entanto, se elas forem acidentalmente impuras, isso ajudará você a notar os erros.
