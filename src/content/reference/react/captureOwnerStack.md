...
- Manipuladores de erro do React ([Opções do React Root](/reference/react-dom/client/createRoot#parameters) `onCaughtError`, `onRecoverableError` e `onUncaughtError`)
...
A Pilha de Proprietários é diferente da Pilha de Componentes disponível em manipuladores de erro do React, como `errorInfo.componentStack` em `onUncaughtError` ([`/reference/react-dom/client/hydrateRoot#error-logging-in-production`](/reference/react-dom/client/hydrateRoot#error-logging-in-production)).
...
createRoot(document.createElement('div'), {
  onUncaughtError: (error, errorInfo) => {
    ...
    // destacar que os navegadores aplicarão sourcemaps às pilhas registradas.
...
Se você interceptar chamadas de <CodeStep step={1}>`console.error`</CodeStep> para destacá-las em uma sobreposição de erro, você pode chamar <CodeStep step={2}>`captureOwnerStack`</CodeStep> para incluir a Pilha de Proprietários.
...
A chamada de `captureOwnerStack` ocorreu fora de uma função controlada pelo React, por exemplo, em um callback de `setTimeout`, após uma chamada `fetch` ou em um manipulador de eventos DOM personalizado. Durante a renderização, Efeitos, manipuladores de eventos do React e manipuladores de erro do React (ex: `hydrateRoot#options.onCaughtError`), as Pilhas de Proprietários devem estar disponíveis.
