...
O React criará uma root para o `domNode`, e assumirá o gerenciamento do DOM dentro dele. Depois de criar uma root, você precisa chamar [`root.render`](#root-render) para exibir um componente React dentro dela:

```js
root.render(<App />);
```

Um aplicativo totalmente construído com React geralmente terá apenas uma chamada `createRoot` para seu componente root. Uma página que usa "salpicos" do React para partes da página pode ter quantas roots separadas forem necessárias.
...
* **opcional** `options`: Um objeto com opções para esta root React.
  ...
  * **opcional** `onUncaughtError`: Callback chamado quando um erro é lançado e não capturado por um Error Boundary. Chamado com o `error` que foi lançado, e um objeto `errorInfo` contendo o `componentStack`.
  * **opcional** `onRecoverableError`: Callback chamado quando React se recupera automaticamente de erros. Chamado com um `error` lançado pelo React, e um objeto `errorInfo` contendo o `componentStack`. Alguns erros recuperáveis podem incluir a causa original do erro como `error.cause`.
...
#### Ressalvas {/*caveats*/}
* Se seu aplicativo for renderizado no servidor, o uso de `createRoot()` não será suportado. Use [`hydrateRoot()`](/reference/react-dom/client/hydrateRoot) em vez disso.
* Você provavelmente terá apenas uma chamada de `createRoot` em seu aplicativo. Se você usar um framework, ele pode fazer essa chamada por você.
* Quando você deseja renderizar um pedaço de JSX em uma parte diferente da árvore DOM que não é filho do seu componente (por exemplo, um modal ou uma dica de ferramenta), use [`createPortal`](/reference/react-dom/createPortal) em vez de `createRoot`.

---

### `root.render(reactNode)` {/*root-render*/}

Chame `root.render` para exibir um pedaço de [JSX](/learn/writing-markup-with-jsx) ("React node") no nó DOM do navegador da root React.
...
* `reactNode`: Um *React node* que você deseja exibir. Isso geralmente será um pedaço de JSX como `<App />`, mas você também pode passar um elemento React construído com [`createElement()`](/reference/react/createElement), uma string, um número, `null` ou `undefined`.
...
* Na primeira vez que você chamar `root.render`, o React limpará todo o conteúdo HTML existente dentro da root React antes de renderizar o componente React nele.

* Se o nó DOM da sua root contiver HTML gerado pelo React no servidor ou durante a build, use [`hydrateRoot()`](/reference/react-dom/client/hydrateRoot) em vez disso, que anexa os event handlers ao HTML existente.

* Se você chamar `render` na mesma root mais de uma vez, o React atualizará o DOM conforme necessário para refletir o JSX mais recente que você passou. O React decidirá quais partes do DOM podem ser reutilizadas e quais precisam ser recriadas, ["combinando-as"](/learn/preserving-and-resetting-state) com a árvore renderizada anteriormente. Chamar `render` na mesma root novamente é semelhante a chamar a função [`set` function](/reference/react/useState#setstate) no componente root: React evita atualizações desnecessárias do DOM.
...
Isso é principalmente útil se o nó DOM da sua root React (ou qualquer um de seus ancestrais) puder ser removido do DOM por algum outro código. Por exemplo, imagine um painel de guias jQuery que remove guias inativas do DOM. Se uma guia for removida, tudo dentro dela (incluindo as roots React dentro) também seria removido do DOM. Nesse caso, você precisa dizer ao React para "parar" de gerenciar o conteúdo da root removida chamando `root.unmount`. Caso contrário, os componentes dentro da root removida não saberão como limpar e liberar recursos globais como subscriptions.

Chamar `root.unmount` desmontará todos os componentes na root e "desvinculará" o React do nó DOM da root, incluindo a remoção de quaisquer event handlers ou state na árvore.
...
* Chamar `root.unmount` desmontará todos os componentes na árvore e "desvinculará" o React do nó DOM da root.

* Depois de chamar `root.unmount`, você não pode mais chamar `root.render` na mesma root. Tentar chamar `root.render` em uma root desmontada lançará um erro "Não é possível atualizar uma root desmontada". No entanto, você pode criar uma nova root para o mesmo nó DOM após a root anterior para esse nó ter sido desmontada.

---

## Uso {/*usage*/}

### Renderizando um aplicativo totalmente criado com React {/*rendering-an-app-fully-built-with-react*/}

Se seu aplicativo for totalmente construído com React, crie uma única root para todo o seu aplicativo.
...
**Se seu aplicativo for totalmente construído com React, você não deverá precisar criar mais roots ou chamar [`root.render`](#root-render) novamente.**

A partir deste ponto, o React irá gerenciar o DOM de todo o seu aplicativo. Para adicionar mais componentes, [aninhá-los dentro do componente `App`.](/learn/importing-and-exporting-components) Quando você precisar atualizar a UI, cada um de seus componentes pode fazer isso [usando state.](/reference/react/useState) Quando você precisar exibir conteúdo extra, como um modal ou uma dica de ferramenta fora do nó DOM, [renderize-o com um portal.](/reference/react-dom/createPortal)

<Note>

Quando o seu HTML estiver vazio, o usuário verá uma página em branco até que o código JavaScript do aplicativo seja carregado e executado:

```html
<div id="root"></div>
```

Isso pode parecer muito lento! Para resolver isso, você pode gerar o HTML inicial de seus componentes [no servidor ou durante a build.](/reference/react-dom/server) Então seus visitantes podem ler texto, ver imagens e clicar em links antes que qualquer código JavaScript seja carregado. Recomendamos [usar um framework](/learn/start-a-new-react-project#production-grade-react-frameworks) que faça essa otimização pronta para uso. Dependendo de quando ele é executado, isso é chamado de *server-side rendering (SSR)* ou *static site generation (SSG).*

</Note>

<Pitfall>

**Aplicativos que usam renderização no servidor ou geração estática devem chamar [`hydrateRoot`](/reference/react-dom/client/hydrateRoot) em vez de `createRoot`.** O React então *hidratará* (reutilizará) os nós DOM do seu HTML em vez de destruí-los e recriá-los.
...
Isso é principalmente útil se os seus componentes React estiverem dentro de um aplicativo escrito em um framework diferente.
...
Por padrão, o React registrará todos os erros não detectados no console. Para implementar seu próprio relatório de erros, você pode fornecer a opção root `onUncaughtError` opcional:
...
Você pode usar a opção root `onUncaughtError` para exibir caixas de diálogo de erro:
...
function reportError({ title, error, componentStack, dismissable }) {
  ...
  // Já que já exibimos a mensagem, remova-a e a primeira linha de erro:
  ...
  // Exiba o botão fechar, se dispensável
  if (dismissable) {
...
Você pode usar a opção root `onCaughtError` para exibir diálogos de erro ou filtrar erros conhecidos de registros:
...
function fallbackRender({ resetErrorBoundary }) {
  return (
    <div role="alert">
      ...
      <p>Algo deu errado.</p>
      <button onClick={resetErrorBoundary}>Resetar</button>
    </div>
  );
}
...
O React pode renderizar automaticamente um componente pela segunda vez para tentar se recuperar de um erro lançado em renderização. Se for bem-sucedido, o React registrará um erro recuperável no console para notificar o desenvolvedor. Para substituir esse comportamento, você pode fornecer a opção root opcional `onRecoverableError`:
...
Você pode usar a opção root `onRecoverableError` para exibir diálogos de erro:
  ...
  return (
    <>
      ...
      >
        ...
        <p>Como ele se recuperou, nenhum Error Boundary foi mostrado, mas <code>onRecoverableError</code> foi usado para mostrar um diálogo de erro.</p>
      </ErrorBoundary>
...
function Throw({error}) {
  // Simula um valor externo mudando durante a renderização concorrente.
  errorThrown = true;
  foo.bar = 'baz';
}
```
...
### Criei uma root, mas nada é exibido {/*ive-created-a-root-but-nothing-is-displayed*/}

Certifique-se de não ter esquecido de *renderizar* seu app na root:
...
Até que você faça isso, nada será exibido.
...
Para corrigir, passe as opções root para `createRoot(...)`, não para `root.render(...)`:
```js {2,5}
...
Este erro significa que qualquer coisa que você está passando para `createRoot` não é um nó do DOM.

Se você não tiver certeza do que está acontecendo, tente registrá-lo:
...
Por exemplo, se `domNode` for `null`, significa que [`getElementById`](https://developer.mozilla.org/en-US/docs/Web/API/Document/getElementById) retornou `null`. Isso acontecerá se não houver nenhum nó no documento com o ID fornecido no momento da sua chamada. Pode haver alguns motivos para isso:
...
Outra maneira comum de obter este erro é escrever `createRoot(<App />)` em vez de `createRoot(domNode)`.

---

### Estou recebendo um erro: "Funções não são válidas como um filho React." {/*im-getting-an-error-functions-are-not-valid-as-a-react-child*/}

Este erro significa que qualquer coisa que você está passando para `root.render` não é um componente React.
...
Se o seu app é renderizado no servidor e inclui o HTML inicial gerado pelo React, você pode notar que criar uma root e chamar `root.render` exclui todo esse HTML e, em seguida, recria todos os nós DOM do zero. Isso pode ser mais lento, redefine as posições de foco e de rolagem e pode perder outras entradas do usuário.
