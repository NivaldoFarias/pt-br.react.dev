...
Chame `createRoot` para criar uma raiz React para exibir conteúdo dentro de um elemento DOM do navegador.
...
O React irá criar uma raiz para o `domNode` e assumirá o controle do gerenciamento do DOM dentro dele. Depois de criar uma raiz, você precisará chamar [`root.render`](#root-render) para exibir um componente React dentro dela:

```js
root.render(<App />);
```

Um aplicativo totalmente construído com React geralmente terá apenas uma chamada `createRoot` para seu componente raiz. Uma página que usa "borrifos" de React para partes da página pode ter quantas raízes separadas forem necessárias.

[Veja mais exemplos abaixo.](#usage)

#### Parâmetros {/*parameters*/}

* `domNode`: Um [elemento DOM.](https://developer.mozilla.org/en-US/docs/Web/API/Element) O React criará uma raiz para esse elemento DOM e permitirá que você chame funções na raiz, como `render`, para exibir conteúdo renderizado do React.

* **opcional** `options`: Um objeto com opções para esta raiz React.

  * **opcional** `onCaughtError`: Callback chamado quando o React captura um erro em um Limite de Erro. Chamado com o `erro` capturado pelo Limite de Erro e um objeto `errorInfo` contendo o `componentStack`.
  * **opcional** `onUncaughtError`: Callback chamado quando um erro é lançado e não é capturado por um Limite de Erro. Chamado com o `erro` que foi lançado e um objeto `errorInfo` contendo o `componentStack`.
  * **opcional** `onRecoverableError`: Callback chamado quando o React se recupera automaticamente de erros. Chamado com um `erro` que o React lança e um objeto `errorInfo` contendo o `componentStack`. Alguns erros recuperáveis podem incluir a causa original do erro como `error.cause`.
...
#### Ressalvas {/*caveats*/}
* Se seu aplicativo for renderizado no servidor, o uso de `createRoot()` não é suportado. Use [`hydrateRoot()`](/reference/react-dom/client/hydrateRoot) em vez disso.
* Você provavelmente terá apenas uma chamada `createRoot` em seu aplicativo. Se você usar um *framework*, ele pode fazer essa chamada para você.
* Quando você quiser renderizar um pedaço de JSX em uma parte diferente da árvore DOM que não é filha do seu componente (por exemplo, um modal ou uma dica de ferramenta), use [`createPortal`](/reference/react-dom/createPortal) em vez de `createRoot`.
...
O React exibirá `<App />` na `root` e assumirá o controle do gerenciamento do DOM dentro dele.
...
* Na primeira vez que você chamar `root.render`, o React limpará todo o conteúdo HTML existente dentro da raiz React antes de renderizar o componente React nele.

* Se o nó DOM da sua raiz contiver HTML gerado pelo React no servidor ou durante a compilação, use [`hydrateRoot()`](/reference/react-dom/client/hydrateRoot) em vez disso, que anexa os manipuladores de eventos ao HTML existente.

* Se você chamar `render` na mesma raiz mais de uma vez, o React atualizará o DOM conforme necessário para refletir o último JSX que você passou. O React decidirá quais partes do DOM podem ser reutilizadas e quais precisam ser recriadas, ["combinando-o"](/learn/preserving-and-resetting-state) com a árvore renderizada anteriormente. Chamar `render` na mesma raiz novamente é semelhante a chamar a função [`set`](/reference/react/useState#setstate) no componente raiz: o React evita atualizações desnecessárias do DOM.
...
Isso é útil principalmente se o nó DOM da sua raiz React (ou qualquer um de seus ancestrais) puder ser removido do DOM por algum outro código. Por exemplo, imagine um painel de abas jQuery que remove abas inativas do DOM. Se uma aba for removida, tudo dentro dela (incluindo as raízes React dentro) também seria removido do DOM. Nesse caso, você precisa dizer ao React para "parar" de gerenciar o conteúdo da raiz removida chamando `root.unmount`. Caso contrário, os componentes dentro da raiz removida não saberão como limpar e liberar recursos globais, como assinaturas.

Chamar`root.unmount` desmontará todos os componentes na raiz e "desvinculará" o React do nó DOM da raiz, incluindo a remoção de quaisquer manipuladores de eventos ou estado na árvore.
...
Se seu aplicativo for totalmente construído com React, crie uma única raiz para todo o seu aplicativo.
...
A partir deste ponto, o React irá gerenciar o DOM de todo o seu aplicativo. Para adicionar mais componentes, [aninhá-los dentro do componente `App`.](/learn/importing-and-exporting-components) Quando você precisar atualizar a UI, cada um de seus componentes pode fazer isso [usando o estado.](/reference/react/useState) Quando você precisar exibir conteúdo extra, como um modal ou uma dica de ferramenta fora do nó DOM, [renderize-o com um portal.](/reference/react-dom/createPortal)
...
Isso pode parecer muito lento! Para resolver isso, você pode gerar o HTML inicial de seus componentes [no servidor ou durante a compilação.](/reference/react-dom/server) Então seus visitantes podem ler texto, ver imagens e clicar em links antes que qualquer um dos códigos JavaScript seja carregado. Recomendamos o [uso de um *framework*](/learn/start-a-new-react-project#production-grade-react-frameworks) que faça essa otimização logo de cara. Dependendo de quando ele é executado, isso é chamado de *renderização no lado do servidor (SSR)* ou *geração de site estático (SSG).*

</Note>

<Pitfall>

**Aplicativos que usam renderização no servidor ou geração estática devem chamar [`hydrateRoot`](/reference/react-dom/client/hydrateRoot) em vez de `createRoot`.** O React irá então *hidratar* (reutilizar) os nós DOM do seu HTML em vez de destruí-los e recriá-los.
...
Se sua página [não for totalmente construída com React](/learn/add-react-to-an-existing-project#using-react-for-a-part-of-your-existing-page), você pode chamar `createRoot` várias vezes para criar uma raiz para cada parte da UI de nível superior gerenciada pelo React. Você pode exibir conteúdo diferente em cada raiz chamando [`root.render`.](#root-render)
...
const root = createRoot(domNode); 
root.render(<Comment />);
document.body.appendChild(domNode); // Você pode adicioná-lo a qualquer lugar no documento
```
...
Não é comum chamar `render` várias vezes. Normalmente, seus componentes [atualizarão o estado](/reference/react/useState) em vez disso.
...
Você pode usar a opção root `onCaughtError` para exibir caixas de diálogo de erro ou filtrar erros conhecidos do log:
...
<body>
<!--
  Caixa de diálogo de erro em HTML puro
  já que um erro no aplicativo React pode travar.
-->
<div id="error-dialog" class="hidden">
  <h1 id="error-title" class="text-red"></h1>
...
function reportError({ title, error, componentStack, dismissable }) {
  ...
  // Uma vez que já exibimos a mensagem, retire-a e a primeira linha Error:.
  errorStack.innerText = error.stack.replace(error.message, '').split(/\n(.*)/s)[1];
  ...
  // Exibir o botão fechar, se for dispensável
  if (dismissable) {
    errorNotDismissible.classList.add('hidden');
    errorClose.classList.remove("hidden");
  } else {
...
O React pode renderizar automaticamente um componente uma segunda vez para tentar se recuperar de um erro lançado em render. Se for bem-sucedido, o React registrará um erro recuperável no console para notificar o desenvolvedor. Para substituir este comportamento, você pode fornecer a opção root opcional `onRecoverableError`:
...
Você pode usar a opção root `onRecoverableError` para exibir caixas de diálogo de erro:
  ...
  return (
    <>
      ...
      >
        ...
        <p>Como se recuperou, nenhum Error Boundary foi exibido, mas <code>onRecoverableError</code> foi usado para exibir uma caixa de diálogo de erro.</p>
      </ErrorBoundary>
...
### Criei um root, mas nada é exibido {/*ive-created-a-root-but-nothing-is-displayed*/}

Certifique-se de não ter esquecido de *renderizar* seu aplicativo no root:
...
Para corrigir, passe as opções root para `createRoot(...)`, não `root.render(...)`:
```js {2,5}
...
Este erro significa que qualquer coisa que você está passando para `createRoot` não é um nó DOM.
...
Por exemplo, se `domNode` for `null`, significa que [`getElementById`](https://developer.mozilla.org/pt-BR/docs/Web/API/Document/getElementById) retornou `null`. Isso acontecerá se não houver nenhum nó no documento com o ID fornecido no momento da sua chamada. Pode haver algumas razões para isso:
...
2. A tag `<script>` do seu bundle não pode "ver" nenhum nó DOM que apareça *depois* dela no HTML.
...
Se seu aplicativo for renderizado no servidor e incluir o HTML inicial gerado pelo React, você pode notar que criar um root e chamar `root.render` exclui todo aquele HTML e, em seguida, recria todos os nós DOM do zero. Isso pode ser mais lento, redefine as posições de foco e rolagem e pode perder outras entradas do usuário.
...
Observe que sua API é diferente. Em particular, geralmente não haverá outra chamada `root.render`.
