...
Todos os componentes embutidos do navegador, como [`<div>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/div), suportam algumas props e eventos comuns.
...
Essas props especiais do React são suportadas para todos os componentes embutidos:

* `children`: Um nó React (um elemento, uma string, um número, [um portal,](/reference/react-dom/createPortal) um nó vazio como `null`, `undefined` e booleanos, ou um array de outros nós React). Especifica o conteúdo dentro do componente. Quando você usa JSX, normalmente você especifica a prop `children` implicitamente aninhando tags como `<div><span /></div>`.
...
* [`contentEditable`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/contenteditable): Um booleano. Se `true`, o navegador permite que o usuário edite o elemento renderizado diretamente. Isso é usado para implementar bibliotecas de entrada de texto avançadas como [Lexical.](https://lexical.dev/) O React avisa se você tentar passar filhos React para um elemento com `contentEditable={true}` porque o React não poderá atualizar seu conteúdo após as edições do usuário.
* [`data-*`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/data-*): Os atributos de dados permitem que você anexe alguns dados de string ao elemento, por exemplo, `data-fruit="banana"`. No React, eles não são comumente usados porque você normalmente leria dados de props ou state.
...
* [`onAuxClick`](https://developer.mozilla.org/en-US/docs/Web/API/Element/auxclick_event): Uma função de [`MouseEvent` handler](#mouseevent-handler). Dispara quando um botão do ponteiro não primário foi clicado.
...
* `onBlur`: Uma função de [`FocusEvent` handler](#focusevent-handler). Dispara quando um elemento perde o foco. Ao contrário do evento [`blur`](https://developer.mozilla.org/en-US/docs/Web/API/Element/blur_event) do navegador embutido, no React o evento `onBlur` propaga.
...
* [`onDragEnter`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/dragenter_event): Uma função de [`DragEvent` handler](#dragevent-handler). Dispara quando o conteúdo arrastado entra em um destino de soltar válido.
...
* [`onDragOver`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/dragover_event): Uma função de [`DragEvent` handler](#dragevent-handler). Dispara em um destino de soltar válido enquanto o conteúdo arrastado é arrastado sobre ele. Você deve chamar `e.preventDefault()` aqui para permitir soltar.
...
* [`onDrop`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/drop_event): Uma função de [`DragEvent` handler](#dragevent-handler). Dispara quando algo é solto em um destino de soltar válido.
* `onDropCapture`: Uma versão de `onDrop` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)
* `onFocus`: Uma função de [`FocusEvent` handler](#focusevent-handler). Dispara quando um elemento recebe foco. Ao contrário do evento [`focus`](https://developer.mozilla.org/en-US/docs/Web/API/Element/focus_event) do navegador embutido, no React o evento `onFocus` propaga.
...
* [`onMouseDown`](https://developer.mozilla.org/en-US/docs/Web/API/Element/mousedown_event): Uma função de [`MouseEvent` handler](#mouseevent-handler). Dispara quando o ponteiro é pressionado para baixo.
...
* [`onMouseEnter`](https://developer.mozilla.org/en-US/docs/Web/API/Element/mouseenter_event): Uma função de [`MouseEvent` handler](#mouseevent-handler). Dispara quando o ponteiro se move para dentro de um elemento. Não tem uma fase de captura. Em vez disso, `onMouseLeave` e `onMouseEnter` propagam do elemento que está sendo deixado para o que está sendo inserido.
* [`onMouseLeave`](https://developer.mozilla.org/en-US/docs/Web/API/Element/mouseleave_event): Uma função de [`MouseEvent` handler](#mouseevent-handler). Dispara quando o ponteiro se move para fora de um elemento. Não tem uma fase de captura. Em vez disso, `onMouseLeave` e `onMouseEnter` propagam do elemento que está sendo deixado para o que está sendo inserido.
...
* [`onPointerCancel`](https://developer.mozilla.org/en-US/docs/Web/API/Element/pointercancel_event): Uma função de [`PointerEvent` handler](#pointerevent-handler). Dispara quando o navegador cancela uma interação do ponteiro.
...
* [`onPointerEnter`](https://developer.mozilla.org/en-US/docs/Web/API/Element/pointerenter_event): Uma função de [`PointerEvent` handler](#pointerevent-handler). Dispara quando um ponteiro se move para dentro de um elemento. Não tem uma fase de captura. Em vez disso, `onPointerLeave` e `onPointerEnter` propagam do elemento que está sendo deixado para o que está sendo inserido.
* [`onPointerLeave`](https://developer.mozilla.org/en-US/docs/Web/API/Element/pointerleave_event): Uma função de [`PointerEvent` handler](#pointerevent-handler). Dispara quando um ponteiro se move para fora de um elemento. Não tem uma fase de captura. Em vez disso, `onPointerLeave` e `onPointerEnter` propagam do elemento que está sendo deixado para o que está sendo inserido.
...
* [`onSelect`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement/select_event): Uma função de [`Event` handler](#event-handler). Dispara após a alteração da seleção dentro de um elemento editável, como uma entrada. O React estende o evento `onSelect` para funcionar também para elementos `contentEditable={true}`. Além disso, o React o estende para disparar para seleção vazia e em edições (que podem afetar a seleção).
* `onSelectCapture`: Uma versão de `onSelect` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)
* [`onTouchCancel`](https://developer.mozilla.org/en-US/docs/Web/API/Element/touchcancel_event): Uma função de [`TouchEvent` handler](#touchevent-handler). Dispara quando o navegador cancela uma interação com o toque.
...
* [`onTouchMove`](https://developer.mozilla.org/en-US/docs/Web/API/Element/touchmove_event): Uma função de [`TouchEvent` handler](#touchevent-handler). Dispara um ou mais pontos de toque sendo movidos.
...
* [`role`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles): Uma string. Especifica explicitamente o role do elemento para tecnologias assistivas.
...
* [`spellCheck`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/spellcheck): Um booleano ou nulo. Se definido explicitamente como `true` ou `false`, ativa ou desativa a verificação ortográfica.
...
* [`translate`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/translate):  `'yes'` ou `'no'`. Passar `'no'` exclui o conteúdo do elemento da tradução.
```Você também pode passar atributos customizados como props, por exemplo `mycustomprop="someValue"`. Isto pode ser útil quando integrar com bibliotecas de terceiros. O nome do atributo customizado deve ser em minúsculas e não deve começar com `on`. O valor será convertido em uma string. Se você passar `null` ou `undefined`, o atributo customizado será removido.

Estes eventos disparam somente para os elementos [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form):
...
Estes eventos disparam somente para elementos [`<dialog>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/dialog). Diferente dos eventos do navegador, eles propagam no React:

* [`onCancel`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLDialogElement/cancel_event): Uma função [`Event` handler](#event-handler). Dispara quando o usuário tenta dispensar a caixa de diálogo.
* `onCancelCapture`: Uma versão de `onCancel` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)
* [`onClose`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLDialogElement/close_event): Uma função [`Event` handler](#event-handler). Dispara quando uma caixa de diálogo foi fechada.
* `onCloseCapture`: Uma versão de `onClose` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)

Estes eventos disparam somente para os elementos [`<details>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/details). Diferente dos eventos do navegador, eles propagam no React:
...
Estes eventos disparam para os elementos [`<img>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img), [`<iframe>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe), [`<object>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/object), [`<embed>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/embed), [`<link>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link) e [SVG `<image>`](https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorial/SVG_Image_Tag). Diferente dos eventos do navegador, eles propagam no React:
...
Estes eventos disparam para recursos como [`<audio>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio) e [`<video>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/video). Diferente dos eventos do navegador, eles propagam no React:
...
* [`onCanPlay`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/canplay_event): Uma função [`Event` handler](#event-handler). Dispara quando há dados suficientes para começar a reproduzir, mas não o suficiente para reproduzir até o fim, sem buffer.
* `onCanPlayCapture`: Uma versão de `onCanPlay` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)
* [`onCanPlayThrough`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/canplaythrough_event): Uma função [`Event` handler](#event-handler). Dispara quando há dados suficientes que é provável que seja possível começar a reproduzir sem buffer até o fim.
...
* [`onError`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/error_event): Uma função [`Event` handler](#event-handler). Dispara quando o recurso não pôde ser carregado.
* `onErrorCapture`: Uma versão de `onError` que dispara na [fase de captura.](/learn/responding-to-events#capture-phase-events)
* [`onLoadedData`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/loadeddata_event): Uma função [`Event` handler](#event-handler). Dispara quando o frame de reprodução atual foi carregado.
...
### Função *callback* `ref` {/*ref-callback*/}
```
### Ao invés de um objeto ref (como o retornado por [`useRef`](/reference/react/useRef#manipulating-the-dom-with-a-ref)), você pode passar uma função para o atributo `ref`.
...
[Veja um exemplo de uso do `ref` callback.](/learn/manipulating-the-dom-with-refs#how-to-manage-a-list-of-refs-using-a-ref-callback)

Quando o nó do DOM `<div>` for adicionado à tela, o React chamará seu `ref` callback com o `node` do DOM como o argumento. Quando esse nó do DOM `<div>` for removido, o React chamará sua função de limpeza retornada do callback.

O React também chamará seu `ref` callback sempre que você passar um callback de `ref` *diferente*. No exemplo acima, `(node) => { ... }` é uma função diferente em cada renderização. Quando seu componente renderizar novamente, a função *anterior* será chamada com `null` como argumento, e a função *seguinte* será chamada com o nó do DOM.

#### Parâmetros {/*ref-callback-parameters*/}

*   `node`: Um nó do DOM. React passará o nó do DOM quando a ref for anexada. A menos que você passe a mesma referência de função para o `ref` callback em cada renderização, o callback será temporariamente limpo e recriado durante cada nova renderização do componente.

<Note>

#### React 19 adicionou funções de limpeza para callbacks `ref`. {/*react-19-added-cleanup-functions-for-ref-callbacks*/}

Para dar suporte à compatibilidade com versões anteriores, se uma função de limpeza não for retornada do `ref` callback, `node` será chamado com `null` quando a `ref` for destacada. Esse comportamento será removido em uma versão futura.

</Note>

#### Retorna {/*returns*/}

*   **opcional** `função de limpeza`: Quando a `ref` é desanexada, o React chamará a função de limpeza. Se uma função não for retornada pelo `ref` callback, o React chamará o callback novamente com `null` como argumento quando a `ref` for desanexada. Esse comportamento será removido em uma versão futura.

#### Ressalvas {/*caveats*/}

*   Quando o Modo Strict estiver ativado, o React vai **executar um ciclo extra (apenas no ambiente de desenvolvimento) de configuração + limpeza** antes da primeira configuração real. Este é um teste de estresse que garante que sua lógica de limpeza "espelhe" sua lógica de configuração e que ela pare ou desfaça o que a configuração está fazendo. Se isso causar um problema, implemente a função de limpeza.
*   Quando você passa um callback de `ref` *diferente*, o React chamará a função de limpeza do callback *anterior*, se fornecida. Se nenhuma função de limpeza for definida, o `ref` callback será chamado com `null` como argumento. A função *seguinte* será chamada com o nó do DOM.

---

### Objeto de evento React {/*react-event-object*/}

Seus manipuladores de eventos receberão um *objeto de evento React.* Ele também é conhecido como "evento sintético".
...
Alguns eventos React não são mapeados diretamente para os eventos nativos do navegador. Por exemplo, em `onMouseLeave`, `e.nativeEvent` apontará para um evento `mouseout`. O mapeamento específico não faz parte da API pública e pode mudar no futuro. Se você precisar do evento do navegador subjacente por algum motivo, leia-o de `e.nativeEvent`.

#### Propriedades {/*react-event-object-properties*/}

Objetos de evento React implementam algumas das propriedades padrão [`Event`](https://developer.mozilla.org/en-US/docs/Web/API/Event):

*   [`bubbles`](https://developer.mozilla.org/en-US/docs/Web/API/Event/bubbles): Um booleano. Retorna se o evento propaga pelo DOM.
...
*   [`target`](https://developer.mozilla.org/en-US/docs/Web/API/Event/target): Um nó do DOM. Retorna o nó em que o evento ocorreu (que pode ser um filho distante).
...
Além disso, os objetos de evento React fornecem estas propriedades:

*   `nativeEvent`: Um [`Event`](https://developer.mozilla.org/en-US/docs/Web/API/Event) do DOM. O objeto de evento do navegador original.

#### Métodos {/*react-event-object-methods*/}

Objetos de evento React implementam alguns dos métodos padrão [`Event`](https://developer.mozilla.org/en-US/docs/Web/API/Event):
...
Além disso, os objetos de evento React fornecem estes métodos:
...
*   Os valores de `currentTarget`, `eventPhase`, `target` e `type` refletem os valores que seu código React espera. Por baixo dos panos, o React anexa manipuladores de eventos na raiz, mas isso não é refletido em objetos de evento React. Por exemplo, `e.currentTarget` pode não ser o mesmo que o `e.nativeEvent.currentTarget` subjacente. Para eventos polyfilled, `e.type` (tipo de evento React) pode diferir de `e.nativeEvent.type` (tipo subjacente).

---

### Função manipuladora de eventos `AnimationEvent` {/*animationevent-handler*/}

Um tipo de manipulador de eventos para os eventos [CSS animation](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Animations/Using_CSS_animations).
...
### Função manipuladora de eventos `ClipboardEvent` {/*clipboadevent-handler*/}

Um tipo de manipulador de eventos para os eventos [Clipboard API](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard_API).
...
### Função manipuladora de eventos `CompositionEvent` {/*compositionevent-handler*/}

Um tipo de manipulador de eventos para os eventos [input method editor (IME)](https://developer.mozilla.org/en-US/docs/Glossary/Input_method_editor).
...
### Função manipuladora de eventos `DragEvent` {/*dragevent-handler*/}

Um tipo de manipulador de eventos para os eventos [HTML Drag and Drop API](https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API).
...
*   `e`: Um [objeto de evento React](#react-event-object) com estas propriedades [`DragEvent`](https://developer.mozilla.org/en-US/docs/Web/API/DragEvent) extras:
    *   [`dataTransfer`](https://developer.mozilla.org/en-US/docs/Web/API/DragEvent/dataTransfer)
    ...
    Também inclui as propriedades [`MouseEvent`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent) herdadas:
    ...
    *   [`shiftKey`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/shiftKey)
    ...
    Também inclui as propriedades [`UIEvent`](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent) herdadas:
...
### Função manipuladora de eventos `FocusEvent` {/*focusevent-handler*/}
...
### Função manipuladora de eventos `Event` {/*event-handler*/}
...
### Função manipuladora de eventos `InputEvent` {/*inputevent-handler*/}
...
### Função manipuladora de eventos `KeyboardEvent` {/*keyboardevent-handler*/}
...
### Função manipuladora de eventos `MouseEvent` {/*mouseevent-handler*/}
...
### Função manipuladora de eventos `PointerEvent` {/*pointerevent-handler*/}
...
``````
  ...
  * [`shiftKey`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/shiftKey)
  ...
  Ele também inclui as propriedades herdadas [`UIEvent`](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent):
...
### Função de manipulador de eventos `TouchEvent` {/*touchevent-handler*/}
...
*   `e`: Um [objeto de evento React](#react-event-object) com estas propriedades [`TouchEvent`](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent) extras:
...
### Função de manipulador de eventos `TransitionEvent` {/*transitionevent-handler*/}
...
*   `e`: Um [objeto de evento React](#react-event-object) com estas propriedades [`TransitionEvent`](https://developer.mozilla.org/en-US/docs/Web/API/TransitionEvent) extras:
...
### Função de manipulador de eventos `UIEvent` {/*uievent-handler*/}
...
*   `e`: Um [objeto de evento React](#react-event-object) com estas propriedades [`UIEvent`](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent) extras:
    *   [`detail`](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent/detail)
    *   [`view`](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent/view)

---

### Função de manipulador de eventos `WheelEvent` {/*wheelevent-handler*/}
...
*   `e`: Um [objeto de evento React](#react-event-object) com estas propriedades [`WheelEvent`](https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent) extras:
    *   [`deltaMode`](https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent/deltaMode)
    *   [`deltaX`](https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent/deltaX)
    *   [`deltaY`](https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent/deltaY)
    *   [`deltaZ`](https://developer.mozilla.org/en-US/docs/Web/API/WheelEvent/deltaZ)

    Ele também inclui as propriedades herdadas [`MouseEvent`](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent):
...
O React não prescreve como você adiciona arquivos CSS. No caso mais simples, você adicionará uma tag [`<link>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link) ao seu HTML. Se você usa uma ferramenta de build ou um framework, consulte sua documentação para saber como adicionar um arquivo CSS ao seu projeto.
...
No exemplo acima, `style={{}}` não é uma sintaxe especial, mas um objeto `{}` regular dentro de `style={ }` [chaves JSX.](/learn/javascript-in-jsx-with-curly-braces) Recomendamos usar o atributo `style` somente quando seus estilos dependem de variáveis JavaScript.
...
Para aplicar classes CSS condicionalmente, você precisa produzir a string `className` por conta própria usando JavaScript.

Por exemplo, `className={'row ' + (isSelected ? 'selected': '')}` produzirá `className="row"` ou `className="row selected"` dependendo se `isSelected` for `true`.
...
### Manipulando um nó do DOM com uma ref {/*manipulating-a-dom-node-with-a-ref*/}

Às vezes, você precisará obter o nó do DOM do navegador associado a uma tag em JSX. Por exemplo, se você quiser focar em um `<input>` quando um botão for clicado, você precisa chamar [`focus()`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/focus) no nó do DOM `<input>` do navegador.

Para obter o nó do DOM do navegador para uma tag, [declare uma ref](/reference/react/useRef) e passe-a como o atributo `ref` para essa tag:
...
O React colocará o nó do DOM em `inputRef.current` depois que ele for renderizado na tela.
...
Leia mais sobre [manipulação do DOM com refs](/learn/manipulating-the-dom-with-refs) e [veja mais exemplos.](/reference/react/useRef#examples-dom)
...
**Isso é perigoso. Como com a propriedade [`innerHTML`](https://developer.mozilla.org/en-US/docs/Web/API/Element/innerHTML) do DOM subjacente, você deve ter extrema cautela! A menos que a marcação venha de uma fonte totalmente confiável, é trivial introduzir uma vulnerabilidade [XSS](https://pt.wikipedia.org/wiki/Cross-site_scripting) dessa maneira.**

Por exemplo, se você usar uma biblioteca Markdown que converte Markdown em HTML, você confia que seu analisador não contém bugs, e o usuário só vê sua própria entrada, você pode exibir o HTML resultante assim:
...
function renderMarkdownToHTML(markdown) {
  // Isso é SEGURO SOMENTE porque a saída HTML
  // é exibida para o mesmo usuário, e porque você
  // confia nesse analisador Markdown para não ter bugs.
...
O objeto `{__html}` deve ser criado o mais próximo possível de onde o HTML é gerado, como o exemplo acima faz na função `renderMarkdownToHTML`. Isso garante que todo o HTML bruto usado em seu código seja explicitamente marcado como tal, e que apenas as variáveis que você espera conter HTML sejam passadas para `dangerouslySetInnerHTML`. Não é recomendado criar o objeto inline como `<div dangerouslySetInnerHTML={{__html: markup}} />`.

Para ver por que renderizar HTML arbitrário é perigoso, substitua o código acima por este:

```js {1-4,7,8}
const post = {
  // Imagine que esse conteúdo está armazenado no banco de dados.
...
O código incorporado no HTML será executado. Um hacker pode usar esse buraco de segurança para roubar informações do usuário ou para executar ações em seu nome. **Use `dangerouslySetInnerHTML` apenas com dados confiáveis e limpos.**
...
No React, [eventos de foco](#focusevent-handler) propagam. Você pode usar `currentTarget` e `relatedTarget` para diferenciar se os eventos de foco ou desfoque se originaram de fora do elemento pai. O exemplo mostra como detectar o foco em um filho, o foco no elemento pai e como detectar a entrada ou saída do foco de toda a subárvore.
    ...
    <div
      ...
      onFocus={(e) => {
        if (e.currentTarget === e.target) {
          console.log('pai focado');
        } else {
          console.log('filho focado', e.target.name);
        }
        if (!e.currentTarget.contains(e.relatedTarget)) {
          // Não é acionado ao trocar o foco entre filhos
          console.log('foco entrou no pai');
        }
      }}
      onBlur={(e) => {
        if (e.currentTarget === e.target) {
          console.log('pai desfocado');
        } else {
          console.log('filho desfocado', e.target.name);
        }
        if (!e.currentTarget.contains(e.relatedTarget)) {
          // Não é acionado ao trocar o foco entre filhos
