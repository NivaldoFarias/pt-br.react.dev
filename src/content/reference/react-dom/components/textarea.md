...
O [componente `<textarea>` nativo do navegador](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/textarea) permite que você renderize uma entrada de texto de várias linhas.
...
Para exibir uma área de texto, renderize o componente [`<textarea>` nativo do navegador](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/textarea).
...
Se seu `<textarea>` for não controlado, você pode passar a prop `defaultValue` em vez disso:

* `defaultValue`: Uma string. Especifica [o valor inicial](#providing-an-initial-value-for-a-text-area) para uma área de texto.

Estas props de `<textarea>` são relevantes tanto para áreas de texto não controladas quanto para controladas:

* [`autoComplete`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#autocomplete): `'on'` ou `'off'`. Especifica o comportamento de preenchimento automático.
* [`autoFocus`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#autofocus): Um booleano. Se for `true`, o React irá focar no elemento ao montá-lo.
...
* [`disabled`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#disabled): Um booleano. Se for `true`, a entrada não será interativa e aparecerá esmaecida.
...
* [`readOnly`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#readonly): Um booleano. Se for `true`, a área de texto não pode ser editada pelo usuário.
* [`required`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#required): Um booleano. Se for `true`, o valor deve ser fornecido para que o formulário seja enviado.
...
* [`wrap`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/textarea#wrap): `'hard'`, `'soft'` ou `'off'`. Especifica como o texto deve ser quebrado ao enviar um formulário.

#### Ressalvas {/*caveats*/}

- Passar filhos como `<textarea>something</textarea>` não é permitido. [Use `defaultValue` para o conteúdo inicial.](#providing-an-initial-value-for-a-text-area)
...
- Toda área de texto controlada precisa de um manipulador de eventos `onChange` que atualiza de forma síncrona seu valor de suporte.
...
Opcionalmente, você pode especificar o valor inicial para a área de texto. Passe-o como a string `defaultValue`.
...
Adicione um [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) em volta da sua textarea com um [`<button type="submit">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/button) dentro. Ele chamará seu manipulador de eventos `<form onSubmit>`. Por padrão, o navegador enviará os dados do formulário para o URL atual e atualizará a página. Você pode substituir esse comportamento chamando `e.preventDefault()`. Leia os dados do formulário com [`new FormData(e.target)`](https://developer.mozilla.org/en-US/docs/Web/API/FormData).
<Sandpack>
...
export default function EditPost() {
  function handleSubmit(e) {
    ...
    // Você pode passar formData como um body de fetch diretamente:
...
Por padrão, *qualquer* `<button>` dentro de um `<form>` o enviará. Isso pode ser surpreendente! Se você tiver seu próprio componente `Button` React personalizado, considere retornar [`<button type="button">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/button) em vez de `<button>`. Então, para ser explícito, use `<button type="submit">` para botões que *devem* enviar o formulário.
...
Uma área de texto como `<textarea />` é *não controlada.* Mesmo se você [passar um valor inicial](#providing-an-initial-value-for-a-text-area) como `<textarea defaultValue="Texto inicial" />`, seu JSX só especifica o valor inicial, não o valor agora.
...
function NewPost() {
  ...
  return (
    <textarea
      ...
      onChange={e => setPostContent(e.target.value)} // ... e atualize a variável de estado em quaisquer edições!
    />
  );
}
```

Isso é útil se você quiser renderizar novamente alguma parte da UI em resposta a cada pressionamento de tecla.
...
**Se você passar `value` sem `onChange`, será impossível digitar na área de texto.** Quando você controla uma área de texto passando algum `value` para ela, você a *força* a sempre ter o valor que você passou. Portanto, se você passar uma variável de estado como um `value`, mas esquecer de atualizar essa variável de estado de forma síncrona durante o manipulador de eventos `onChange`, o React reverterá a área de texto após cada pressionamento de tecla de volta para o `value` que você especificou.
...
Se isso não resolver o problema, é possível que a área de texto seja removida e adicionada novamente do DOM a cada pressionamento de tecla. Isso pode acontecer se você estiver acidentalmente [redefinindo o estado](/learn/preserving-and-resetting-state) a cada nova renderização. Por exemplo, isso pode acontecer se a área de texto ou um de seus pais sempre receber um atributo `key` diferente, ou se você aninhar definições de componentes (o que não é permitido no React e faz com que o componente "interno" seja remontado a cada renderização).
...
Se você fornecer um `value` ao componente, ele deve permanecer uma string ao longo de sua vida útil.

Você não pode passar `value={undefined}` primeiro e depois passar `value="some string"` porque o React não saberá se você deseja que o componente seja não controlado ou controlado. Um componente controlado sempre deve receber um `value` de string, não `null` ou `undefined`.
