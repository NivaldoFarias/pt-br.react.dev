...
O [componente `<option>` do navegador integrado](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option) permite que você renderize uma opção dentro de uma caixa [`<select>`](/reference/react-dom/components/select).
...
*   [`disabled`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/option#disabled): Um booleano. Se for `true`, a opção não poderá ser selecionada e aparecerá esmaecida.
