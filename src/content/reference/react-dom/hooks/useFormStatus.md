...
`useFormStatus` é um Hook que fornece informações de status da última submissão de formulário.
...
Para obter informações de status, o componente `Submit` deve ser renderizado dentro de um `<form>`. O Hook retorna informações como a propriedade <CodeStep step={1}>`pending`</CodeStep> que informa se o formulário está enviando ativamente.

No exemplo acima, `Submit` usa essas informações para desabilitar os cliques no `<button>` enquanto o formulário está sendo enviado.
...
*   `data`: Um objeto que implementa a [`interface FormData`](https://developer.mozilla.org/en-US/docs/Web/API/FormData) que contém os dados que o `<form>` pai está enviando. Se não houver uma submissão ativa ou nenhum `<form>` pai, será `null`.

*   `method`: Um valor string de `'get'` ou `'post'`. Isso representa se o `<form>` pai está enviando com um [`GET`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods) ou `POST` [HTTP method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods). Por padrão, um `<form>` usará o método `GET` e pode ser especificado pela propriedade [`method`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form#method).

    [//]: # (Link to `<form>` documentation. "Read more on the `action` prop on `<form>`.")
*   `action`: Uma referência à função passada para a prop `action` no `<form>` pai. Se não houver um `<form>` pai, a propriedade será `null`. Se houver um valor URI fornecido para a prop `action`, ou nenhuma prop `action` especificada, `status.action` será `null`.
...
Para exibir um estado pendente enquanto um formulário está sendo enviado, você pode chamar o Hook `useFormStatus` em um componente renderizado em um `<form>` e ler a propriedade `pending` retornada.
...
O Hook `useFormStatus` retorna apenas informações de status para um `<form>` pai e não para nenhum `<form>` renderizado no mesmo componente que chama o Hook, ou componentes filhos.
...
`useFormStatus` não rastreará o status de um `<form>` renderizado no mesmo componente. Veja a [Ressalva](#useformstatus-will-not-return-status-information-for-a-form-rendered-in-the-same-component) para obter mais detalhes.
```