...
`renderToString` não oferece suporte a streaming ou espera por dados. [Veja as alternativas.](#alternatives)
...
// A sintaxe do manipulador da rota depende da sua estrutura de backend
...
`renderToString` retorna uma `string` imediatamente, portanto, não oferece suporte ao streaming de conteúdo conforme ele carrega.

Quando possível, recomendamos usar estas alternativas com todos os recursos:
...
`renderToString` retorna uma `string` imediatamente, portanto, não oferece suporte à espera do carregamento de dados para geração de HTML estático.

Recomendamos usar estas alternativas com todos os recursos:
...
Importar `react-dom/server` **no cliente** aumenta desnecessariamente o tamanho do seu bundle e deve ser evitado. Se você precisar renderizar algum componente em HTML no navegador, use [`createRoot`](/reference/react-dom/client/createRoot) e leia o HTML do DOM:
...
Se algum componente suspender (por exemplo, porque está definido com [`lazy`](/reference/react/lazy) ou busca dados), `renderToString` não esperará que seu conteúdo seja resolvido. Em vez disso, `renderToString` encontrará o [`<Suspense>`](/reference/react/Suspense) mais próximo acima dele e renderizará sua prop `fallback` no HTML. O conteúdo não aparecerá até que o código do cliente seja carregado.

Para resolver isso, use uma das [soluções de streaming recomendadas.](#alternatives) Para renderização no lado do servidor, elas podem transmitir conteúdo em pedaços conforme ele é resolvido no servidor, de modo que o usuário veja a página sendo progressivamente preenchida antes que o código do cliente seja carregado. Para geração de site estático, elas podem esperar que todo o conteúdo seja resolvido antes de gerar o HTML estático.
```