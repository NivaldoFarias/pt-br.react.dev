...
`renderToPipeableStream` renderiza uma árvore React para um [Node.js Stream](https://nodejs.org/api/stream.html) com suporte a pipe.
...
Esta API é específica para Node.js. Ambientes com [Web Streams,](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) como Deno e runtimes de borda modernos, devem usar [`renderToReadableStream`](/reference/react-dom/server/renderToReadableStream) em vez disso.
...
* **opcional** `options`: Um objeto com opções de streaming.
  ...
  * **optional** `identifierPrefix`: Um prefixo de string que o React usa para IDs gerados por [`useId`.](/reference/react/useId) Útil para evitar conflitos ao usar vários roots na mesma página. Deve ser o mesmo prefixo passado para [`hydrateRoot`.](/reference/react-dom/client/hydrateRoot#parameters)
  ...
  * **opcional** `onAllReady`: Um callback que dispara quando toda a renderização estiver completa, incluindo o [shell](#specifying-what-goes-into-the-shell) e todo o [conteúdo](#streaming-more-content-as-it-loads) adicional. Você pode usar isso em vez de `onShellReady` [para rastreadores e geração estática.](#waiting-for-all-content-to-load-for-crawlers-and-static-generation) Se você começar o streaming aqui, não obterá nenhum carregamento progressivo. O stream conterá o HTML final.
  * **opcional** `onError`: Um callback que dispara sempre que há um erro do servidor, seja [recuperável](#recovering-from-errors-outside-the-shell) ou [não.](#recovering-from-errors-inside-the-shell) Por padrão, isso apenas chama `console.error`. Se você substituí-lo para [registrar relatórios de falhas,](#logging-crashes-on-the-server) certifique-se de ainda chamar `console.error`. Você também pode usá-lo para [ajustar o código de status](#setting-the-status-code) antes que o shell seja emitido.
  * **opcional** `onShellReady`: Um callback que dispara logo após a [shell inicial](#specifying-what-goes-into-the-shell) ter sido renderizada. Você pode [definir o código de status](#setting-the-status-code) e chamar `pipe` aqui para iniciar o streaming. React irá [transmitir o conteúdo adicional](#streaming-more-content-as-it-loads) após o shell, juntamente com as tags `<script>` embutidas que substituem os fallbacks de carregamento HTML pelo conteúdo.
  * **opcional** `onShellError`: Um callback que dispara se houve um erro ao renderizar o shell inicial. Ele recebe o erro como um argumento. Nenhum byte foi emitido do stream ainda, e nem `onShellReady` nem `onAllReady` serão chamados, então você pode [gerar um fallback HTML shell.](#recovering-from-errors-inside-the-shell)
...
* `pipe` gera o HTML no [Writable Node.js Stream.](https://nodejs.org/api/stream.html#writable-streams) Chame `pipe` em `onShellReady` se você quiser habilitar o streaming, ou em `onAllReady` para rastreadores e geração estática.
...
React irá injetar o [doctype](https://developer.mozilla.org/en-US/docs/Glossary/Doctype) e suas <CodeStep step={2}>tags de `<script>` de bootstrap</CodeStep> no stream HTML resultante:
...
Os URLs finais de ativos (como arquivos JavaScript e CSS) costumam ser hash após a build. Por exemplo, em vez de `styles.css`, você pode acabar com `styles.123456.css`. O hash de nomes de arquivos de ativos estáticos garante que cada build distinto do mesmo ativo terá um nome de arquivo diferente. Isso é útil porque permite que você habilite o caching de longo prazo para ativos estáticos com segurança: um arquivo com um determinado nome nunca mudaria de conteúdo.

No entanto, se você não souber os URLs dos ativos até depois da build, não haverá como colocá-los no código-fonte. Por exemplo, codificar `"/styles.css"` no JSX como antes não funcionaria. Para mantê-los fora do seu código-fonte, seu componente raiz pode ler os nomes de arquivos reais de um mapa passado como uma prop:
...
O streaming permite que o usuário comece a ver o conteúdo mesmo antes que todos os dados tenham sido carregados no servidor. Por exemplo, considere uma página de perfil que mostra uma capa, uma barra lateral com amigos e fotos e uma lista de postagens:
...
Imagine que carregar dados para `<Posts />` leva algum tempo. Idealmente, você gostaria de mostrar o restante do conteúdo da página de perfil para o usuário sem esperar pelas postagens. Para fazer isso, [envolva `Posts` em um limite de `<Suspense>`:](/reference/react/Suspense#displaying-a-fallback-while-content-is-loading)
...
Isso diz a React para começar a transmitir o HTML antes que `Posts` carregue seus dados. React enviará o HTML para o fallback de carregamento (`PostsGlimmer`) primeiro e, em seguida, quando `Posts` terminar de carregar seus dados, React enviará o HTML restante junto com uma tag `<script>` embutida que substitui o fallback de carregamento por esse HTML. Da perspectiva do usuário, a página aparecerá primeiro com o `PostsGlimmer`, depois substituída por `Posts`.

Você pode ainda [aninhar limites de `<Suspense>`s](/reference/react/Suspense#reveling-nested-content-as-it-loads) para criar uma sequência de carregamento mais granular:
...
Suspense **não** detecta quando os dados são buscados dentro de um Effect ou event handler.

A maneira exata como você carregaria dados no componente `Posts` acima depende da sua estrutura. Se você usar uma estrutura habilitada para Suspense, encontrará os detalhes em sua documentação de recuperação de dados.

A busca de dados habilitada para Suspense sem o uso de uma estrutura com opinião não é suportada. Os requisitos para implementar uma fonte de dados habilitada para Suspense são instáveis ​​e não documentados. Uma API oficial para integrar fontes de dados com Suspense será lançada em uma versão futura do React.
...
A parte do seu aplicativo fora dos limites do `<Suspense>` é chamada de *shell:*
...
Se você envolver todo o aplicativo em um limite de `<Suspense>` na raiz, o shell conterá apenas esse spinner. No entanto, essa não é uma experiência de usuário agradável, pois ver um spinner grande na tela pode parecer mais lento e irritante do que esperar um pouco mais e ver o layout real. É por isso que geralmente você vai querer colocar os limites de `<Suspense>` para que o shell pareça *mínimo, mas completo*--como um esqueleto de todo o layout da página.

O callback `onShellReady` dispara quando todo o shell foi renderizado. Normalmente, você começará a transmitir então:
...
No momento em que `onShellReady` dispara, os componentes nos limites `<Suspense>` aninhados podem ainda estar carregando dados.
...
Se você fornecer uma implementação `onError` personalizada, não se esqueça de registrar também erros no console como acima.
...
Se houver um erro ao gerar o shell, tanto `onError` quanto `onShellError` dispararão. Use `onError` para relatórios de erros e use `onShellError` para enviar o documento HTML de fallback. Seu HTML de fallback não precisa ser uma página de erro. Em vez disso, você pode incluir um shell alternativo que renderiza seu aplicativo apenas no cliente.
...
Se um erro acontecer no componente `Posts` ou em algum lugar dentro dele, o React [tentará se recuperar disso:](/reference/react/Suspense#providing-a-fallback-for-server-errors-and-client-only-content)

1. Ele emitirá o fallback de carregamento para o limite de `<Suspense>` mais próximo (`PostsGlimmer`) no HTML.
2. Ele irá "desistir" de tentar renderizar o conteúdo `Posts` no servidor.
...
Se a tentativa de renderizar `Posts` no cliente for bem-sucedida, o fallback de carregamento do servidor será substituído pela saída de renderização do cliente. O usuário não saberá que houve um erro no servidor. No entanto, o callback `onError` do servidor e os callbacks [`onRecoverableError`](/reference/react-dom/client/hydrateRoot#hydrateroot) do cliente serão disparados para que você possa ser notificado sobre o erro.

---

### Definindo o código de status {/*setting-the-status-code*/}

O streaming introduz um tradeoff. Você quer começar a transmitir a página o mais rápido possível para que o usuário possa ver o conteúdo mais cedo. No entanto, assim que você começar a transmitir, não poderá mais definir o código de status da resposta.

Ao [dividir seu aplicativo](#specifying-what-goes-into-the-shell) no shell (acima de todos os limites de `<Suspense>`) e no restante do conteúdo, você já resolveu parte desse problema. Se o shell tiver erros, você receberá o callback `onShellError`, que permite definir o código de status do erro. Caso contrário, você sabe que o aplicativo pode se recuperar no cliente, então você pode enviar "OK".
...
Se um componente *fora* do shell (ou seja, dentro de um limite de `<Suspense>`) lançar um erro, o React não interromperá a renderização. Isso significa que o callback `onError` será disparado, mas você ainda receberá `onShellReady` em vez de `onShellError`. Isso ocorre porque o React tentará se recuperar desse erro no cliente, [conforme descrito acima.](#recovering-from-errors-outside-the-shell)
...
Isso só irá detectar erros fora do shell que aconteceram ao gerar o conteúdo inicial do shell, então não é exaustivo. Se saber se um erro ocorreu para determinado conteúdo for fundamental, você pode movê-lo para o shell.
...
Lembre-se que, assim que você emitir o shell e começar o streaming, não poderá alterar o código de status.
...
O streaming oferece uma melhor experiência do usuário porque o usuário pode ver o conteúdo conforme ele se torna disponível.

No entanto, quando um rastreador visita sua página ou se você estiver gerando as páginas no momento da build, convém deixar todo o conteúdo carregar primeiro e, em seguida, produzir a saída HTML final em vez de revelá-la progressivamente.

Você pode esperar que todo o conteúdo seja carregado usando o callback `onAllReady`:
...
Um visitante comum receberá um stream de conteúdo carregado progressivamente. Um rastreador receberá a saída HTML final após o carregamento de todos os dados. No entanto, isso também significa que o rastreador terá que esperar por *todos* os dados, alguns dos quais podem ser lentos para carregar ou com erros. Dependendo do seu aplicativo, você pode optar por enviar o shell para os rastreadores também.
