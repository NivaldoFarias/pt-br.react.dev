...
`preloadModule` te permite buscar ansiosamente um módulo ESM que você espera usar.
...
Para fazer o pré-carregamento de um módulo ESM, chame a função `preloadModule` de `react-dom`.
...
*   Na renderização do lado do servidor ou ao renderizar Server Components, `preloadModule` só tem efeito se você chamá-lo enquanto estiver renderizando um componente ou em um contexto assíncrono originando-se da renderização de um componente. Quaisquer outras chamadas serão ignoradas.
