...
[Frameworks baseados em React](/learn/start-a-new-react-project) frequentemente lidam com o carregamento de recursos para você, então talvez você não precise chamar esta API você mesmo. Consulte a documentação do seu framework para mais detalhes.

</Note>

<Intro>

`preloadModule` permite que você busque ansiosamente um módulo ESM que você espera usar.
...
A função `preloadModule` fornece ao navegador uma dica de que ele deve começar a baixar o módulo fornecido, o que pode economizar tempo.
...
* `options`: um objeto. Ele contém as seguintes propriedades:
  ...
  *  `integrity`: a string. Um hash criptográfico do módulo, para [verificar sua autenticidade](https://developer.mozilla.org/pt-BR/docs/Web/Security/Subresource_Integrity).
...
* Múltiplas chamadas para `preloadModule` com o mesmo `href` têm o mesmo efeito de uma única chamada.
...
* Na renderização do lado do servidor ou ao renderizar Server Components, `preloadModule` só tem efeito se você chamá-lo ao renderizar um componente ou em um contexto assíncrono originado da renderização de um componente. Quaisquer outras chamadas serão ignoradas.
...
Se você deseja que o navegador comece a executar o módulo imediatamente (em vez de apenas baixá-lo), use [`preinitModule`](/reference/react-dom/preinitModule) em vez disso. Se você deseja carregar um script que não é um módulo ESM, use [`preload`](/reference/react-dom/preload).
