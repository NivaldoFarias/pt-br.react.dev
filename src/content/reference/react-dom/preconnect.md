...
`preconnect` permite que você se conecte antecipadamente a um servidor do qual você espera carregar recursos.
...
Para se preconnectar a um host, chame a função `preconnect` de `react-dom`.
...
* Várias chamadas para `preconnect` com o mesmo servidor têm o mesmo efeito que uma única chamada.
...
* Na renderização do lado do servidor ou ao renderizar Componentes de Servidor, `preconnect` só tem efeito se você o chamar ao renderizar um componente ou em um contexto assíncrono originário da renderização de um componente. Quaisquer outras chamadas serão ignoradas.
...
* Não há nenhum benefício em se preconnectar ao mesmo servidor do qual a própria página da web é hospedada, porque ela já foi conectada quando a dica seria dada.

---

## Uso {/*usage*/}

### Preconnecting ao renderizar {/*preconnecting-when-rendering*/}
