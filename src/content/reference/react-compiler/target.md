...
- **`'19'`**: Destina-se ao React 19 (padrão). Nenhuma runtime adicional é necessária.
...
```js
// Saída compilada usa APIs nativas do React 19
...
```js
// Saída compilada usa o runtime de polyfill
import { c as _c } from 'react-compiler-runtime';
```
...
Certifique-se de que o pacote de runtime:

1. Esteja instalado em seu projeto (não globalmente)
2. Esteja listado nas dependências do seu `package.json`
3. Seja a versão correta (tag `@rc`)
4. Não esteja em `devDependencies` (ele é necessário em tempo de execução)

### Verificando a saída compilada {/*checking-output*/}

Para verificar se o runtime correto está sendo usado, observe a importação diferente (`react/compiler-runtime` para o integrado, pacote `react-compiler-runtime` autônomo para 17/18):
...
// Para React 17/18 (runtime de polyfill)
import { c } from 'react-compiler-runtime'
//                      ^
```