...
Quando uma função contém `"use memo"`, o React Compiler a analisará e otimizará em tempo de build. O compilador automaticamente memorizará valores e componentes para prevenir recomputações e re-renderizações desnecessárias.
...
Em um aplicativo React que usa o React Compiler, as funções são analisadas em tempo de build para determinar se podem ser otimizadas. Por padrão, o compilador infere automaticamente quais componentes memorizar, mas isso pode depender da sua configuração de [`compilationMode`](/reference/react-compiler/compilationMode), se você a definiu.
