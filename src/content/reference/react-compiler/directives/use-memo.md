...
Quando uma função contém `"use memo"`, o React Compiler a analisará e otimizará durante o tempo de compilação (build time). O compilador automaticamente memorizará valores e componentes para prevenir recomputações e re-renderizações desnecessárias.
...
```js
// Comece otimizando componentes folha (leaf components)
