...
|-----------|---------|-------------|
| [`"use memo"`](/reference/react-compiler/directives/use-memo) | Forçar compilação | Ao usar o modo `annotation` ou para substituir as heurísticas do modo `infer` |
| [`"use no memo"`](/reference/react-compiler/directives/use-no-memo) | Prevenir compilação | Depurando problemas ou trabalhando com código incompatível |
...
Diretivas são "escape hatches". Prefira configurar o compilador no nível do projeto:
...
function DataGrid() {
  "use no memo"; // TODO: Remover após corrigir o problema com alturas de linha dinâmicas (JIRA-123)
...
Diretivas de opt-out devem ser temporárias:
