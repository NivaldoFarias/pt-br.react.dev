...
Definir uma variável de estado irá enfileirar outra renderização. Mas às vezes você pode querer realizar várias operações sobre o valor antes de enfileirar a próxima renderização. Para fazer isso, é útil entender como o React agrupa as atualizações de estado.
...
Mas há um outro fator em jogo aqui. **O React espera até que *todo* o código nos manipuladores de eventos tenha sido executado antes de processar suas atualizações de estado.** É por isso que a nova renderização só acontece *depois* de todas essas chamadas `setNumber()`.

Isso pode lembrar você de um garçom fazendo um pedido no restaurante. Um garçom não corre para a cozinha ao mencionar seu primeiro prato! Em vez disso, eles deixam você terminar seu pedido, permitem que você faça alterações nele e até mesmo aceitam pedidos de outras pessoas na mesa.
...
Isso permite que você atualize várias variáveis de estado -- mesmo de vários componentes -- sem acionar muitas [re-renderizações.](/learn/render-and-commit#re-renders-when-state-updates) Mas isso também significa que a UI não será atualizada até _depois_ que seu manipulador de eventos, e qualquer código nele, for concluído. Este comportamento, também conhecido como **agrupamento,** faz com que seu aplicativo React seja executado muito mais rápido. Ele também evita lidar com renderizações confusas "meio-acabadas" onde apenas algumas das variáveis foram atualizadas.

**O React não agrupa em *múltiplos* eventos intencionais como cliques** -- cada clique é tratado separadamente. Tenha certeza de que o React só faz agrupamento quando geralmente é seguro fazê-lo. Isso garante que, por exemplo, se o primeiro clique no botão desativar um formulário, o segundo clique não o enviaria novamente.
...
1. O React enfileira essa função para ser processada depois que todo o outro código no manipulador de eventos for executado.
2. Durante a próxima renderização, o React percorre a fila e lhe dá o estado final atualizado.
...
1. `setNumber(n => n + 1)`: `n => n + 1` é uma função. React adiciona à fila.
1. `setNumber(n => n + 1)`: `n => n + 1` é uma função. React adiciona à fila.
1. `setNumber(n => n + 1)`: `n => n + 1` é uma função. React adiciona à fila.
...
Veja como o React trabalha com essas linhas de código enquanto executa este manipulador de eventos:
...
* O React processa as atualizações de estado depois que os manipuladores de eventos terminam de ser executados. Isso é chamado de agrupamento.
* Para atualizar algum estado várias vezes em um evento, você pode usar a função de atualização `setNumber(n => n + 1)`.
...
Você está trabalhando em um aplicativo de mercado de arte que permite ao usuário enviar vários pedidos de um item de arte ao mesmo tempo. Toda vez que o usuário pressionar o botão "Comprar", o contador "Pendente" deverá aumentar em um. Após três segundos, o contador "Pendente" deverá diminuir e o contador "Concluído" deverá aumentar.

No entanto, o contador "Pendente" não se comporta como pretendido. Quando você pressiona "Comprar", ele diminui para `-1` (o que não deveria ser possível!). E se você clicar rapidamente duas vezes, ambos os contadores parecem se comportar de forma imprevisível.
...
Isso garante que, ao incrementar ou decrementar um contador, você faça isso em relação ao seu *último* estado, em vez de qual era o estado no momento do clique.

</Solution>

#### Implementar a fila de estado você mesmo {/*implement-the-state-queue-yourself*/}

Neste desafio, você reimplementará uma pequena parte do React do zero! Não é tão difícil quanto parece.

Percorra a visualização do sandbox. Observe que ela mostra **quatro casos de teste.** Eles correspondem aos exemplos que você viu anteriormente nesta página. Sua tarefa é implementar a função `getFinalState` para que ela retorne o resultado correto para cada um desses casos. Se você implementá-la corretamente, todos os quatro testes deverão passar.

Você receberá dois argumentos: `baseState` é o estado inicial (como `0`), e a `queue` é uma array que contém uma mistura de números (como `5`) e funções de atualização (como `n => n + 1`) na ordem em que foram adicionados.
