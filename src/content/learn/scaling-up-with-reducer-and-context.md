...
In larger apps, the `TaskApp` component might become too big. You'll need to keep your codebase organized, so it would be useful to move your `reducer` and context definitions to a separate file:

```js
// In a separate file (e.g. 'tasks.js'):
...
Your `TaskApp` component would then import these:
...
This way, all of the `reducer` and context-related logic stays in one place. This makes it easier to read and update!
```js {3,9-13}
export default function AddTask() {
  const [text, setText] = useState('');
  const dispatch = useContext(TasksDispatchContext);
...
Você não precisa fazer isso, mas você pode remover ainda mais os componentes movendo tanto o reducer quanto o contexto para um único arquivo. Atualmente, `TasksContext.js` contém apenas duas declarações de contexto:
...
Este arquivo está prestes a ficar lotado! Você moverá o reducer para o mesmo arquivo. Então você declarará um novo componente `TasksProvider` no mesmo arquivo. Este componente amarrará todas as peças:
...
Funções como `useTasks` e `useTasksDispatch` são chamadas de *[Hooks personalizados](/learn/reusing-logic-with-custom-hooks)*. Sua função é considerada um Hook personalizado se seu nome começar com `use`. Isso permite que você use outros Hooks, como `useContext`, dentro dela.
...
- Para fornecer o estado e a função de envio para os componentes abaixo:
  1. Crie dois contextos (para o estado e para as funções de envio).
  2. Forneça ambos os contextos do componente que usa o reducer.
  3. Use qualquer contexto de componentes que precisam lê-los.
- Você pode limpar ainda mais os componentes movendo toda a fiação para um arquivo.
