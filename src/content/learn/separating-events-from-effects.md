...
function Page({ url }) {
  useEffect(() => {
    logVisit(url);
  }, []); // 🔴 React Hook useEffect tem uma dependência faltando: 'url'
  // ...
}
```
...
function Page({ url }) {
  ...
  }, [url]); // 🔴 React Hook useEffect tem uma dependência faltando: 'numberOfItems'
  // ...
}
```
...
Como seu Effect Event explicitamente "pede" a `visitedUrl`, agora você não pode acidentalmente remover `url` das dependências do Effect. Se você remover a dependência `url` (fazendo com que visitas distintas à página sejam contadas como uma), o linter o alertará sobre isso. Você quer que `onVisit` seja reativo em relação à `url`, então, em vez de ler a `url` internamente (onde não seria reativo), você a passa *do* seu Effect.
...
#### Corrija um contador congelado {/*fix-a-freezing-counter*/}
...
O código dentro de Eventos de Efeito não é reativo. Existem casos em que você _gostaria_ que a chamada `setInterval` fosse reexecutada?
