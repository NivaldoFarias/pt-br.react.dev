...
Mais tarde, você adiciona várias rotas ao seu site. Agora seu componente `Page` recebe uma prop `url` com o caminho atual. Você deseja passar a `url` como parte de sua chamada `logVisit`, mas o linter de dependências reclama:

```js {1,3}
function Page({ url }) {
  useEffect(() => {
    logVisit(url);
  }, []); // 🔴 O Hook useEffect do React tem uma dependência ausente: 'url'
  // ...
}
```
...
function Page({ url }) {
  ...
  }, [url]); // 🔴 O Hook useEffect do React tem uma dependência ausente: 'numberOfItems'
  // ...
}
```
...
O código dentro de Effect Events não é reativo. Existem casos em que você _quereria_ que a chamada `setInterval` fosse executada novamente?
