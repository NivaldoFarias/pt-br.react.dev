...
Toda vez que você ajusta as dependências de um Effect para refletir o código, examine a lista de dependências. Faz sentido para o Effect ser reexecutado quando qualquer uma dessas dependências mudar? Às vezes, a resposta é "não":
...
Para encontrar a solução correta, você precisará responder a algumas perguntas sobre o seu Effect. Vamos percorrê-las.

### Este código deveria ser movido para um manipulador de eventos? {/*should-this-code-move-to-an-event-handler*/}

A primeira coisa em que você deve pensar é se este código deveria ser um Effect.
...
Ele usa a variável `messages` para [criar um novo array](/learn/updating-arrays-in-state) começando com todas as mensagens existentes e adiciona a nova mensagem ao final. No entanto, como `messages` é um valor reativo lido por um Effect, ele deve ser uma dependência:
...
**Note como seu Effect não lê mais a variável `messages` de forma alguma.** Você só precisa passar uma função atualizadora como `msgs => [...msgs, receivedMessage]`. O React [coloca sua função atualizadora em uma fila](/learn/queueing-a-series-of-state-updates) e a fornecerá como argumento `msgs` durante a próxima renderização. É por isso que o Effect em si não precisa mais depender de `messages`. Como resultado dessa correção, receber uma mensagem de chat não fará mais com que o chat se reconecte.
...
Como `onReceiveMessage` é uma dependência, isso faria com que o Effect ressincronizasse após cada renderização pai. Isso faria com que ele reconectasse ao chat. Para resolver isso, encapsule a chamada em um Effect Event:
...
**Dependências de objetos e funções podem fazer com que seu Effect se re-sincronize com mais frequência do que o necessário.**
...
Como `createOptions` é declarado fora do seu componente, ele não é um valor reativo. É por isso que ele não precisa ser especificado nas dependências do seu Effect, e por que ele nunca fará com que seu Effect se re-sincronize.

#### Mova objetos e funções dinâmicos para dentro do seu Effect {/*move-dynamic-objects-and-functions-inside-your-effect*/}

Se o seu objeto depende de algum valor reativo que pode mudar como resultado de uma re-renderização, como uma prop `roomId`, você não pode movê-lo *para fora* do seu componente. Você pode, no entanto, mover sua criação *para dentro* do código do seu Effect:
...
Isso só funciona para funções [puras](/learn/keeping-components-pure) porque elas são seguras para serem chamadas durante a renderização. Se sua função for um manipulador de eventos, mas você não quiser que suas mudanças re-sincronizem seu Effect, [envolva-a em um Effect Event em vez disso.](#do-you-want-to-read-a-value-without-reacting-to-its-changes)
...
- Se partes diferentes do seu Effect devem ser re-executadas por diferentes motivos, divida-o em vários Effects.
...
Effect Events como `onAppear` não são reativos, então você pode ler `duration` internamente sem reativar a animação.
...
#### Corrija um chat que reconecta, novamente {/*fix-a-reconnecting-chat-again*/}
