...
function ChatRoom({ roomId }) {
  useEffect(() => {
    ...
    connection.connect();
    return () => connection.disconnect();
  }, []); // 🔴 React Hook useEffect tem uma dependência faltando: 'roomId'
  // ...
}
```
...
Vamos dizer que você queria executar o Efeito "apenas na montagem". Você leu que [dependências vazias (`[]`)](/learn/lifecycle-of-reactive-effects#what-an-effect-with-empty-dependencies-means) fazem isso, então você decidiu ignorar o linter e forçou `[]` como dependências.
...
Toda vez que você ajustar as dependências de um Effect para refletir o código, olhe para a lista de dependências. Faz sentido para o Effect ser executado novamente quando qualquer uma dessas dependências mudar? Às vezes, a resposta é "não":
...
Agora o primeiro Effect só é executado novamente se o `country` mudar, enquanto o segundo Effect é executado novamente quando o `city` muda. Você os separou por propósito: duas coisas diferentes são sincronizadas por dois Effects separados. Dois Effects separados têm duas listas de dependências separadas, então eles não se acionarão mutuamente não intencionalmente.
...
Toda vez que você recebe uma mensagem, `setMessages()` faz com que o componente seja renderizado novamente com um novo array `messages` que inclui a mensagem recebida. No entanto, como este Effect agora depende de `messages`, isso também *ressincronizará* o Effect. Assim, cada nova mensagem fará com que o chat se reconecte. O usuário não gostaria disso!
...
O problema é que toda vez que `isMuted` muda (por exemplo, quando o usuário pressiona o botão "Silenciar"), o Effect resincroniza e reconecta ao chat. Esta não é a experiência do usuário desejada! (Neste exemplo, mesmo desabilitar o linter não funcionaria - se você fizer isso, `isMuted` ficaria "preso" com seu valor antigo.)
...
Como `onReceiveMessage` é uma dependência, isso faria com que o Effect resincronizasse após cada renderização pai. Isso faria com que ele reconectasse ao chat. Para resolver isso, encapsule a chamada em um Effect Event:
...
Se o objeto não depender de nenhum prop e estado, você pode mover esse objeto para fora do seu componente:
...
Dessa forma, você *prova* ao linter que ele não é reativo. Ele não pode mudar como resultado de uma nova renderização, então não precisa ser uma dependência. Agora, a nova renderização de `ChatRoom` não fará com que seu Effect resincronize.
...
Se o seu objeto depender de algum valor reativo que possa mudar como resultado de uma nova renderização, como um prop `roomId`, você não pode movê-lo *para fora* do seu componente. Você pode, no entanto, mover sua criação *para dentro* do código do seu Effect:
...
Para evitar torná-la uma dependência (e fazer com que ela se reconecte em novas renderizações), chame-a fora do Effect. Isso fornece os valores `roomId` e `serverUrl` que não são objetos, e que você pode ler de dentro do seu Effect:
