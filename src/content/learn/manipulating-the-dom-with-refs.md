...
- Como acessar nós DOM de outro componente
...
Quando o Strict Mode está habilitado, os callbacks de ref serão executados duas vezes em desenvolvimento.
...
<Pitfall>
Refs são uma saída de emergência. Manipular manualmente nós DOM de *outro* componente pode tornar seu código frágil.
</Pitfall>
...
No exemplo acima, uma ref é criada no componente pai, `MyForm`, e passada para o componente filho, `MyInput`. `MyInput` então passa a ref para `<input>`. Como `<input>` é um [componente integrado](/reference/react-dom/components/common), o React define a propriedade `.current` da ref para o elemento DOM `<input>`.
...
* Durante a **renderização,** o React chama seus componentes para determinar o que deve aparecer na tela.
...
**Normalmente, você acessará refs a partir de manipuladores de eventos.** Se você quiser fazer algo com uma ref, mas não houver um evento específico para isso, você pode precisar de um Effect. Discutiremos os Effects nas próximas páginas.
...
Considere um código como este, que adiciona um novo item à lista de tarefas e rola a tela para o último filho da lista. Observe como, por algum motivo, ele sempre rola para o item que estava *imediatamente antes* do último adicionado:
...
No React, [as atualizações de estado são enfileiradas.](/learn/queueing-a-state-updates) Geralmente, é isso que você quer. No entanto, aqui isso causa um problema porque `setTodos` não atualiza o DOM imediatamente. Portanto, no momento em que você rola a lista para seu último elemento, o item da lista ainda não foi adicionado. É por isso que a rolagem sempre "atrasa" em um item.
...
Isso instruirá o React a atualizar o DOM de forma síncrona logo após a execução do código envolvido em `flushSync`. Como resultado, o último item da lista já estará no DOM quando você tentar rolar para ele:
...
Depois de remover manualmente o elemento DOM, tentar usar `setState` para exibi-lo novamente levará a um erro. Isso ocorre porque você alterou o DOM, e o React não sabe como continuar gerenciando-o corretamente.
...
No entanto, isso não significa que você não possa fazer isso. Requer cautela. **Você pode modificar com segurança partes do DOM que o React não tem _motivo_ para atualizar.** Por exemplo, se um `<div>` estiver sempre vazio no JSX, o React não terá um motivo para tocar em sua lista de filhos. Portanto, é seguro adicionar ou remover elementos manualmente lá.
...
#### Rolar um carrossel de imagens {/*scrolling-an-image-carousel*/}
...
Você precisará passar `ref` como uma prop para expor um nó DOM do seu próprio componente, como `SearchInput`.
