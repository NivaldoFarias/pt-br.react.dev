...
Isso ocorre porque **Hooks só devem ser chamados no nível superior do seu componente.** Você não pode chamar `useRef` em um loop, em uma condição ou dentro de uma chamada `map()`.
...
No React, [as atualizações de estado são enfileiradas.](/learn/queueing-a-series-of-state-updates) Geralmente, é isso que você quer. No entanto, aqui isso causa um problema porque `setTodos` não atualiza o DOM imediatamente. Portanto, no momento em que você rola a lista para seu último elemento, o item da lista ainda não foi adicionado. É por isso que a rolagem sempre "atrasa" em um item.
...
Isso instruirá o React a atualizar o DOM de forma síncrona logo após a execução do código envolvido por `flushSync`. Como resultado, o último item da lista já estará no DOM quando você tentar rolar para ele:
...
Note que a chamada `flushSync` é necessária para forçar o React a atualizar o DOM antes de rolar. Caso contrário, `selectedRef.current` sempre apontaria para o item selecionado anteriormente.
