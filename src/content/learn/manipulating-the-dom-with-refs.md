...
Neste exemplo, `itemsRef` não contém um único nó DOM. Em vez disso, ele contém um [Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) do ID do item para um nó DOM. ([Refs podem conter quaisquer valores!](/learn/referencing-values-with-refs)) O [`ref` callback](/reference/react-dom/components/common#ref-callback) em cada item da lista cuida de atualizar o Map:
...
<Pitfall>
Refs são uma "escape hatch" (solução de último recurso). Manipular manualmente nós DOM de *outro* componente pode tornar seu código frágil.
</Pitfall>
...
Refs são uma saída de emergência. Você só deve usá-las quando precisar "sair do React". Exemplos comuns incluem gerenciar foco, posição de rolagem ou chamar APIs do navegador que o React não expõe.
...
**Evite alterar nós DOM gerenciados pelo React.** Modificar, adicionar filhos ou remover filhos de elementos que são gerenciados pelo React pode levar a resultados visuais inconsistentes ou falhas como a acima.
...
Você precisará adicionar uma prop `onClick` ao `SearchButton`, e fazer com que o `SearchButton` a passe para o `<button>` do navegador. Você também passará um ref para o `<SearchInput>`, que o encaminhará para o `<input>` real e o preencherá. Finalmente, no manipulador de clique, você chamará `focus` no nó DOM armazenado dentro desse ref.
