...
Em CSS, propriedades diferentes como `color` e `background-color` não se substituem. Você pode definir a `color` de todos os `<div>`s como vermelho sem impactar `background-color`. Da mesma forma, **diferentes contextos React não se substituem.** Cada contexto que você cria com `createContext()` é completamente separado dos outros, e une componentes que usam e fornecem *esse contexto específico*. Um componente pode usar ou fornecer muitos contextos diferentes sem problemas.
...
* Contexto permite que você escreva componentes que "se adaptam ao seu entorno".
...
Crie e exporte `ImageSizeContext` de `Context.js`. Em seguida, envolva a Lista em `<ImageSizeContext value={imageSize}>` para passar o valor para baixo e `useContext(ImageSizeContext)` para lê-lo em `PlaceImage`:
