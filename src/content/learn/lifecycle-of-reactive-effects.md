...
Para **parar a sincronização,** o React chamará a função de limpeza que seu Effect retornou após conectar à sala `"general"`. Como `roomId` era `"general"`, a função de limpeza desconecta da sala `"general"`:
...
A principal razão pela qual seu Effect resincronizará na prática é se algum dado que ele usa mudou. Na sandbox acima, altere a sala de chat selecionada. Observe como, quando `roomId` muda, seu Effect resincroniza.
...
function ChatRoom({ roomId, selectedServerUrl }) { // roomId é reativo
  ...
  }, [roomId, serverUrl]); // Então ele precisa ressincronizar quando qualquer um deles mudar!
  // ...
}
```
...
Se você vir uma regra de linter sendo suprimida, remova a supressão! É aí que os erros geralmente se encontram.
