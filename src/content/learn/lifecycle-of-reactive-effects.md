...
Imagine que este componente `ChatRoom` recebe uma prop `roomId` que o usuário escolhe em um dropdown. Vamos dizer que inicialmente o usuário escolhe a sala `"general"` como `roomId`. Seu aplicativo exibe o chat da sala `"general"`:
...
Após a interface do usuário ser exibida, o React executará seu Effect para **iniciar a sincronização.** Ele se conecta à sala `"general"`:
...
Pense no que deve acontecer a seguir. O usuário vê que `"travel"` é a sala de chat selecionada na interface. No entanto, o Effect que foi executado da última vez ainda está conectado à sala `"general"`. **A prop `roomId` mudou, então o que seu Effect fez naquela época (conectar à sala `"general"`) não corresponde mais à interface do usuário.**
...
Graças a isso, você agora está conectado à mesma sala que o usuário escolheu na interface. Desastre evitado!
...
A principal razão pela qual seu Effect resincronizará na prática é se algum dado que ele usa mudou. Na sandbox acima, altere a sala de chat selecionada. Observe como, quando o `roomId` muda, seu Effect resincroniza.

No entanto, também existem casos mais incomuns em que a resincronização é necessária. Por exemplo, tente editar o `serverUrl` na sandbox acima enquanto o chat estiver aberto. Observe como o Effect resincroniza em resposta às suas edições no código. No futuro, o React pode adicionar mais recursos que dependem da resincronização.
...
Toda vez após seu componente renderizar novamente, o React olhará para o array de dependências que você passou. Se algum dos valores no array for diferente do valor no mesmo local que você passou durante a renderização anterior, o React resincronizará seu Effect.
...
**Um valor mutável como [`ref.current`](/reference/react/useRef#reference) ou coisas que você lê dele também não pode ser uma dependência.** O objeto ref retornado por `useRef` em si pode ser uma dependência, mas sua propriedade `current` é intencionalmente mutável. Ele permite que você [mantenha o controle de algo sem acionar uma re-renderização.](/learn/referencing-values-with-refs) Mas como alterá-lo não aciona uma re-renderização, não é um valor reativo, e o React não saberá executar seu Effect quando ele mudar.
...
**No entanto, você poderia "provar" ao linter que esses valores não são reativos,** ou seja, que eles *não podem* mudar como resultado de uma nova renderização. Por exemplo, se `serverUrl` e `roomId` não dependem da renderização e sempre têm os mesmos valores, você pode movê-los para fora do componente. Agora eles não precisam ser dependências:
...
**Effects são blocos de código reativos.** Eles ressincronizam quando os valores que você lê dentro deles mudam. Diferente dos manipuladores de eventos, que rodam apenas uma vez por interação, os Effects rodam sempre que a sincronização é necessária.

**Você não pode "escolher" suas dependências.** Suas dependências devem incluir todo [valor reativo](#all-variables-declared-in-the-component-body-are-reactive) que você lê dentro do Effect. O linter impõe isso. Às vezes, isso pode levar a problemas como loops infinitos e fazer com que seu Effect ressincronize com muita frequência. Não corrija esses problemas suprimindo o linter! Veja o que tentar em vez disso:
...
- Cada Effect tem um ciclo de vida separado do componente ao redor.
...
No entanto, há um problema. Sempre que você digita na caixa de mensagem na parte inferior, `ChatRoom` *também* se reconecta ao chat. (Você pode notar isso limpando o console e digitando na caixa de entrada.) Corrija o problema para que isso não aconteça.
...
Este Effect não tinha um array de dependências, então ele ressincronizava após cada nova renderização. Primeiro, adicione um array de dependências. Em seguida, certifique-se de que cada valor reativo usado pelo Effect esteja especificado no array. Por exemplo, `roomId` é reativo (porque é uma prop), então ele deve ser incluído no array. Isso garante que, quando o usuário selecionar uma sala diferente, o chat se reconectará. Por outro lado, `serverUrl` é definido fora do componente. É por isso que ele não precisa estar no array.
...
Há também uma caixa de seleção. Marcar a caixa de seleção alterna a variável de estado `canMove`, mas essa variável de estado não é usada em nenhum lugar no código. Sua tarefa é alterar o código para que, quando `canMove` for `false` (a caixa de seleção estiver desmarcada), o ponto pare de se mover. Depois de alternar a caixa de seleção de volta (e definir `canMove` como `true`), o ponto deve seguir o movimento novamente. Em outras palavras, se o ponto pode se mover ou não deve permanecer sincronizado com se a caixa de seleção está marcada.
...
Em ambos os casos, `canMove` é uma variável reativa que você lê dentro do Effect. É por isso que ela deve ser especificada na lista de dependências do Effect. Isso garante que o Effect ressincronize após cada mudança em seu valor.

</Solution>

#### Investigue um erro de valor desatualizado (stale value bug) {/*investigate-a-stale-value-bug*/}

Neste exemplo, o ponto rosa deveria se mover quando a caixa de seleção está marcada e parar de se mover quando desmarcada. A lógica para isso já foi implementada: o manipulador de eventos `handleMove` verifica a variável de estado `canMove`.

No entanto, por algum motivo, a variável de estado `canMove` dentro de `handleMove` parece estar "desatualizada" (stale): ela está sempre como `true`, mesmo depois de desmarcar a caixa de seleção. Como isso é possível? Encontre o erro no código e corrija-o.

<Hint>

Se você vir uma regra do linter sendo suprimida, remova a supressão! É aí que os erros geralmente se encontram.
...
O autor do código original "mentiu" para o React, dizendo que o Effect não depende (`[]`) de nenhum valor reativo. É por isso que o React não resincronizou o Effect após `canMove` ter mudado (e `handleMove` com ele). Como o React não resincronizou o Effect, o `handleMove` adicionado como ouvinte é a função `handleMove` criada durante a renderização inicial. Durante a renderização inicial, `canMove` era `true`, e é por isso que o `handleMove` da renderização inicial sempre verá esse valor.
...
Esta solução funciona, mas não é ideal. Se você colocar `console.log('Resubscribing')` dentro do Effect, notará que ele ressubscreve após cada re-renderização. Ressubscrever é rápido, mas ainda assim seria bom evitar fazer isso com tanta frequência.
...
Neste exemplo, existem duas caixas de seleção. Uma caixa de seleção permite ao usuário escolher um planeta. Outra caixa de seleção permite ao usuário escolher um lugar *nesse planeta*. A segunda caixa ainda não funciona. Sua tarefa é fazê-la mostrar os lugares do planeta escolhido.
