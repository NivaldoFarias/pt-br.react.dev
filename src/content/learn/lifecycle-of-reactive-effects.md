...
function ChatRoom({ roomId }) {
  ...
  /*
    O efeito precisa reconectar no início, desconectar no final,
...
#### Faça o relógio funcionar {/*make-the-clock-work*/}

Este componente exibe um relógio que não atualiza. Corrija o código para que o relógio realmente atualize a hora a cada segundo.

<Hint>

Você provavelmente precisará de um `setInterval`. Não se esqueça de limpar ele!
...
function Clock() {
  ...
  return (
    <h2>
      A hora atual é: {time.toLocaleTimeString()}
    </h2>
  );
}
...
É correto que o `createConnection` é uma dependência. No entanto, este código é um pouco frágil porque alguém poderia editar o componente `App` para passar uma função inline como o valor desta prop. Nesse caso, seu valor seria diferente toda vez que o componente `App` renderizasse novamente, então o Effect pode ressincronizar muitas vezes. Para evitar isso, você pode passar `isEncrypted` para baixo em vez disso:
...
Se você implementar isso corretamente, selecionar um planeta deve preencher a lista de lugares. Mudar de planeta deve mudar a lista de lugares.
...
export default function Page() {
  ...
  useEffect(() => {
    if (planetId === '') {
      // Nada é selecionado na primeira caixa ainda
      return;
    }
...
Verifique a aba `useSelectOptions.js` no sandbox para ver como funciona. Idealmente, a maioria dos Effects em seu aplicativo deve, eventualmente, serem substituídos por Hooks personalizados, sejam escritos por você ou pela comunidade. Os Hooks personalizados ocultam a lógica de sincronização, portanto, o componente que faz a chamada não sabe sobre o Effect. À medida que você continua trabalhando em seu aplicativo, você desenvolverá uma paleta de Hooks para escolher e, eventualmente, não precisará escrever Effects em seus componentes com muita frequência.
