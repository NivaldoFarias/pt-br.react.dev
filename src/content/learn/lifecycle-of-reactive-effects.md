...
- Como pensar em cada Efeito individualmente
...
**Um valor mutável como [`ref.current`](/reference/react/useRef#reference) ou coisas que você lê dele também não podem ser uma dependência.** O objeto ref retornado por `useRef` em si pode ser uma dependência, mas sua propriedade `current` é intencionalmente mutável. Ele permite que você [mantenha o controle de algo sem acionar uma re-renderização.](/learn/referencing-values-with-refs) Mas como mudá-lo não aciona uma re-renderização, não é um valor reativo, e o React não saberá executar seu Effect quando ele mudar.
...
*   **Verifique se o seu Effect representa um processo de sincronização independente.** Se o seu Effect não sincroniza nada, [ele pode ser desnecessário.](/learn/you-might-not-need-an-effect) Se ele sincroniza várias coisas independentes, [divida-o.](#each-effect-represents-a-separate-synchronization-process)
...
No entanto, por algum motivo, a variável de estado `canMove` dentro de `handleMove` parece estar "desatualizada" (stale): ela está sempre como `true`, mesmo depois de você desmarcar a caixa de seleção. Como isso é possível? Encontre o erro no código e corrija-o.

<Hint>

Se você vir uma regra do linter sendo suprimida, remova a supressão! É aí que os erros geralmente se escondem.
...
O problema com o código original era a supressão do linter de dependências. Se você remover a supressão, verá que este Effect depende da função `handleMove`. Isso faz sentido: `handleMove` é declarada dentro do corpo do componente, o que a torna um valor reativo. Todo valor reativo deve ser especificado como dependência, ou ele pode potencialmente ficar desatualizado (stale) com o tempo!
...
**Se você nunca suprimir o linter, nunca terá problemas com valores desatualizados (stale values).** Existem algumas maneiras diferentes de resolver este erro, mas você deve sempre começar removendo a supressão do linter. Em seguida, altere o código para corrigir o erro do linter.
...
Observe que, inicialmente, os logs do console indicam que a conexão não está criptografada. Tente marcar a caixa de seleção: nada acontecerá. No entanto, se você alterar a sala selecionada depois disso, o chat se reconectará *e* ativará a criptografia (como você verá nas mensagens do console). Este é um erro. Corrija o erro para que marcar a caixa de seleção *também* faça o chat se reconectar.
