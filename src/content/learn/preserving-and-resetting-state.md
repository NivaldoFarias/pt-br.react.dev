...
O estado é isolado entre componentes. O React acompanha qual estado pertence a qual componente com base em sua posição na árvore de renderização. Você pode controlar quando preservar o estado e quando resetá-lo entre as re-renderizações.
...
## O estado está atrelado a uma posição na árvore de renderização {/*state-is-tied-to-a-position-in-the-tree*/}

O React constrói [árvores de renderização](learn/understanding-your-ui-as-a-tree#the-render-tree) para a estrutura de componentes na sua interface.

Quando você dá estado a um componente, você pode pensar que o estado "vive" dentro do componente. Mas o estado é, na verdade, mantido dentro do React. O React associa cada pedaço de estado que está mantendo ao componente correto pela sua posição na árvore de renderização.
...
Aqui está como eles se parecem como uma árvore:    
...
Tente clicar em ambos os contadores e note que eles não afetam um ao outro:
...
**O React preserva o estado de um componente enquanto ele estiver sendo renderizado em sua posição na árvore de renderização.** Se ele for removido, ou um componente diferente for renderizado na mesma posição, o React descarta seu estado.
...
Lembre-se que **é a posição na árvore de renderização — não no markup JSX — que importa para o React!** Este componente tem duas cláusulas `return` com tags `<Counter />` diferentes dentro e fora do `if`:
...
O estado do contador é resetado quando você clica na caixa de seleção. Embora você renderize um `Counter`, o primeiro filho do `div` muda de uma `section` para um `div`. Quando o `section` filho foi removido do DOM, toda a árvore abaixo dele (incluindo o `Counter` e seu estado) também foi destruída.

<DiagramGroup>

<Diagram name="preserving_state_diff_same_pt1" height={350} width={794} alt="Diagrama com três seções, com uma seta transicionando cada seção entre si. A primeira seção contém um componente React rotulado 'div' com um único filho rotulado 'section', que tem um único filho rotulado 'Counter' contendo uma bolha de estado rotulada 'count' com valor 3. A seção do meio tem o mesmo pai 'div', mas os componentes filhos foram agora deletados, indicado por uma imagem 'proof' amarela. A terceira seção tem o mesmo pai 'div' novamente, agora com um novo filho rotulado 'div', destacado em amarelo, também com um novo filho rotulado 'Counter' contendo uma bolha de estado rotulada 'count' com valor 0, tudo destacado em amarelo.">

Quando `section` muda para `div`, o `section` é deletado e o novo `div` é adicionado
...
Ao alternar de volta, o `div` é deletado e o novo `section` é adicionado

</Diagram>

</DiagramGroup>

Como regra geral, **se você deseja preservar o estado entre as re-renderizações, a estrutura da sua árvore precisa "corresponder"** de uma renderização para outra. Se a estrutura for diferente, o estado é destruído porque o React destrói o estado quando remove um componente da árvore.
...
Atualmente, quando você muda de jogador, o placar é preservado. Os dois `Counter`s aparecem na mesma posição, então o React os considera o *mesmo* `Counter` cuja prop `person` foi alterada.
...
Neste aplicativo de chat, o componente `<Chat>` contém o estado do campo de texto:
...
Tente digitar algo no campo e, em seguida, clique em "Alice" ou "Bob" para escolher um destinatário diferente. Você notará que o estado do campo de texto é preservado porque o componente `<Chat>` é renderizado na mesma posição na árvore.
...
Agora, ao trocar de destinatário, o campo de texto é sempre limpo:
...
Em um aplicativo de chat real, você provavelmente gostaria de recuperar o estado do campo de texto quando o usuário selecionar o destinatário anterior novamente. Existem algumas maneiras de manter o estado "vivo" para um componente que não está mais visível:
...
Não importa qual estratégia você escolha, um chat _com Alice_ é conceitualmente distinto de um chat _com Bob_, então faz sentido dar uma `key` para a árvore `<Chat>` com base no destinatário atual.
...
Este exemplo mostra uma mensagem quando você pressiona o botão. No entanto, pressionar o botão também redefine acidentalmente o campo de texto. Por que isso acontece? Corrija para que pressionar o botão não redefina o texto do campo.
...
A solução mais fácil é unificar os branches para que `Form` sempre seja renderizado na mesma posição:
...
Este formulário permite que você insira o primeiro e o último nome. Ele também tem uma caixa de seleção que controla qual campo vai primeiro. Quando você marca a caixa, o campo "Last name" aparecerá antes do campo "First name".

Quase funciona, mas há um bug. Se você preencher o campo "First name" e marcar a caixa, o texto permanecerá no primeiro campo (que agora é "Last name"). Corrija para que o texto do campo também se mova quando você inverter a ordem.
...
Dê uma `key` a ambos os componentes `<Field>` em ambos os branches `if` e `else`. Isso diz ao React como "corresponder" o estado correto para cada `<Field>`, mesmo que sua ordem dentro do pai mude:
...
#### Limpar um formulário de detalhes {/*reset-a-detail-form*/}

Esta é uma lista de contatos editável. Você pode editar os detalhes do contato selecionado e, em seguida, pressionar "Salvar" para atualizá-lo, ou "Limpar" para desfazer suas alterações.

Quando você seleciona um contato diferente (por exemplo, Alice), o estado é atualizado, mas o formulário continua mostrando os detalhes do contato anterior. Corrija para que o formulário seja limpo quando o contato selecionado mudar.
...
Dê `key={selectedId}` ao componente `EditContact`. Dessa forma, alternar entre contatos diferentes limpará o formulário:
...
Quando você pressiona "Próximo", o navegador começa a carregar a próxima imagem. No entanto, como ela é exibida na mesma tag `<img>`, por padrão você ainda veria a imagem anterior até que a próxima carregasse. Isso pode ser indesejável se for importante que o texto sempre corresponda à imagem. Altere para que, no momento em que você pressionar "Próximo", a imagem anterior seja limpa imediatamente.
...
Nesta lista, cada `Contact` tem um estado que determina se "Mostrar e-mail" foi pressionado para ele. Pressione "Mostrar e-mail" para Alice e, em seguida, marque a caixa de seleção "Mostrar em ordem inversa". Você notará que é o e-mail de _Taylor_ que está expandido agora, mas o de Alice - que se moveu para o final - aparece colapsado.
