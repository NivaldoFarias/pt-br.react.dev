...
O estado é isolado entre componentes. O React acompanha qual estado pertence a qual componente com base em sua posição na árvore de renderização. Você pode controlar quando preservar o estado e quando redefini-lo entre as re-renderizações.
...
* Como as chaves e os tipos afetam a preservação do estado
...
O React manterá o estado disponível enquanto você renderizar o mesmo componente na mesma posição na árvore. Para ver isso, incremente ambos os contadores, depois remova o segundo componente desmarcando a caixa de seleção "Renderizar o segundo contador" e, em seguida, adicione-o de volta marcando-a novamente:
...
**O React preserva o estado de um componente enquanto ele estiver sendo renderizado em sua posição na árvore de UI.** Se ele for removido, ou se um componente diferente for renderizado na mesma posição, o React descartará seu estado.
...
Resetar o estado com uma chave é particularmente útil quando se lida com formulários.
...
#### Preservando estado para componentes removidos {/*preserving-state-for-removed-components*/}
...
- Você também pode usar uma fonte diferente além do estado do React. Por exemplo, você provavelmente quer que um rascunho de mensagem persista mesmo que o usuário feche acidentalmente a página. Para implementar isso, você poderia ter o componente `Chat` inicializando seu estado lendo do [`localStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage), e salvar os rascunhos lá também.
...
#### Corrigir texto de entrada desaparecendo {/*fix-disappearing-input-text*/}
...
O problema é que `Form` é renderizado em posições diferentes. No branch `if`, ele é o segundo filho do `<div>`, mas no branch `else`, ele é o primeiro filho. Portanto, o tipo de componente em cada posição muda. A primeira posição muda entre conter um `p` e um `Form`, enquanto a segunda posição muda entre conter um `Form` e um `button`. O React redefine o estado toda vez que o tipo do componente muda.
...
Dê uma `key` a ambos os componentes `<Field>` em ambos os branches `if` e `else`. Isso diz ao React como "associar" o estado correto para qualquer `<Field>`, mesmo que sua ordem dentro do pai mude:
