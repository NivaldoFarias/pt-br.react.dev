...
Tente digitar algo no campo de texto e, em seguida, clique em "Alice" ou "Bob" para escolher um destinatário diferente. Você notará que o estado do campo de texto é preservado porque o componente `<Chat>` é renderizado na mesma posição na árvore.
...
Este exemplo mostra uma mensagem quando você pressiona o botão. No entanto, pressionar o botão também redefine acidentalmente o campo de texto. Por que isso acontece? Corrija para que pressionar o botão não redefina o texto do campo de entrada.
...
Quase funciona, mas há um bug. Se você preencher o campo "First name" e marcar a caixa, o texto permanecerá no primeiro campo (que agora é "Last name"). Corrija para que o texto do campo de entrada também se mova quando você inverter a ordem.
...
Dê uma `key` a ambos os componentes `<Field>` em ambos os branches `if` e `else`. Isso diz ao React como "combinar" o estado correto para qualquer `<Field>`, mesmo que sua ordem dentro do pai mude:
